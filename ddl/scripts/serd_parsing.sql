declare
   ...
   v_number_of_comments number;
   v_idx                number;
   v_str                number;
   type v_comments_type is varray(9) of varchar2(70);
   v_comments v_comments_type := v_comments_type();
begin
   ...
   loop
      ...
      v_serd_stg_header.marsden_square						      	:= case when trim(substr(t_row_content.row_content,12,3)) is not null then substr(t_row_content.row_content,12,3) else null end; 
      v_serd_stg_header.degree_square						      	:= case when trim(substr(t_row_content.row_content,15,2)) is not null then substr(t_row_content.row_content,15,2) else null end; 
      ...
      v_serd_stg_header.position_determination_code	   		:= case when trim(substr(t_row_content.row_content,33,1)) is not null then substr(t_row_content.row_content,33,1) else null end; 
      v_serd_stg_header.position_accuracy_code			   		:= case when trim(substr(t_row_content.row_content,34,1)) is not null then substr(t_row_content.row_content,34,1) else null end; 
      ...
      v_serd_stg_header.ship_number_type					   		:= case when trim(substr(t_row_content.row_content,65,1)) is not null then substr(t_row_content.row_content,65,1) else null end;
      v_serd_stg_header.originator_cruise_number			  		:= substr(t_row_content.row_content,66,8);
      v_serd_stg_header.station_number 						     	:= case when trim(substr(t_row_content.row_content,74,6)) is not null then substr(t_row_content.row_content,74,6) else null end;
      v_serd_stg_header.institute_number						   	:= case when trim(substr(t_row_content.row_content,80,3)) is not null then substr(t_row_content.row_content,80,3) else null end;
      v_serd_stg_header.institute_number_type				     	:= case when trim(substr(t_row_content.row_content,83,1)) is not null then substr(t_row_content.row_content,83,1) else null end;
      v_serd_stg_header.land_check_indicator				   		:= substr(t_row_content.row_content,84,1); -- There is a filler after
      v_serd_stg_header.depth_level_number				     		:= case when trim(substr(t_row_content.row_content,86,2)) is not null then substr(t_row_content.row_content,86,2) else null end;
    ...
    --v_serd_stg_header.min_observation_depth			     		:= substr(t_row_content.row_content,93,4); LAST LINE HAS THE VALUE WITH A '-'
      v_serd_stg_header.max_observation_depth			     		:= case when trim(substr(t_row_content.row_content,97,4)) is not null then substr(t_row_content.row_content,97,4) else null end;
    ...
    --v_serd_stg_header.sound_velocity_correction		   		:= substr(t_row_content.row_content,112,3); SOME VALUES HAVE '+  ' 
      v_serd_stg_header.observation_unit_code			     		:= case when trim(substr(t_row_content.row_content,115,1)) is not null then substr(t_row_content.row_content,115,1) else null end;
      v_serd_stg_header.instrument_code				      		:= case when trim(substr(t_row_content.row_content,116,2)) is not null then substr(t_row_content.row_content,116,2) else null end;
    --v_serd_stg_header.data_type_code					      		:= substr(t_row_content.row_content,118,1); DONT GET IT
      v_serd_stg_header.data_mode_code					      		:= case when trim(substr(t_row_content.row_content,119,1)) is not null then substr(t_row_content.row_content,119,1) else null end;
      v_serd_stg_header.data_method_code			   				:= case when trim(substr(t_row_content.row_content,120,1)) is not null then substr(t_row_content.row_content,120,1) else null end;
...
--
      if v_serd_stg_header.record_type = 2 then
         v_number_of_comments := case when trim(substr(t_row_content.row_content,215,2)) is not null then substr(t_row_content.row_content,215,2) else null end;
         
         dbms_output.put_line('v_number_of_comments:' || v_number_of_comments);
         
         if v_number_of_comments is not null and v_number_of_comments > 0 then
            v_str := 217;
            for v_idx in 1 .. v_number_of_comments
            loop
               v_comments.extend;
               v_comments(v_comments.last) := substr(t_row_content.row_content, v_str, 70);
               v_str := v_str + 70;
               dbms_output.put_line('comment:' || v_comments(v_idx));
            end loop;
         end if;
      end if;
