...
(
   ...
   where  a.meic_number        = nvl(p.meic_number,a.meic_number)
   and    a.job_number         = nvl(p.meds_job_number,a.job_number)
   and    a.meds_cruise_number = nvl(p.meds_cruise_number,a.meds_cruise_number)
   and    a.meds_ship_number   = nvl(p.meds_ship_number,a.meds_ship_number)
   and    a.instrument_code    = nvl(p.instrument_code,a.instrument_code)
   and    a.originator         = nvl(p.originator,a.originator)
)
select p.icon
,      p.color
,      b.meds_job_number
