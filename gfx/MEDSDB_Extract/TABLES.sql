--------------------------------------------------------
--  DDL for Table ACCESS_CONTROL
--------------------------------------------------------

  CREATE TABLE "MEDSADMIN"."ACCESS_CONTROL" 
   (	"TABLE_NAME" VARCHAR2(32), 
	"USER_NAME" VARCHAR2(32), 
	"MULTI_SELECT" NUMBER(1,0)
   ) ;
--------------------------------------------------------
--  DDL for Table ADCP_DATA
--------------------------------------------------------

  CREATE TABLE "MEDSADMIN"."ADCP_DATA" 
   (	"MEDS_JOB_NUMBER" NUMBER(10,0), 
	"MEDS_OBSERVATION_NUMBER" NUMBER(10,0), 
	"DEPTH_BSL_TRANS" NUMBER(12,4), 
	"HEADING" NUMBER(12,4), 
	"PITCH" NUMBER(12,4), 
	"ROLL" NUMBER(12,4), 
	"COMPASS" NUMBER(12,4), 
	"TEMPERATURE" NUMBER(12,4), 
	"BIN_SIZE" NUMBER(12,4), 
	"BIN_1_DISTANCE" NUMBER(12,4), 
	"ENSEMBLE" VARCHAR2(20), 
	"WATER_DEPTH" NUMBER(20,10), 
	"NO_BINS" NUMBER(10,0), 
	"NO_ENSEMBLES" NUMBER(10,0), 
	"SPEED_SHIP" NUMBER(20,10), 
	"DIR_SHIP" NUMBER(20,10), 
	"U_SHIP" NUMBER(20,10), 
	"V_SHIP" NUMBER(20,10), 
	"U_BOTTOM" NUMBER(20,10), 
	"V_BOTTOM" NUMBER(20,10), 
	"SCALE_FACTOR" NUMBER(12,4), 
	"PROFILE" NUMBER(10,0), 
	"JULIAN_DAY" NUMBER(12,4)
   ) ;
--------------------------------------------------------
--  DDL for Table ADCP_OBSERVATION
--------------------------------------------------------

  CREATE TABLE "MEDSADMIN"."ADCP_OBSERVATION" 
   (	"LOCATION" "MDSYS"."SDO_GEOMETRY" , 
	"MEDS_JOB_NUMBER" NUMBER(10,0), 
	"MEDS_OBSERVATION_NUMBER" NUMBER(10,0), 
	"LONGITUDE" NUMBER(20,10), 
	"LATITUDE" NUMBER(20,10), 
	"DATE_RECORDED" DATE, 
	"ENSEMBLE" VARCHAR2(20), 
	"COG" NUMBER(12,4), 
	"PROFILE" NUMBER(10,0)
   ) ;
--------------------------------------------------------
--  DDL for Table ADCP_REPEAT
--------------------------------------------------------

  CREATE TABLE "MEDSADMIN"."ADCP_REPEAT" 
   (	"MEDS_JOB_NUMBER" NUMBER(10,0), 
	"MEDS_OBSERVATION_NUMBER" NUMBER(10,0), 
	"DATA_ID" NUMBER(10,0), 
	"BIN" NUMBER(10,0), 
	"SPEED" NUMBER(20,10), 
	"DIRECTION" NUMBER(20,10), 
	"U_EAST_WEST" NUMBER(20,10), 
	"V_NORTH_SOUTH" NUMBER(20,10), 
	"W_VERTICAL" NUMBER(20,10), 
	"ERROR" NUMBER(20,10), 
	"PERCENTAGE_ERROR" NUMBER(10,0), 
	"ECHO_INT_AVERAGE" NUMBER(12,4), 
	"DEPTH_BSL" NUMBER(12,4), 
	"QUALITY" NUMBER(12,6), 
	"ENSEMBLE" VARCHAR2(20), 
	"PROFILE" NUMBER(10,0)
   ) ;
--------------------------------------------------------
--  DDL for Table AQUA2081_LINE_DATA
--------------------------------------------------------

  CREATE TABLE "MEDSADMIN"."AQUA2081_LINE_DATA" 
   (	"LATITUDE" NUMBER(12,4), 
	"LONGITUDE" NUMBER(12,4), 
	"DATE_RECORDED" DATE, 
	"PRESSURE" NUMBER(12,4), 
	"DEPTH" NUMBER(12,4), 
	"TEMPERATURE" NUMBER(12,4), 
	"SALINITY" NUMBER(12,4), 
	"CONDUCTIVITY" NUMBER(12,4), 
	"SOUND_SPEED" NUMBER(12,4), 
	"BIOLUMMIN" NUMBER(12,4), 
	"BIOLUMMAX" NUMBER(12,4), 
	"BIOLUMSD" NUMBER(12,4), 
	"BIOLUMN" NUMBER(6,0), 
	"CHLORN" NUMBER(6,0), 
	"GELBN" NUMBER(6,0), 
	"HYDRON" NUMBER(6,0), 
	"SOUNDMIN" NUMBER(12,4), 
	"SOUNDMAX" NUMBER(12,4), 
	"SOUNDSD" NUMBER(12,4), 
	"SOUNDN" NUMBER(6,0), 
	"TEMPMIN" NUMBER(12,4), 
	"TEMPMAX" NUMBER(12,4), 
	"TEMPSD" NUMBER(12,4), 
	"TEMPN" NUMBER(6,0), 
	"CONDMIN" NUMBER(12,4), 
	"CONDMAX" NUMBER(12,4), 
	"CONDSD" NUMBER(12,4), 
	"CONDN" NUMBER(6,0), 
	"SALMAX" NUMBER(12,4), 
	"SALMIN" NUMBER(12,4), 
	"SALSD" NUMBER(12,4), 
	"SALN" NUMBER(6,0), 
	"MEDS_JOB_NUMBER" NUMBER(10,0), 
	"MEDS_OBSERVATION_NUMBER" NUMBER(10,0), 
	"BIOLUMINESCENCE" NUMBER(20,4), 
	"GELBSTOFFE" NUMBER(20,17), 
	"CHLOROPHYLL" NUMBER(20,17), 
	"HYDROCARBONS" NUMBER(20,17), 
	"GELBMIN" NUMBER(20,17), 
	"GELBMAX" NUMBER(20,17), 
	"GELBSD" NUMBER(20,17), 
	"HYDROMIN" NUMBER(20,17), 
	"HYDROMAX" NUMBER(20,17), 
	"HYDROSD" NUMBER(20,17), 
	"CHLORMIN" NUMBER(20,17), 
	"CHLORMAX" NUMBER(20,17), 
	"CHLORSD" NUMBER(20,17), 
	"SEABED_DEPTH" NUMBER(12,4)
   ) ;
--------------------------------------------------------
--  DDL for Table AQUA2081_LINE_OBSERVATION
--------------------------------------------------------

  CREATE TABLE "MEDSADMIN"."AQUA2081_LINE_OBSERVATION" 
   (	"LOCATION" "MDSYS"."SDO_GEOMETRY" , 
	"MEDS_JOB_NUMBER" NUMBER(10,0), 
	"MEDS_OBSERVATION_NUMBER" NUMBER(10,0), 
	"LATITUDE" NUMBER(12,4), 
	"LONGITUDE" NUMBER(12,4), 
	"DATE_RECORDED" DATE
   ) ;
--------------------------------------------------------
--  DDL for Table AQUAPACK_JOB
--------------------------------------------------------

  CREATE TABLE "MEDSADMIN"."AQUAPACK_JOB" 
   (	"VALIDATED" VARCHAR2(1), 
	"TEMPORAL_RESOLUTION" VARCHAR2(10), 
	"GEOGRAPHICAL_RESOLUTION" VARCHAR2(20), 
	"DATE_ARCHIVED" DATE, 
	"SENSOR_UNIT_NUMBER" VARCHAR2(16), 
	"VESSEL" VARCHAR2(50), 
	"COMMENTS" VARCHAR2(500), 
	"JOB_NUMBER" NUMBER(10,0)
   ) ;
--------------------------------------------------------
--  DDL for Table AQUAPACK_LINE_DATA
--------------------------------------------------------

  CREATE TABLE "MEDSADMIN"."AQUAPACK_LINE_DATA" 
   (	"PRESSURE" NUMBER(12,4), 
	"DEPTH" NUMBER(12,4), 
	"TEMPERATURE" NUMBER(12,4), 
	"SALINITY" NUMBER(12,4), 
	"CONDUCTIVITY" NUMBER(12,4), 
	"SOUND_SPEED" NUMBER(12,4), 
	"BIOLUMMIN" NUMBER(12,4), 
	"BIOLUMMAX" NUMBER(12,4), 
	"BIOLUMSD" NUMBER(12,4), 
	"BIOLUMN" NUMBER(6,0), 
	"CHLORN" NUMBER(6,0), 
	"GELBN" NUMBER(6,0), 
	"HYDRON" NUMBER(6,0), 
	"SOUNDMIN" NUMBER(12,4), 
	"SOUNDMAX" NUMBER(12,4), 
	"SOUNDSD" NUMBER(12,4), 
	"SOUNDN" NUMBER(6,0), 
	"TEMPMIN" NUMBER(12,4), 
	"TEMPMAX" NUMBER(12,4), 
	"TEMPSD" NUMBER(12,4), 
	"TEMPN" NUMBER(6,0), 
	"CONDMIN" NUMBER(12,4), 
	"CONDMAX" NUMBER(12,4), 
	"CONDSD" NUMBER(12,4), 
	"CONDN" NUMBER(6,0), 
	"SALMAX" NUMBER(12,4), 
	"SALMIN" NUMBER(12,4), 
	"SALSD" NUMBER(12,4), 
	"SALN" NUMBER(6,0), 
	"MEDS_JOB_NUMBER" NUMBER(10,0), 
	"MEDS_OBSERVATION_NUMBER" NUMBER(10,0), 
	"HYDROCARBONS" NUMBER(20,17), 
	"GELBSTOFFE" NUMBER(20,17), 
	"CHLOROPHYLL" NUMBER(20,17), 
	"BIOLUMINESCENCE" NUMBER(20,4), 
	"HYDROMIN" NUMBER(20,17), 
	"HYDROMAX" NUMBER(20,17), 
	"HYDROSD" NUMBER(20,17), 
	"CHLORMIN" NUMBER(20,17), 
	"CHLORMAX" NUMBER(20,17), 
	"CHLORSD" NUMBER(20,17), 
	"GELBMIN" NUMBER(20,17), 
	"GELBMAX" NUMBER(20,17), 
	"GELBSD" NUMBER(20,17), 
	"LATITUDE" NUMBER(12,4), 
	"LONGITUDE" NUMBER(12,4), 
	"DATE_RECORDED" DATE, 
	"SEABED_DEPTH" NUMBER(12,4)
   ) ;
--------------------------------------------------------
--  DDL for Table AQUAPACK_LINE_OBSERVATION
--------------------------------------------------------

  CREATE TABLE "MEDSADMIN"."AQUAPACK_LINE_OBSERVATION" 
   (	"LOCATION" "MDSYS"."SDO_GEOMETRY" , 
	"MEDS_JOB_NUMBER" NUMBER(10,0), 
	"MEDS_OBSERVATION_NUMBER" NUMBER(10,0), 
	"LATITUDE" NUMBER(12,4), 
	"LONGITUDE" NUMBER(12,4), 
	"DATE_RECORDED" DATE
   ) ;
--------------------------------------------------------
--  DDL for Table AQUAPACK_PROFILE_DATA
--------------------------------------------------------

  CREATE TABLE "MEDSADMIN"."AQUAPACK_PROFILE_DATA" 
   (	"PRESSURE" NUMBER(12,4), 
	"DEPTH" NUMBER(12,4), 
	"TEMPERATURE" NUMBER(12,4), 
	"SALINITY" NUMBER(12,4), 
	"CONDUCTIVITY" NUMBER(12,4), 
	"SOUND_SPEED" NUMBER(12,4), 
	"BIOLUMMIN" NUMBER(12,4), 
	"BIOLUMMAX" NUMBER(12,4), 
	"BIOLUMSD" NUMBER(12,4), 
	"BIOLUMN" NUMBER(6,0), 
	"CHLORN" NUMBER(6,0), 
	"GELBN" NUMBER(6,0), 
	"HYDRON" NUMBER(6,0), 
	"SOUNDMIN" NUMBER(12,4), 
	"SOUNDMAX" NUMBER(12,4), 
	"SOUNDSD" NUMBER(12,4), 
	"SOUNDN" NUMBER(6,0), 
	"TEMPMIN" NUMBER(12,4), 
	"TEMPMAX" NUMBER(12,4), 
	"TEMPSD" NUMBER(12,4), 
	"TEMPN" NUMBER(6,0), 
	"CONDMIN" NUMBER(12,4), 
	"CONDMAX" NUMBER(12,4), 
	"CONDSD" NUMBER(12,4), 
	"CONDN" NUMBER(6,0), 
	"SALMAX" NUMBER(12,4), 
	"SALMIN" NUMBER(12,4), 
	"SALSD" NUMBER(12,4), 
	"SALN" NUMBER(6,0), 
	"MEDS_JOB_NUMBER" NUMBER(10,0), 
	"MEDS_OBSERVATION_NUMBER" NUMBER(10,0), 
	"BIOLUMINESCENCE" NUMBER(20,4), 
	"GELBSTOFFE" NUMBER(20,17), 
	"CHLOROPHYLL" NUMBER(20,17), 
	"HYDROCARBONS" NUMBER(20,17), 
	"GELBMIN" NUMBER(20,17), 
	"GELBMAX" NUMBER(20,17), 
	"GELBSD" NUMBER(20,17), 
	"HYDROMIN" NUMBER(20,17), 
	"HYDROMAX" NUMBER(20,17), 
	"HYDROSD" NUMBER(20,17), 
	"CHLORMIN" NUMBER(20,17), 
	"CHLORMAX" NUMBER(20,17), 
	"CHLORSD" NUMBER(20,17)
   ) ;
--------------------------------------------------------
--  DDL for Table AQUAPACK_PROFILE_OBSERVATION
--------------------------------------------------------

  CREATE TABLE "MEDSADMIN"."AQUAPACK_PROFILE_OBSERVATION" 
   (	"LOCATION" "MDSYS"."SDO_GEOMETRY" , 
	"MEDS_JOB_NUMBER" NUMBER(10,0), 
	"MEDS_OBSERVATION_NUMBER" NUMBER(10,0), 
	"LONGITUDE" NUMBER(12,4), 
	"LATITUDE" NUMBER(12,4), 
	"DATE_RECORDED" DATE, 
	"SEABED_DEPTH" NUMBER(12,0) DEFAULT 0
   ) ;
--------------------------------------------------------
--  DDL for Table ASRAP_POINT_CADS_REPEAT
--------------------------------------------------------

  CREATE TABLE "MEDSADMIN"."ASRAP_POINT_CADS_REPEAT" 
   (	"MEDS_JOB_NUMBER" NUMBER(10,0), 
	"MEDS_OBSERVATION_NUMBER" NUMBER(10,0), 
	"MEAN_DEPTH" VARCHAR2(4), 
	"YEAR" NUMBER(4,0), 
	"QUALITY" VARCHAR2(1), 
	"NUMBER_OF_DEPTH_LEVELS" NUMBER(2,0), 
	"NUMBER_OF_COMMENTS" NUMBER(2,0), 
	"COMMENTS" VARCHAR2(650), 
	"MONTH" VARCHAR2(2)
   ) ;
--------------------------------------------------------
--  DDL for Table ASRAP_POINT_DATA
--------------------------------------------------------

  CREATE TABLE "MEDSADMIN"."ASRAP_POINT_DATA" 
   (	"HFBL" NUMBER(12,3), 
	"LFBL" NUMBER(12,3), 
	"SHIPPING_NOISE" NUMBER(12,3), 
	"MEAN_DEPTH" VARCHAR2(4), 
	"MEAN_DEEP_DEPTH" VARCHAR2(4), 
	"JAN_HEIGHT" VARCHAR2(5), 
	"FEB_HEIGHT" VARCHAR2(5), 
	"MAR_HEIGHT" VARCHAR2(5), 
	"APR_HEIGHT" VARCHAR2(5), 
	"MAY_HEIGHT" VARCHAR2(5), 
	"JUN_HEIGHT" VARCHAR2(5), 
	"JUL_HEIGHT" VARCHAR2(5), 
	"AUG_HEIGHT" VARCHAR2(5), 
	"SEP_HEIGHT" VARCHAR2(5), 
	"OCT_HEIGHT" VARCHAR2(5), 
	"NOV_HEIGHT" VARCHAR2(5), 
	"DEC_HEIGHT" VARCHAR2(5), 
	"MEDS_JOB_NUMBER" NUMBER(10,0), 
	"MEDS_OBSERVATION_NUMBER" NUMBER(10,0), 
	"HO_CAVEAT" VARCHAR2(25)
   ) ;
--------------------------------------------------------
--  DDL for Table ASRAP_POINT_OBSERVATION
--------------------------------------------------------

  CREATE TABLE "MEDSADMIN"."ASRAP_POINT_OBSERVATION" 
   (	"DOMAIN_ID" VARCHAR2(10), 
	"LOCATION" "MDSYS"."SDO_GEOMETRY" , 
	"MEDS_JOB_NUMBER" NUMBER(10,0), 
	"MEDS_OBSERVATION_NUMBER" NUMBER(10,0), 
	"LONGITUDE" NUMBER(12,6), 
	"LATITUDE" NUMBER(12,6)
   ) ;
--------------------------------------------------------
--  DDL for Table ASRAP_POINT_REPEAT
--------------------------------------------------------

  CREATE TABLE "MEDSADMIN"."ASRAP_POINT_REPEAT" 
   (	"DEPTH" NUMBER(12,3), 
	"TEMPERATURE" NUMBER(12,3), 
	"SALINITY" NUMBER(12,3), 
	"SOUND_VELOCITY" NUMBER(12,3), 
	"SOURCE_CODE" VARCHAR2(1), 
	"SV_CODE" VARCHAR2(1), 
	"MEDS_JOB_NUMBER" NUMBER(10,0), 
	"MEDS_OBSERVATION_NUMBER" NUMBER(10,0), 
	"MONTH" VARCHAR2(2)
   ) ;
--------------------------------------------------------
--  DDL for Table ASRAP_POLYGON_OBSERVATION
--------------------------------------------------------

  CREATE TABLE "MEDSADMIN"."ASRAP_POLYGON_OBSERVATION" 
   (	"LOCATION" "MDSYS"."SDO_GEOMETRY" , 
	"MEDS_JOB_NUMBER" NUMBER(10,0), 
	"MEDS_OBSERVATION_NUMBER" NUMBER(10,0), 
	"DOMAIN_ID" VARCHAR2(10)
   ) ;
--------------------------------------------------------
--  DDL for Table ASRAP_POLYGON_REPEAT
--------------------------------------------------------

  CREATE TABLE "MEDSADMIN"."ASRAP_POLYGON_REPEAT" 
   (	"HO_CAVEAT_1" VARCHAR2(4), 
	"HO_CAVEAT_2" VARCHAR2(5), 
	"ICES_QUADRANT" VARCHAR2(1), 
	"MARSDEN_SQUARE" VARCHAR2(3), 
	"DEGREE_SQUARE" VARCHAR2(2), 
	"MEDS_JOB_NUMBER" NUMBER(10,0), 
	"MEDS_OBSERVATION_NUMBER" NUMBER(10,0), 
	"LATITUDE" NUMBER(12,8), 
	"LONGITUDE" NUMBER(12,8), 
	"POINT_ORDER" NUMBER(8,0)
   ) ;
--------------------------------------------------------
--  DDL for Table BEACH_DATA
--------------------------------------------------------

  CREATE TABLE "MEDSADMIN"."BEACH_DATA" 
   (	"TBLBEACHESID" VARCHAR2(20), 
	"MAP" VARCHAR2(10), 
	"SHEET" VARCHAR2(10), 
	"GRID" VARCHAR2(10), 
	"ANCHORAGE" VARCHAR2(500), 
	"SURF_CONDITIONS" VARCHAR2(20), 
	"SURF_SEA_SWELL" VARCHAR2(600), 
	"APPROACH_SEAWARD" VARCHAR2(700), 
	"OBSTRUCTIONS" VARCHAR2(500), 
	"JETTIES_RAMPS_HARDS" VARCHAR2(600), 
	"AIR_LANDING_SITES" VARCHAR2(500), 
	"WATER_AND_ENGINEERING" VARCHAR2(600), 
	"PUBLICATION_STATUS" VARCHAR2(500), 
	"CD_ROM_REFERENCE" VARCHAR2(50), 
	"TYPE_OF_IMAGERY" VARCHAR2(100), 
	"CLASSIFICATION" VARCHAR2(20), 
	"ORIGINATOR" VARCHAR2(50), 
	"CURRENT_SPEED" VARCHAR2(20), 
	"CURRENT_ORIENTATION" VARCHAR2(20), 
	"GRADIENT_OVERALL" VARCHAR2(20), 
	"GRADIENT_SURVEY_DATE" VARCHAR2(20), 
	"GRADIENT_BEARING" VARCHAR2(20), 
	"DROP_ZONE_DESCRIPTION" VARCHAR2(500), 
	"DROP_ZONE_LOCATION" VARCHAR2(100), 
	"LINK_TO_GROUND_PHOTO" VARCHAR2(500), 
	"LINK_TO_BEACH_DIAGRAM" VARCHAR2(500), 
	"LINK_TO_GRADIENT_DIAGRAM" VARCHAR2(500), 
	"CARD_2" VARCHAR2(20), 
	"MEDS_JOB_NUMBER" NUMBER(10,0), 
	"MEDS_OBSERVATION_NUMBER" NUMBER(10,0), 
	"CARD_1" VARCHAR2(10), 
	"EXIT_DESCRIPTION" VARCHAR2(700), 
	"HINTERLAND" VARCHAR2(800)
   ) ;
--------------------------------------------------------
--  DDL for Table BEACH_OBSERVATION
--------------------------------------------------------

  CREATE TABLE "MEDSADMIN"."BEACH_OBSERVATION" 
   (	"BEACH_NAME" VARCHAR2(50), 
	"BEACH_CATEGORY" NUMBER(1,0), 
	"BEACH_NUMBER" VARCHAR2(20), 
	"DATE_OF_SURVEY" DATE, 
	"GRADIENT_NEARSHORE" NUMBER(1,0), 
	"GRADIENT_FORESHORE" NUMBER(1,0), 
	"GRADIENT_BACKSHORE" NUMBER(1,0), 
	"TRAFFICABILITY" NUMBER(1,0), 
	"EXIT_CONDITIONS" NUMBER(1,0), 
	"SECTOR" VARCHAR2(10), 
	"LATITUDE" NUMBER(13,4), 
	"LONGITUDE" NUMBER(13,4), 
	"SURF_HEIGHT" NUMBER(10,2), 
	"BEACH_DIMENSIONS" NUMBER(10,0), 
	"QUALITY_OF_REECE" VARCHAR2(3), 
	"BEACH_COMPOSITION" VARCHAR2(17), 
	"MEDS_JOB_NUMBER" NUMBER(10,0), 
	"MEDS_OBSERVATION_NUMBER" NUMBER(10,0), 
	"LOCATION" "MDSYS"."SDO_GEOMETRY" , 
	"COUNTRY" VARCHAR2(50), 
	"BEACH_CHANGES" VARCHAR2(500), 
	"BEACH_CHANGES_NO" NUMBER(2,0)
   ) ;
--------------------------------------------------------
--  DDL for Table BIOLUMINESCENCE_DATA
--------------------------------------------------------

  CREATE TABLE "MEDSADMIN"."BIOLUMINESCENCE_DATA" 
   (	"MP_BAND" VARCHAR2(20), 
	"MP_DIRECTION" VARCHAR2(20), 
	"MP_DUR" VARCHAR2(20), 
	"AREA" VARCHAR2(200), 
	"MP_FREQ" VARCHAR2(20), 
	"MP_LENGTH" VARCHAR2(20), 
	"MP_NO" VARCHAR2(20), 
	"MP_ROTATE" VARCHAR2(20), 
	"MP_SEPERATE" VARCHAR2(20), 
	"BOWWV" VARCHAR2(20), 
	"MP_SPD" VARCHAR2(20), 
	"COLOUR" VARCHAR2(200), 
	"DIFFUSE" VARCHAR2(20), 
	"PATCH" VARCHAR2(200), 
	"STIMULUS" VARCHAR2(200), 
	"SURFACE" VARCHAR2(20), 
	"VHEAD" VARCHAR2(20), 
	"VKTS" VARCHAR2(20), 
	"WAKE" VARCHAR2(20), 
	"WDEP" VARCHAR2(20), 
	"SAMP" VARCHAR2(20), 
	"WVCRSTS" VARCHAR2(20), 
	"RECORD_NUMBER" VARCHAR2(20), 
	"VESSEL" VARCHAR2(50), 
	"PT" VARCHAR2(20), 
	"SS" VARCHAR2(50), 
	"WD_DIR" VARCHAR2(50), 
	"WD_SPD" VARCHAR2(50), 
	"MEDS_JOB_NUMBER" NUMBER(8,0), 
	"MEDS_OBSERVATION_NUMBER" NUMBER(8,0), 
	"SST" VARCHAR2(20), 
	"IDENT" VARCHAR2(50), 
	"COMMENT_1" VARCHAR2(700), 
	"AIRTD" VARCHAR2(20), 
	"AIRTW" VARCHAR2(20), 
	"BARO" VARCHAR2(20), 
	"CLASS" VARCHAR2(300), 
	"ARCHIVED" VARCHAR2(25), 
	"REFERENCE" VARCHAR2(100), 
	"RANGE" VARCHAR2(20)
   ) ;
--------------------------------------------------------
--  DDL for Table BIOLUMINESCENCE_OBSERVATION
--------------------------------------------------------

  CREATE TABLE "MEDSADMIN"."BIOLUMINESCENCE_OBSERVATION" 
   (	"MEDS_OBSERVATION_NUMBER" NUMBER(8,0), 
	"DATETIME" DATE, 
	"LATITUDE" NUMBER(12,6), 
	"LONGITUDE" NUMBER(12,6), 
	"LOCATION" "MDSYS"."SDO_GEOMETRY" , 
	"MEDS_JOB_NUMBER" NUMBER(8,0)
   ) ;
--------------------------------------------------------
--  DDL for Table BIOMASS_DATA
--------------------------------------------------------

  CREATE TABLE "MEDSADMIN"."BIOMASS_DATA" 
   (	"UPPER_DEPTH" NUMBER(8,3), 
	"LOWER_DEPTH" NUMBER(8,3), 
	"TOTAL_DISPLACEMENT_VOL" NUMBER(12,8), 
	"TOTAL_SETTLED_VOLUME" NUMBER(12,8), 
	"TOTAL_WET_MASS" NUMBER(12,8), 
	"TOTAL_DRY_MASS" NUMBER(12,8), 
	"MEDS_JOB_NUMBER" NUMBER(10,0), 
	"MEDS_OBSERVATION_NUMBER" NUMBER(10,0)
   ) ;
--------------------------------------------------------
--  DDL for Table BIOMASS_OBSERVATION
--------------------------------------------------------

  CREATE TABLE "MEDSADMIN"."BIOMASS_OBSERVATION" 
   (	"MEDS_OBSERVATION_NUMBER" NUMBER(10,0), 
	"MEDS_JOB_NUMBER" NUMBER(10,0), 
	"LOCATION" "MDSYS"."SDO_GEOMETRY" , 
	"DATE_RECORDED" DATE, 
	"LATITUDE" NUMBER(12,8), 
	"LONGITUDE" NUMBER(12,8)
   ) ;
--------------------------------------------------------
--  DDL for Table CHART_PANEL
--------------------------------------------------------

  CREATE TABLE "MEDSADMIN"."CHART_PANEL" 
   (	"CHART_NUMBER_PREFIX" CHAR(3), 
	"CHART_NUMBER" NUMBER, 
	"CHART_NUMBER_SUFFIX" CHAR(1), 
	"PANEL_NUMBER" NUMBER, 
	"MAIN_TITLE" VARCHAR2(100), 
	"SCALE" NUMBER, 
	"HORIZONTAL_DATUM_CODE" CHAR(3), 
	"PROJECTION_CODE" NUMBER, 
	"WGS_SHIFT_NS" NUMBER, 
	"WGS_SHIFT_EW" NUMBER, 
	"LIMITS" "MDSYS"."SDO_GEOMETRY" 
   ) ;
--------------------------------------------------------
--  DDL for Table CLASS_INFORMATION
--------------------------------------------------------

  CREATE TABLE "MEDSADMIN"."CLASS_INFORMATION" 
   (	"DATA_CLASS" VARCHAR2(50), 
	"DATA_TYPE" VARCHAR2(50), 
	"TEMPORAL" VARCHAR2(500), 
	"GRID" VARCHAR2(50), 
	"RESOLUTION" VARCHAR2(40), 
	"JOB_COL" VARCHAR2(30), 
	"TABLE_NAME" VARCHAR2(40), 
	"HEADER_TABLE" VARCHAR2(40), 
	"DATA_STYLE" VARCHAR2(40), 
	"FILE_NAME" VARCHAR2(30), 
	"COLOR" VARCHAR2(10), 
	"DATA_TYPE_INDEX" NUMBER(8,0)
   ) ;
--------------------------------------------------------
--  DDL for Table CRUISE_LAYER
--------------------------------------------------------

  CREATE TABLE "MEDSADMIN"."CRUISE_LAYER" 
   (	"CRUISE_GEOMETRY" "MDSYS"."SDO_GEOMETRY" , 
	"START_PORT" VARCHAR2(100), 
	"START_DATE" DATE, 
	"END_DATE" DATE, 
	"END_PORT" VARCHAR2(100), 
	"CRUISE_NAME" VARCHAR2(50), 
	"CRUISE_DESCRIPTOR" VARCHAR2(50), 
	"MEDS_CRUISE_NUMBER" NUMBER(10,0), 
	"BOOKING_NUMBER" NUMBER(10,0)
   ) ;
--------------------------------------------------------
--  DDL for Table DATASET_LOOKUP
--------------------------------------------------------

  CREATE TABLE "MEDSADMIN"."DATASET_LOOKUP" 
   (	"NAME" VARCHAR2(50), 
	"DATASET_INDEX" NUMBER(8,0)
   ) ;
--------------------------------------------------------
--  DDL for Table DATA_TYPES_TABLE
--------------------------------------------------------

  CREATE TABLE "MEDSADMIN"."DATA_TYPES_TABLE" 
   (	"DESCRIPTION" VARCHAR2(100), 
	"DATA_TYPE" VARCHAR2(20), 
	"TYPE_INDEX" NUMBER(8,0)
   ) ;
--------------------------------------------------------
--  DDL for Table DBFIELD_LOOKUP
--------------------------------------------------------

  CREATE TABLE "MEDSADMIN"."DBFIELD_LOOKUP" 
   (	"FIELD_INDEX" NUMBER(4,0), 
	"FIELD_NAME" VARCHAR2(50), 
	"HEADER_DATA_FLAG" NUMBER(1,0), 
	"DATA_TYPE" VARCHAR2(50), 
	"FILE_EXT" VARCHAR2(50), 
	"FIELD_SIZE" NUMBER(4,0), 
	"FIELD_PRECISION" NUMBER(4,0), 
	"SINGLE_REPEAT" VARCHAR2(1), 
	"FIELD_TYPE" VARCHAR2(3), 
	"FIELD_VALUE" VARCHAR2(50)
   ) ;
--------------------------------------------------------
--  DDL for Table DEEP_SCATTERING_DATA
--------------------------------------------------------

  CREATE TABLE "MEDSADMIN"."DEEP_SCATTERING_DATA" 
   (	"SONAR_FREQUENCY" NUMBER(5,0), 
	"TYPE_OF_SONAR" VARCHAR2(50), 
	"E_S_TRACE_REFERENCE" VARCHAR2(50), 
	"SHIP" VARCHAR2(100), 
	"HI_CRUISE" VARCHAR2(100), 
	"REMARKS" VARCHAR2(500), 
	"SEA_STATE" NUMBER(1,0), 
	"MEDS_OBSERVATION_NUMBER" NUMBER(10,0), 
	"MEDS_JOB_NUMBER" NUMBER(10,0), 
	"ARCHIVED" VARCHAR2(25), 
	"END_TIME" VARCHAR2(20), 
	"GAIN_SETTINGS" VARCHAR2(20), 
	"MONTH" VARCHAR2(10), 
	"START_TIME" VARCHAR2(10), 
	"DEPTH_OF_TOP_LAYER" VARCHAR2(20), 
	"LAYER_THICKNESS" VARCHAR2(20), 
	"EXTENT_OF_MOTION" VARCHAR2(20), 
	"END_LAT_LONG" VARCHAR2(50)
   ) ;
--------------------------------------------------------
--  DDL for Table DEEP_SCATTERING_OBSERVATION
--------------------------------------------------------

  CREATE TABLE "MEDSADMIN"."DEEP_SCATTERING_OBSERVATION" 
   (	"LATITUDE" NUMBER(12,6), 
	"LONGITUDE" NUMBER(12,6), 
	"DATE_RECORDED" DATE, 
	"LOCATION" "MDSYS"."SDO_GEOMETRY" , 
	"MEDS_JOB_NUMBER" NUMBER(10,0), 
	"MEDS_OBSERVATION_NUMBER" NUMBER(10,0)
   ) ;
--------------------------------------------------------
--  DDL for Table DIRECTORY_LOOKUP
--------------------------------------------------------

  CREATE TABLE "MEDSADMIN"."DIRECTORY_LOOKUP" 
   (	"USER_NAME" CHAR(50), 
	"LOOKUP_TYPE" CHAR(16), 
	"FILE_PATH" CHAR(256)
   ) ;
--------------------------------------------------------
--  DDL for Table DIR_AMBIENT_DATA
--------------------------------------------------------

  CREATE TABLE "MEDSADMIN"."DIR_AMBIENT_DATA" 
   (	"EXERSIZE" VARCHAR2(20), 
	"PLATFORM" VARCHAR2(20), 
	"ARRAY_TYPE" VARCHAR2(20), 
	"ARRAY_DEPTH" VARCHAR2(20), 
	"FREQUENCY" VARCHAR2(20), 
	"OMNI" VARCHAR2(20), 
	"STANDARD_DEVIATION" VARCHAR2(20), 
	"NUMBER_OF_ITERATIONS" VARCHAR2(20), 
	"NUMBER_OF_HEADINGS" VARCHAR2(20), 
	"WIND_SPEED" VARCHAR2(20), 
	"SEA_STATE" VARCHAR2(20), 
	"OCTAVE_NUMBER" VARCHAR2(20), 
	"RECORD_NUMBER" VARCHAR2(20), 
	"SITE_ID_" VARCHAR2(20), 
	"NUMBER_OF_FREQUENCIES" VARCHAR2(20), 
	"COMMENTS" VARCHAR2(100), 
	"WATER_DEPTH" VARCHAR2(20), 
	"MEDS_OBSERVATION_NUMBER" NUMBER(10,0), 
	"MEDS_JOB_NUMBER" NUMBER(10,0), 
	"SHIP_CODE" VARCHAR2(20), 
	"COUNTRY_CODE" VARCHAR2(20), 
	"LOCATION_DESCRIPTION" VARCHAR2(100), 
	"SPEED" VARCHAR2(20), 
	"ARCHIVE_DAT" VARCHAR2(20)
   ) ;
--------------------------------------------------------
--  DDL for Table DIR_AMBIENT_OBSERVATION
--------------------------------------------------------

  CREATE TABLE "MEDSADMIN"."DIR_AMBIENT_OBSERVATION" 
   (	"DATE_RECORDED" DATE, 
	"LOCATION" "MDSYS"."SDO_GEOMETRY" , 
	"LATITUDE" NUMBER(12,6), 
	"LONGITUDE" NUMBER(12,6), 
	"MEDS_OBSERVATION_NUMBER" NUMBER(10,0), 
	"MEDS_JOB_NUMBER" NUMBER(10,0)
   ) ;
--------------------------------------------------------
--  DDL for Table DIR_AMBIENT_REPEAT
--------------------------------------------------------

  CREATE TABLE "MEDSADMIN"."DIR_AMBIENT_REPEAT" 
   (	"SPECTRAL_LEVEL" NUMBER(5,0), 
	"DECIBELS" NUMBER(12,3), 
	"MEDS_OBSERVATION_NUMBER" NUMBER(10,0), 
	"MEDS_JOB_NUMBER" NUMBER(10,0)
   ) ;
--------------------------------------------------------
--  DDL for Table DIVE_SITE_DATA
--------------------------------------------------------

  CREATE TABLE "MEDSADMIN"."DIVE_SITE_DATA" 
   (	"SITE_NUMBER" NUMBER(5,0), 
	"NAME" VARCHAR2(100), 
	"REGION" VARCHAR2(50), 
	"AREA" VARCHAR2(50), 
	"TYPE_OF_ACTIVITY" VARCHAR2(50), 
	"INSHORE_OFFSHORE" VARCHAR2(20), 
	"COMMENTS" VARCHAR2(200), 
	"DATE_RECORDED" DATE, 
	"WATER_CLARITY" VARCHAR2(30), 
	"MEDS_OBSERVATION_NUMBER" NUMBER(10,0), 
	"MEDS_JOB_NUMBER" NUMBER(10,0), 
	"MAIN_SPECIES" VARCHAR2(250), 
	"TIME_OF_YEAR" VARCHAR2(200), 
	"DEPTH" VARCHAR2(50), 
	"LAT_LON" VARCHAR2(50)
   ) ;
--------------------------------------------------------
--  DDL for Table DIVE_SITE_OBSERVATION
--------------------------------------------------------

  CREATE TABLE "MEDSADMIN"."DIVE_SITE_OBSERVATION" 
   (	"LATITUDE" NUMBER(12,6), 
	"LONGITUDE" NUMBER(12,6), 
	"LOCATION" "MDSYS"."SDO_GEOMETRY" , 
	"MEDS_JOB_NUMBER" NUMBER(10,0), 
	"MEDS_OBSERVATION_NUMBER" NUMBER(10,0)
   ) ;
--------------------------------------------------------
--  DDL for Table DUC_FORMAT
--------------------------------------------------------

  CREATE TABLE "MEDSADMIN"."DUC_FORMAT" 
   (	"DATA_USE_CODE" VARCHAR2(50), 
	"DESCRIPTION" VARCHAR2(200), 
	"CLASSIFICATION" VARCHAR2(200), 
	"SHORT_DESC" VARCHAR2(20)
   ) ;
--------------------------------------------------------
--  DDL for Table EDDY_DATA
--------------------------------------------------------

  CREATE TABLE "MEDSADMIN"."EDDY_DATA" 
   (	"WATER_TYPES_PRESENT" VARCHAR2(400), 
	"MAJOR_AXIS" VARCHAR2(100), 
	"MINOR_AXIS" VARCHAR2(100), 
	"CORE_TEMPERATURE" VARCHAR2(100), 
	"TEMPERATURE_CORE" VARCHAR2(100), 
	"TEMPERATURE_EDGE" VARCHAR2(100), 
	"SALINITY_CORE" VARCHAR2(100), 
	"SALINITY_EDGE" VARCHAR2(100), 
	"DENSITY_CORE" VARCHAR2(200), 
	"DENSITY_EDGE" VARCHAR2(100), 
	"EDDY_CENTRE_DEPTH" VARCHAR2(100), 
	"AUTHORS" VARCHAR2(200), 
	"TITLE" VARCHAR2(200), 
	"JOURNAL_REPORT" VARCHAR2(300), 
	"VOLUME" VARCHAR2(100), 
	"PAGES" VARCHAR2(100), 
	"PUBLICATION_DATE" VARCHAR2(100), 
	"MAX_U__THETA_" VARCHAR2(100), 
	"PROPAGATION_SPEED" VARCHAR2(100), 
	"DEPTH_OF_P_THERM" VARCHAR2(100), 
	"PROPAGATION_DIRECTION" VARCHAR2(100), 
	"DEPTH_OF_S_THERM" VARCHAR2(100), 
	"COMMENT_H" VARCHAR2(800), 
	"DEPTH_TO_BOTTOM" VARCHAR2(100), 
	"SEA_FLOOR_REGION" VARCHAR2(100), 
	"COMMENT_Z" VARCHAR2(800), 
	"EDDY_ROTATION" VARCHAR2(100), 
	"EDDY_DEPTH_DESCRIPTOR" VARCHAR2(100), 
	"PROPOGATION_CHARACTERISTICS" VARCHAR2(200), 
	"OXYGEN_CORE" VARCHAR2(100), 
	"OXYGEN_EDGE" VARCHAR2(100), 
	"COMMENT_AY" VARCHAR2(800), 
	"MEDS_OBSERVATION_NUMBER" NUMBER(10,0), 
	"MEDS_JOB_NUMBER" NUMBER(10,0), 
	"EDDY_TYPE" VARCHAR2(100), 
	"MAJOR_AXIS_ORIENTATION" VARCHAR2(100), 
	"MAX_U_THETA_CHARACTERISTICS" VARCHAR2(400), 
	"ORIGIN" VARCHAR2(100), 
	"COMMENT_AP" VARCHAR2(800), 
	"DENSITY_DIFF_P_THERM" VARCHAR2(100), 
	"DENSITY_DIFF_S_THERM" VARCHAR2(100)
   ) ;
--------------------------------------------------------
--  DDL for Table EDDY_OBSERVATION
--------------------------------------------------------

  CREATE TABLE "MEDSADMIN"."EDDY_OBSERVATION" 
   (	"LIFETIME" VARCHAR2(100), 
	"DURATION_OF_OBSERVATION" VARCHAR2(100), 
	"COMMENT_S" VARCHAR2(600), 
	"EDDY_NAME" VARCHAR2(100), 
	"LATITUDE" NUMBER(12,6), 
	"LONGITUDE" NUMBER(12,6), 
	"CRUISE_INFORMATION" VARCHAR2(600), 
	"INSTRUMENT" VARCHAR2(600), 
	"OBSERVATION_YEAR" VARCHAR2(20), 
	"OBSERVATION_MONTH" VARCHAR2(40), 
	"MEDS_JOB_NUMBER" NUMBER(10,0), 
	"MEDS_OBSERVATION_NUMBER" NUMBER(10,0), 
	"LOCATION" "MDSYS"."SDO_GEOMETRY" , 
	"DATABASE_REFERENCE" VARCHAR2(20), 
	"DATA_AVAILABLE" VARCHAR2(300)
   ) ;
--------------------------------------------------------
--  DDL for Table EDDY_REPEAT
--------------------------------------------------------

  CREATE TABLE "MEDSADMIN"."EDDY_REPEAT" 
   (	"MEDS_OBSERVATION_NUMBER" NUMBER(10,0), 
	"MEDS_JOB_NUMBER" NUMBER(10,0), 
	"DEPTH" NUMBER(8,0), 
	"DENSITY" NUMBER(12,3), 
	"TEMPERATURE" NUMBER(12,3), 
	"RADIUS" NUMBER(12,3)
   ) ;
--------------------------------------------------------
--  DDL for Table EDDY_SATELLITE_DATA
--------------------------------------------------------

  CREATE TABLE "MEDSADMIN"."EDDY_SATELLITE_DATA" 
   (	"EDDY_NUMBER" VARCHAR2(20), 
	"FIRST_YEAR_OBSERVED" VARCHAR2(20), 
	"SWIRL_VELOCITY" VARCHAR2(20), 
	"MAJOR_AXIS_LENGTH" VARCHAR2(20), 
	"MINOR_AXIS_LENGTH" VARCHAR2(20), 
	"MAJOR_AXIS_ORIENTATION" VARCHAR2(20), 
	"EDDY_SST" VARCHAR2(20), 
	"EDDY_TYPE" VARCHAR2(20), 
	"EDDY_GRADIENT" VARCHAR2(20), 
	"MEDS_JOB_NUMBER" NUMBER(10,0), 
	"MEDS_OBSERVATION_NUMBER" NUMBER(10,0), 
	"EDDY_FRONTAL_DEPTH" VARCHAR2(20), 
	"EDDY_CONFIDENCE" NUMBER(8,0), 
	"EDDY_ID" NUMBER(10,0), 
	"EDDY_BOUNDARY_TYPE" VARCHAR2(20), 
	"CENTRE_LATITUDE" NUMBER(12,6), 
	"CENTRE_LONGITUDE" NUMBER(12,6), 
	"EDDY_FRONT_TYPE" VARCHAR2(20), 
	"PARENT_FEATURE" VARCHAR2(100)
   ) ;
--------------------------------------------------------
--  DDL for Table EDDY_SATELLITE_OBSERVATION
--------------------------------------------------------

  CREATE TABLE "MEDSADMIN"."EDDY_SATELLITE_OBSERVATION" 
   (	"LOCATION" "MDSYS"."SDO_GEOMETRY" , 
	"MEDS_JOB_NUMBER" NUMBER(10,0), 
	"MEDS_OBSERVATION_NUMBER" NUMBER(10,0), 
	"EDDY_OBSERVED_DATE" DATE
   ) ;
--------------------------------------------------------
--  DDL for Table EDDY_SATELLITE_REPEAT
--------------------------------------------------------

  CREATE TABLE "MEDSADMIN"."EDDY_SATELLITE_REPEAT" 
   (	"LATITUDE" NUMBER(12,6), 
	"LONGITUDE" NUMBER(12,6), 
	"VERTEX_ID" NUMBER(20,0), 
	"POINT_ORDER" NUMBER(8,0), 
	"MEDS_JOB_NUMBER" NUMBER(10,0), 
	"MEDS_OBSERVATION_NUMBER" NUMBER(10,0)
   ) ;
--------------------------------------------------------
--  DDL for Table EVENTLOG
--------------------------------------------------------

  CREATE TABLE "MEDSADMIN"."EVENTLOG" 
   (	"ID" NUMBER(10,0), 
	"USERID" VARCHAR2(50), 
	"WORKSTATION" VARCHAR2(50), 
	"EVENTDETAILS" VARCHAR2(4000), 
	"EVENTTYPEID" NUMBER(10,0), 
	"EVENTDATETIME" DATE, 
	"VERBOSEDETAILS" VARCHAR2(4000)
   ) ;
--------------------------------------------------------
--  DDL for Table EVENTTYPE
--------------------------------------------------------

  CREATE TABLE "MEDSADMIN"."EVENTTYPE" 
   (	"ID" NUMBER(10,0), 
	"DESCRIPTION" VARCHAR2(50)
   ) ;
--------------------------------------------------------
--  DDL for Table EXCHANGE_LIST
--------------------------------------------------------

  CREATE TABLE "MEDSADMIN"."EXCHANGE_LIST" 
   (	"MEIC_NUMBER" NUMBER, 
	"DESCRIPTION" VARCHAR2(32), 
	"SUPPLIER" VARCHAR2(64), 
	"CLASSIFICATION" VARCHAR2(50), 
	"SOURCE" VARCHAR2(20), 
	"COMMENTS" VARCHAR2(512), 
	"DATABASED" DATE
   ) ;
--------------------------------------------------------
--  DDL for Table EXTRACTION_OPTION_LOOKUP
--------------------------------------------------------

  CREATE TABLE "MEDSADMIN"."EXTRACTION_OPTION_LOOKUP" 
   (	"DATA_TYPE_INDEX" NUMBER(8,0), 
	"EXTRACTION_TYPE" NUMBER(8,0), 
	"TABLE_NAME" VARCHAR2(50), 
	"FIELD_NAME_ONE" VARCHAR2(50), 
	"FIELD_NAME_TWO" VARCHAR2(50), 
	"CAPTION" VARCHAR2(50), 
	"USER_NAME" VARCHAR2(100)
   ) ;
--------------------------------------------------------
--  DDL for Table EXTRACTION_TYPES_LOOKUP
--------------------------------------------------------

  CREATE TABLE "MEDSADMIN"."EXTRACTION_TYPES_LOOKUP" 
   (	"DESCIPTION" VARCHAR2(50), 
	"EXTRACTION_TYPE" NUMBER(8,0)
   ) ;
--------------------------------------------------------
--  DDL for Table FIELD_LOOKUP
--------------------------------------------------------

  CREATE TABLE "MEDSADMIN"."FIELD_LOOKUP" 
   (	"DATA_TYPE_INDEX" NUMBER(8,0), 
	"FIELD_NAME" VARCHAR2(50), 
	"FIELD_TYPE" VARCHAR2(10) DEFAULT 'S', 
	"FIELD_POSITION" NUMBER(8,0) DEFAULT 0, 
	"FIELD_WIDTH" NUMBER(8,0) DEFAULT 0, 
	"INCLUDE_IN_INPUT" NUMBER(1,0) DEFAULT 0, 
	"INCLUDE_IN_OUTPUT" NUMBER(1,0) DEFAULT 0, 
	"FIELD_SIZE" NUMBER(8,0) DEFAULT 0, 
	"FIELD_PRECISION" NUMBER(8,0) DEFAULT 0, 
	"TABLE_NAME" VARCHAR2(50), 
	"UPDATE_METHOD" NUMBER(8,0) DEFAULT 0, 
	"FILE_EXTENSION" VARCHAR2(20), 
	"REPEAT_ID" NUMBER(8,0) DEFAULT 0, 
	"TABLE_INDEX" VARCHAR2(50), 
	"FIELD_INDEX" VARCHAR2(50), 
	"REPEAT_VALUE" VARCHAR2(50), 
	"REPEAT_OFFSET" NUMBER(8,0) DEFAULT 0, 
	"REPEAT_COUNT" NUMBER(8,0) DEFAULT 0, 
	"FILE_TYPE" NUMBER(1,0) DEFAULT 0, 
	"TABLE_ORDER" NUMBER(2,0) DEFAULT 0, 
	"COLUMN_NAME" VARCHAR2(50), 
	"BIN_SIZE" NUMBER(12,0) DEFAULT NULL, 
	"SORT_ORDER" NUMBER(2,0) DEFAULT 0
   ) ;
--------------------------------------------------------
--  DDL for Table FIELD_LOOKUP_CHECK
--------------------------------------------------------

  CREATE TABLE "MEDSADMIN"."FIELD_LOOKUP_CHECK" 
   (	"TABLE_NAME" VARCHAR2(50), 
	"FIELD_NAME" VARCHAR2(50), 
	"FIELD_EXISTS" NUMBER
   ) ;
--------------------------------------------------------
--  DDL for Table FIELD_LOOKUP_DUP
--------------------------------------------------------

  CREATE TABLE "MEDSADMIN"."FIELD_LOOKUP_DUP" 
   (	"DATA_TYPE_INDEX" NUMBER(8,0), 
	"FIELD_NAME" VARCHAR2(50), 
	"FIELD_TYPE" VARCHAR2(10) DEFAULT 'S', 
	"FIELD_POSITION" NUMBER(8,0) DEFAULT 0, 
	"FIELD_WIDTH" NUMBER(8,0) DEFAULT 0, 
	"INCLUDE_IN_INPUT" NUMBER(1,0) DEFAULT 0, 
	"INCLUDE_IN_OUTPUT" NUMBER(1,0) DEFAULT 0, 
	"FIELD_SIZE" NUMBER(8,0) DEFAULT 0, 
	"FIELD_PRECISION" NUMBER(8,0) DEFAULT 0, 
	"TABLE_NAME" VARCHAR2(50), 
	"UPDATE_METHOD" NUMBER(8,0) DEFAULT 0, 
	"FILE_EXTENSION" VARCHAR2(20), 
	"REPEAT_ID" NUMBER(8,0) DEFAULT 0, 
	"TABLE_INDEX" VARCHAR2(50), 
	"FIELD_INDEX" VARCHAR2(50), 
	"REPEAT_VALUE" VARCHAR2(50), 
	"REPEAT_OFFSET" NUMBER(8,0) DEFAULT 0, 
	"REPEAT_COUNT" NUMBER(8,0) DEFAULT 0, 
	"FILE_TYPE" NUMBER(1,0) DEFAULT 0, 
	"TABLE_ORDER" NUMBER(2,0) DEFAULT 0, 
	"COLUMN_NAME" VARCHAR2(50), 
	"BIN_SIZE" NUMBER(12,0) DEFAULT NULL, 
	"SORT_ORDER" NUMBER(2,0) DEFAULT 0
   ) ;
--------------------------------------------------------
--  DDL for Table FILE_LOOKUP
--------------------------------------------------------

  CREATE TABLE "MEDSADMIN"."FILE_LOOKUP" 
   (	"FILE_NAME" VARCHAR2(32), 
	"PATH_NAME" VARCHAR2(128), 
	"ARGUMENT" VARCHAR2(64), 
	"ARGUMENT2" VARCHAR2(64)
   ) ;
--------------------------------------------------------
--  DDL for Table FILE_PATH_LOOKUP
--------------------------------------------------------

  CREATE TABLE "MEDSADMIN"."FILE_PATH_LOOKUP" 
   (	"FILE_PATH" VARCHAR2(256), 
	"USER_NAME" VARCHAR2(50)
   ) ;
--------------------------------------------------------
--  DDL for Table FLAG_TYPES_LOOKUP
--------------------------------------------------------

  CREATE TABLE "MEDSADMIN"."FLAG_TYPES_LOOKUP" 
   (	"FLAG_TYPE_NAME" VARCHAR2(50)
   ) ;
--------------------------------------------------------
--  DDL for Table FRONT_DEPTH_PLAN_OBSERVATION
--------------------------------------------------------

  CREATE TABLE "MEDSADMIN"."FRONT_DEPTH_PLAN_OBSERVATION" 
   (	"FEATURE" VARCHAR2(50), 
	"GRID_SIZE" VARCHAR2(50), 
	"LOCATION" "MDSYS"."SDO_GEOMETRY" , 
	"SURVEY_DATES" VARCHAR2(100), 
	"DEPTH" VARCHAR2(50), 
	"BREAKPOINTS" VARCHAR2(20), 
	"MEDS_JOB_NUMBER" NUMBER(10,0), 
	"MEDS_OBSERVATION_NUMBER" NUMBER(10,0), 
	"START_DATE" DATE, 
	"FILE_HDR_1" VARCHAR2(100), 
	"FILE_HDR_2" VARCHAR2(100), 
	"FILE_HDR_3" VARCHAR2(100), 
	"FILE_HDR_4" VARCHAR2(100), 
	"FILE_HDR_5" VARCHAR2(100), 
	"FILE_HDR_6" VARCHAR2(100)
   ) ;
--------------------------------------------------------
--  DDL for Table FRONT_DEPTH_PLAN_REPEAT
--------------------------------------------------------

  CREATE TABLE "MEDSADMIN"."FRONT_DEPTH_PLAN_REPEAT" 
   (	"LATITUDE" NUMBER(12,6), 
	"LONGITUDE" NUMBER(12,6), 
	"TEMPERATURE" NUMBER(12,5), 
	"SALINITY" NUMBER(12,5), 
	"SOUND_SPEED" NUMBER(12,5), 
	"DENSITY" NUMBER(12,5), 
	"TSGR" NUMBER(12,5), 
	"SSGR" NUMBER(12,5), 
	"SSSGR" NUMBER(12,5), 
	"DENSGR" NUMBER(12,5), 
	"WIDTH" NUMBER(12,5), 
	"CODE" VARCHAR2(10), 
	"MEDS_OBSERVATION_NUMBER" NUMBER(10,0), 
	"MEDS_JOB_NUMBER" NUMBER(10,0), 
	"POINT_ORDER" NUMBER(8,0)
   ) ;
--------------------------------------------------------
--  DDL for Table FRONT_FWOC_OBSERVATION
--------------------------------------------------------

  CREATE TABLE "MEDSADMIN"."FRONT_FWOC_OBSERVATION" 
   (	"FRONT_NAME" VARCHAR2(100), 
	"DEPTH" VARCHAR2(20), 
	"BREAKPOINTS" VARCHAR2(20), 
	"LOCATION" "MDSYS"."SDO_GEOMETRY" , 
	"MEDS_JOB_NUMBER" NUMBER(10,0), 
	"MEDS_OBSERVATION_NUMBER" NUMBER(10,0), 
	"FRONT_DATE" DATE, 
	"ARCHIVE_DATE" DATE
   ) ;
--------------------------------------------------------
--  DDL for Table FRONT_FWOC_REPEAT
--------------------------------------------------------

  CREATE TABLE "MEDSADMIN"."FRONT_FWOC_REPEAT" 
   (	"MEDS_JOB_NUMBER" NUMBER(10,0), 
	"MEDS_OBSERVATION_NUMBER" NUMBER(10,0), 
	"LATITUDE" NUMBER(12,6), 
	"LONGITUDE" NUMBER(12,6), 
	"POINT_ORDER" NUMBER(8,0)
   ) ;
--------------------------------------------------------
--  DDL for Table FRONT_GRID_DATA
--------------------------------------------------------

  CREATE TABLE "MEDSADMIN"."FRONT_GRID_DATA" 
   (	"MEDS_JOB_NUMBER" NUMBER(10,0), 
	"FRONT_NAME" VARCHAR2(50), 
	"REGION" VARCHAR2(50), 
	"DURATION" VARCHAR2(50), 
	"DEPTH" VARCHAR2(20), 
	"SOURCE" VARCHAR2(50), 
	"DATES" VARCHAR2(50), 
	"RESOLUTION" VARCHAR2(20), 
	"ANALYSED" VARCHAR2(20), 
	"CREATOR" VARCHAR2(50), 
	"MEDS_OBSERVATION_NUMBER" NUMBER(10,0)
   ) ;
--------------------------------------------------------
--  DDL for Table FRONT_GRID_OBSERVATION
--------------------------------------------------------

  CREATE TABLE "MEDSADMIN"."FRONT_GRID_OBSERVATION" 
   (	"MEDS_JOB_NUMBER" NUMBER(10,0), 
	"MEDS_OBSERVATION_NUMBER" NUMBER(10,0), 
	"LOCATION" "MDSYS"."SDO_GEOMETRY" , 
	"IDENTIFIER" VARCHAR2(20), 
	"TRH_LATITUDE" NUMBER(12,8), 
	"TRH_LONGITUDE" NUMBER(12,8), 
	"BLH_LONGITUDE" NUMBER(12,8), 
	"BLH_LATITUDE" NUMBER(12,8)
   ) ;
--------------------------------------------------------
--  DDL for Table FRONT_GRID_REPEAT
--------------------------------------------------------

  CREATE TABLE "MEDSADMIN"."FRONT_GRID_REPEAT" 
   (	"LATITUDE" NUMBER(12,6), 
	"LONGITUDE" NUMBER(12,6), 
	"COUNT" VARCHAR2(20), 
	"OCCUPANCY" VARCHAR2(20), 
	"MEDS_JOB_NUMBER" NUMBER(10,0), 
	"MEDS_OBSERVATION_NUMBER" NUMBER(10,0), 
	"TOTAL" VARCHAR2(20), 
	"POINT_ORDER" NUMBER(8,0)
   ) ;
--------------------------------------------------------
--  DDL for Table FRONT_IMAGE_DATA
--------------------------------------------------------

  CREATE TABLE "MEDSADMIN"."FRONT_IMAGE_DATA" 
   (	"MEDS_OBSERVATION_NUMBER" NUMBER(10,0), 
	"MEDS_JOB_NUMBER" NUMBER(10,0), 
	"POINT_TYPE" VARCHAR2(20), 
	"FRONTAL_GRADIENT" VARCHAR2(20), 
	"FRONTAL_DEPTH" VARCHAR2(20), 
	"FRONTAL_NAME" VARCHAR2(100), 
	"FRONTAL_TYPE" VARCHAR2(20), 
	"FRONTAL_CONFIDENCE" VARCHAR2(20), 
	"FRONTAL_LINE_ID" NUMBER(10,0), 
	"BOUNDARY_TYPE" VARCHAR2(20)
   ) ;
--------------------------------------------------------
--  DDL for Table FRONT_IMAGE_REPEAT
--------------------------------------------------------

  CREATE TABLE "MEDSADMIN"."FRONT_IMAGE_REPEAT" 
   (	"LATITUDE" NUMBER, 
	"LONGITUDE" NUMBER, 
	"CONFIDENCE" VARCHAR2(20), 
	"STRENGTH" VARCHAR2(20), 
	"MEDS_JOB_NUMBER" NUMBER(10,0), 
	"MEDS_OBSERVATION_NUMBER" NUMBER(10,0), 
	"POINT_ORDER" NUMBER(8,0), 
	"VERTEX_ID" NUMBER(10,0)
   ) ;
--------------------------------------------------------
--  DDL for Table FRONT_MEAN_DATA
--------------------------------------------------------

  CREATE TABLE "MEDSADMIN"."FRONT_MEAN_DATA" 
   (	"MEDS_JOB_NUMBER" NUMBER(10,0), 
	"FRONT_NAME" VARCHAR2(100), 
	"REGION" VARCHAR2(100), 
	"BOUNDARY" VARCHAR2(100), 
	"PROPERTY" VARCHAR2(100), 
	"FRONT_TYPE" VARCHAR2(100), 
	"DURATION" VARCHAR2(100), 
	"STRENGTH" VARCHAR2(100), 
	"DEPTH" VARCHAR2(100), 
	"LEVEL_1" VARCHAR2(100), 
	"SOURCE" VARCHAR2(100), 
	"DATES" VARCHAR2(100), 
	"OBSERVATIONS" VARCHAR2(100), 
	"RESOLUTION" VARCHAR2(100), 
	"ANALYSED" VARCHAR2(100), 
	"CREATOR" VARCHAR2(100), 
	"MEDS_OBSERVATION_NUMBER" NUMBER(10,0)
   ) ;
--------------------------------------------------------
--  DDL for Table FRONT_MEAN_OBSERVATION
--------------------------------------------------------

  CREATE TABLE "MEDSADMIN"."FRONT_MEAN_OBSERVATION" 
   (	"MEDS_JOB_NUMBER" NUMBER(10,0), 
	"LOCATION" "MDSYS"."SDO_GEOMETRY" , 
	"LINE_TYPE" VARCHAR2(20), 
	"IDENTIFIER" VARCHAR2(20), 
	"MEDS_OBSERVATION_NUMBER" NUMBER(10,0)
   ) ;
--------------------------------------------------------
--  DDL for Table FRONT_MEAN_REPEAT
--------------------------------------------------------

  CREATE TABLE "MEDSADMIN"."FRONT_MEAN_REPEAT" 
   (	"MEDS_JOB_NUMBER" NUMBER(10,0), 
	"LATITUDE" NUMBER(12,6), 
	"LONGITUDE" NUMBER(12,6), 
	"VERTEX_ID" VARCHAR2(20), 
	"MEDS_OBSERVATION_NUMBER" NUMBER(10,0), 
	"POINT_ORDER" NUMBER(8,0)
   ) ;
--------------------------------------------------------
--  DDL for Table FRONT_SATELLITE_OBSERVATION
--------------------------------------------------------

  CREATE TABLE "MEDSADMIN"."FRONT_SATELLITE_OBSERVATION" 
   (	"MEDS_OBSERVATION_NUMBER" NUMBER(10,0), 
	"MEDS_JOB_NUMBER" NUMBER(10,0), 
	"LOCATION" "MDSYS"."SDO_GEOMETRY" , 
	"OBSERVERED_DATE" DATE
   ) ;
--------------------------------------------------------
--  DDL for Table FRONT_SURVEY_DATA
--------------------------------------------------------

  CREATE TABLE "MEDSADMIN"."FRONT_SURVEY_DATA" 
   (	"INSTRUMENTS" VARCHAR2(100), 
	"MAXIMUM_DEPTH" VARCHAR2(20), 
	"NUMBER_OF_OBSERVATIONS" NUMBER(5,0), 
	"QUALITY" VARCHAR2(30), 
	"FILENAME" VARCHAR2(500), 
	"MEDS_JOB_NUMBER" NUMBER(10,0), 
	"MEDS_OBSERVATION_NUMBER" NUMBER(10,0), 
	"BLH_LATITUDE" NUMBER(12,6), 
	"TRH_LATITUDE" NUMBER(12,6), 
	"TRH_LONGITUDE" NUMBER(12,6), 
	"BLH_LONGITUDE" NUMBER(12,6)
   ) ;
--------------------------------------------------------
--  DDL for Table FRONT_SURVEY_OBSERVATION
--------------------------------------------------------

  CREATE TABLE "MEDSADMIN"."FRONT_SURVEY_OBSERVATION" 
   (	"LOCATION" "MDSYS"."SDO_GEOMETRY" , 
	"MEDS_JOB_NUMBER" NUMBER(10,0), 
	"MEDS_OBSERVATION_NUMBER" NUMBER(10,0), 
	"START_DATE" DATE, 
	"END_DATE" DATE
   ) ;
--------------------------------------------------------
--  DDL for Table FRONT_TRANSECT_OBSERVATION
--------------------------------------------------------

  CREATE TABLE "MEDSADMIN"."FRONT_TRANSECT_OBSERVATION" 
   (	"TRANSECT_FEATURE" VARCHAR2(50), 
	"LOCATION" "MDSYS"."SDO_GEOMETRY" , 
	"BEARING" VARCHAR2(20), 
	"TRANSECT_DATES" VARCHAR2(300), 
	"BREAKPOINTS" VARCHAR2(20), 
	"MEDS_JOB_NUMBER" NUMBER(10,0), 
	"MEDS_OBSERVATION_NUMBER" NUMBER(10,0), 
	"START_DATE" DATE, 
	"MEAN_VALUES" VARCHAR2(100), 
	"FILE_HDR_1" VARCHAR2(100), 
	"FILE_HDR_2" VARCHAR2(100), 
	"FILE_HDR_3" VARCHAR2(100), 
	"FILE_HDR_4" VARCHAR2(100), 
	"FILE_HDR_5" VARCHAR2(100), 
	"FILE_HDR_6" VARCHAR2(100), 
	"FILE_HDR_7" VARCHAR2(100)
   ) ;
--------------------------------------------------------
--  DDL for Table FRONT_TRANSECT_REPEAT
--------------------------------------------------------

  CREATE TABLE "MEDSADMIN"."FRONT_TRANSECT_REPEAT" 
   (	"LATITUDE" NUMBER(12,6), 
	"LONGITUDE" NUMBER(12,6), 
	"DEPTH" NUMBER(12,5), 
	"TEMPERATURE" NUMBER(12,5), 
	"SALINITY" NUMBER(12,5), 
	"SOUND_SPEED" NUMBER(12,5), 
	"DENSITY" NUMBER(12,5), 
	"TSGR" NUMBER(12,5), 
	"SSGR" NUMBER(12,5), 
	"SSSGR" NUMBER(12,5), 
	"DENSGR" NUMBER(12,5), 
	"CODE" VARCHAR2(10), 
	"MEDS_OBSERVATION_NUMBER" NUMBER(10,0), 
	"MEDS_JOB_NUMBER" NUMBER(10,0), 
	"SLOPE" VARCHAR2(50), 
	"WIDTH" VARCHAR2(20), 
	"POINT_ORDER" NUMBER(8,0)
   ) ;
--------------------------------------------------------
--  DDL for Table GENERIC_EDIT
--------------------------------------------------------

  CREATE TABLE "MEDSADMIN"."GENERIC_EDIT" 
   (	"ROW_TYPE" VARCHAR2(20), 
	"TABLE_NAME" VARCHAR2(30), 
	"COLUMN_NAME" VARCHAR2(30), 
	"COLUMN_HEADER" VARCHAR2(30), 
	"COLUMN_ORDER" NUMBER, 
	"TEXT_SEARCH" NUMBER, 
	"COMBO_SEARCH" NUMBER, 
	"DATA_TYPE_ALIAS" VARCHAR2(30), 
	"DATE_RECORD_NAME" VARCHAR2(30), 
	"ALLOW_TIME" NUMBER(1,0), 
	"DATE_REQUIRED" NUMBER(1,0), 
	"CRUISE_REQUIRED" NUMBER(1,0), 
	"DATE_SEARCH_REQUIRED" NUMBER(1,0), 
	"HAS_TRACK" NUMBER(1,0) DEFAULT 0, 
	"JOB_TYPE_DESCRIPTION" VARCHAR2(30) DEFAULT NULL
   ) ;
--------------------------------------------------------
--  DDL for Table GEOLOGY_PARAM_DATA
--------------------------------------------------------

  CREATE TABLE "MEDSADMIN"."GEOLOGY_PARAM_DATA" 
   (	"SEA_BED_TYPE__OLD_" NUMBER(2,0), 
	"SEA_BED_TYPE__NEW_" NUMBER(2,0), 
	"SEA_BED_SEDIMENT_THICKNESS" NUMBER(3,0), 
	"BEDROCK_TYPE" NUMBER(2,0), 
	"BEDROCK_LITHOLOGY" NUMBER(3,0), 
	"BEDROCK_ID" VARCHAR2(10), 
	"BATHMETRY" NUMBER(5,0), 
	"GAS_TYPE" NUMBER(2,0), 
	"GAS_DEPTH" NUMBER(4,0), 
	"NUMBER_BULK_SEDIMENT_LAYERS" NUMBER(3,0), 
	"MASTER_BULK_SEDIMENT_LAYER" NUMBER(3,0), 
	"MASTER_BULK_SEDIMENT_THICKNESS" NUMBER(4,0), 
	"MEDS_JOB_NUMBER" NUMBER(10,0), 
	"MEDS_OBSERVATION_NUMBER" NUMBER(10,0)
   ) ;
--------------------------------------------------------
--  DDL for Table GEOLOGY_PARAM_OBSERVATION
--------------------------------------------------------

  CREATE TABLE "MEDSADMIN"."GEOLOGY_PARAM_OBSERVATION" 
   (	"LONGITUDE" NUMBER(12,6), 
	"LATITUDE" NUMBER(12,6), 
	"LOCATION" "MDSYS"."SDO_GEOMETRY" , 
	"MEDS_JOB_NUMBER" NUMBER(10,0), 
	"MEDS_OBSERVATION_NUMBER" NUMBER(10,0)
   ) ;
--------------------------------------------------------
--  DDL for Table GEOLOGY_QUALITY_DATA
--------------------------------------------------------

  CREATE TABLE "MEDSADMIN"."GEOLOGY_QUALITY_DATA" 
   (	"SEDIMENT_TYPE_CONFIDENCE" NUMBER(3,0), 
	"SEDIMENT_TYPE_DATA" NUMBER(3,0), 
	"SEDIMENT_THICKNESS_CONFIDENCE" NUMBER(3,0), 
	"SEDIMENT_THICKNESS_DATA" NUMBER(3,0), 
	"BEDROCK_TYPE_CONFIDENCE" NUMBER(3,0), 
	"BEDROCK_TYPE_DATA" NUMBER(3,0), 
	"BEDROCK_LITH_ID_CONFIDENCE" NUMBER(3,0), 
	"BEDROCK_LITH_ID_DATA" NUMBER(3,0), 
	"BATHYMETRY_CONFIDENCE" NUMBER(3,0), 
	"BATHYMETRY_DATA" NUMBER(3,0), 
	"GAS_TYPE_DATA" NUMBER(3,0), 
	"GAS_TYPE_CONFIDENCE" NUMBER(3,0), 
	"GAS_DEPTH_DATA" NUMBER(3,0), 
	"GAS_DEPTH_CONFIDENCE" NUMBER(3,0), 
	"BULK_LAYERS_CONFIDENCE" NUMBER(3,0), 
	"BULK_LAYERS_DATA_CODE" NUMBER(3,0), 
	"MASTER_BSL" NUMBER(3,0), 
	"MASTER_BSL_DATA" NUMBER(3,0), 
	"MASTER_BSL_THICKNESS_CONFIDENC" NUMBER(3,0), 
	"MASTER_BSL_THICKNESS_CODE" NUMBER(3,0), 
	"MEDS_OBSERVATION_NUMBER" NUMBER(10,0), 
	"MEDS_JOB_NUMBER" NUMBER(10,0)
   ) ;
--------------------------------------------------------
--  DDL for Table GEOLOGY_QUALITY_OBSERVATION
--------------------------------------------------------

  CREATE TABLE "MEDSADMIN"."GEOLOGY_QUALITY_OBSERVATION" 
   (	"LONGITUDE" NUMBER(12,6), 
	"LATITUDE" NUMBER(12,6), 
	"LOCATION" "MDSYS"."SDO_GEOMETRY" , 
	"MEDS_JOB_NUMBER" NUMBER(10,0), 
	"MEDS_OBSERVATION_NUMBER" NUMBER(10,0)
   ) ;
--------------------------------------------------------
--  DDL for Table GLIDER_THREADED_DATA
--------------------------------------------------------

  CREATE TABLE "MEDSADMIN"."GLIDER_THREADED_DATA" 
   (	"MEDS_JOB_NUMBER" NUMBER(10,0), 
	"MEDS_OBSERVATION_NUMBER" NUMBER(10,0), 
	"DEPTH" NUMBER(12,4), 
	"PRESSURE" NUMBER(12,4), 
	"TEMPERATURE" NUMBER(12,4), 
	"TEMP_FLAG" NUMBER(3,0), 
	"SALINITY" NUMBER(12,4), 
	"SAL_FLAG" NUMBER(3,0), 
	"SOUND_SPEED" NUMBER(12,4), 
	"DENSITY" NUMBER(12,4), 
	"CHLOROPHYLL" NUMBER(20,10), 
	"HYDROCARBONS" NUMBER(20,10), 
	"GELBSTOFFE" NUMBER(20,10), 
	"BIOLUMINESCENCE" NUMBER(20,10), 
	"LONGITUDE" NUMBER(12,6), 
	"LATITUDE" NUMBER(12,6), 
	"DATE_RECORDED" DATE, 
	"POINT_ORDER" NUMBER(8,0), 
	"BBP700" NUMBER(20,10)
   ) ;
--------------------------------------------------------
--  DDL for Table GLIDER_THREADED_OBSERVATION
--------------------------------------------------------

  CREATE TABLE "MEDSADMIN"."GLIDER_THREADED_OBSERVATION" 
   (	"LOCATION" "MDSYS"."SDO_GEOMETRY" , 
	"MEDS_JOB_NUMBER" NUMBER(10,0), 
	"MEDS_OBSERVATION_NUMBER" NUMBER(10,0), 
	"MID_LATITUDE" NUMBER(12,6), 
	"MID_LONGITUDE" NUMBER(12,6), 
	"PROFILE_ID" NUMBER(10,0), 
	"PROFILE_DIR" NUMBER(2,0), 
	"DISTANCE_TRAVELED" NUMBER(12,4), 
	"MID_TIME" NUMBER(10,0)
   ) ;
--------------------------------------------------------
--  DDL for Table HELP
--------------------------------------------------------

  CREATE TABLE "MEDSADMIN"."HELP" 
   (	"TOPIC" VARCHAR2(50), 
	"SEQ" NUMBER, 
	"INFO" VARCHAR2(80)
   ) ;
--------------------------------------------------------
--  DDL for Table ICES_SHIP_DESC
--------------------------------------------------------

  CREATE TABLE "MEDSADMIN"."ICES_SHIP_DESC" 
   (	"ICES_SHIP_CODE" VARCHAR2(4), 
	"CODE_DESCRIPTION" VARCHAR2(200)
   ) ;
--------------------------------------------------------
--  DDL for Table INGRES_ROWS_PER_JOB
--------------------------------------------------------

  CREATE TABLE "MEDSADMIN"."INGRES_ROWS_PER_JOB" 
   (	"BOOKING_JOB" NUMBER(*,0), 
	"N_ROWS" NUMBER(*,0)
   ) ;
--------------------------------------------------------
--  DDL for Table INSTRUMENT
--------------------------------------------------------

  CREATE TABLE "MEDSADMIN"."INSTRUMENT" 
   (	"OCEAN" NUMBER(4,0), 
	"DESCRIPTION" VARCHAR2(200), 
	"NEBT" VARCHAR2(2), 
	"SERD" VARCHAR2(4), 
	"NODEF_" VARCHAR2(4), 
	"DATA_TYPE" NUMBER(1,0)
   ) ;
--------------------------------------------------------
--  DDL for Table INTERNAL_WAVE_DATA
--------------------------------------------------------

  CREATE TABLE "MEDSADMIN"."INTERNAL_WAVE_DATA" 
   (	"GENERAL_DESCRIPTION" VARCHAR2(150), 
	"LONG_LENGTH_SCALE" NUMBER(12,3), 
	"SHORT_LENGTH_SCALE" NUMBER(12,3), 
	"MEASUREMENT" VARCHAR2(200), 
	"SPEED" VARCHAR2(30), 
	"PROPAGATION_DISTANCE" VARCHAR2(100), 
	"FIGURES" VARCHAR2(100), 
	"AUTHORS" VARCHAR2(200), 
	"TITLE" VARCHAR2(200), 
	"JOURNAL_REPORT" VARCHAR2(300), 
	"VOLUME" VARCHAR2(100), 
	"PAGES" VARCHAR2(30), 
	"PUBLICATION_DATE" VARCHAR2(50), 
	"CROSS_REFERENCE" VARCHAR2(50), 
	"WAVE_PERIOD" NUMBER(12,3), 
	"MARSDEN_SQUARE" VARCHAR2(20), 
	"MEDS_OBSERVATION_NUMBER" NUMBER(10,0), 
	"MEDS_JOB_NUMBER" NUMBER(10,0), 
	"OTHER" VARCHAR2(200), 
	"BATHYMETRY" VARCHAR2(300), 
	"SURVEY_IDENTIFIER" VARCHAR2(150), 
	"LOCATION_LONGITUDE" VARCHAR2(40), 
	"LOCATION_LATITUDE" VARCHAR2(50), 
	"OCEANOGRAPHY" VARCHAR2(500), 
	"METEOROLOGY" VARCHAR2(300), 
	"WAVELENGTH" VARCHAR2(100), 
	"INSTRUMENT" VARCHAR2(150), 
	"IW_PERIOD" VARCHAR2(200), 
	"REPEAT_PERIOD" VARCHAR2(150), 
	"PROPOSED_GENERATING_MECH" VARCHAR2(500), 
	"DESCRIPTION" VARCHAR2(500), 
	"IW_DIRECTION_OF_PROPAGATION" VARCHAR2(150), 
	"OBSERVED_DATE" VARCHAR2(75), 
	"BOOK_EDITOR_PUBLISHER" VARCHAR2(150), 
	"THERMOCLINE_PYCNOCLINE" VARCHAR2(100), 
	"IW_SPEED_OBS" VARCHAR2(100), 
	"IW_AMPLITUDE_OBS" VARCHAR2(150), 
	"DEPTH" VARCHAR2(50), 
	"AMPLITUDE" VARCHAR2(100)
   ) ;
--------------------------------------------------------
--  DDL for Table INTERNAL_WAVE_OBSERVATION
--------------------------------------------------------

  CREATE TABLE "MEDSADMIN"."INTERNAL_WAVE_OBSERVATION" 
   (	"LATITUDE" NUMBER(12,6), 
	"LONGITUDE" NUMBER(12,6), 
	"LOCATION" "MDSYS"."SDO_GEOMETRY" , 
	"DATABASE_IDENTIFICATION_NUMBER" VARCHAR2(20), 
	"MONTH" VARCHAR2(20), 
	"MEDS_JOB_NUMBER" NUMBER(10,0), 
	"MEDS_OBSERVATION_NUMBER" NUMBER(10,0)
   ) ;
--------------------------------------------------------
--  DDL for Table INTERNAL_WAVE_REPEAT
--------------------------------------------------------

  CREATE TABLE "MEDSADMIN"."INTERNAL_WAVE_REPEAT" 
   (	"DIRECTION" VARCHAR2(20), 
	"MEDS_JOB_NUMBER" NUMBER(10,0), 
	"MEDS_OBSERVATION_NUMBER" NUMBER(10,0)
   ) ;
--------------------------------------------------------
--  DDL for Table IPR_LOOKUP
--------------------------------------------------------

  CREATE TABLE "MEDSADMIN"."IPR_LOOKUP" 
   (	"IPR_OWNER_TYPE" VARCHAR2(100), 
	"IPR_PERIOD" DATE, 
	"IPR_OWNER" VARCHAR2(50), 
	"RIGHTS" NUMBER(1,0), 
	"FOI_EXEMPT" NUMBER(1,0), 
	"FOI_DECLARE" NUMBER(1,0), 
	"CONFIDENTIALITY" NUMBER(1,0)
   ) ;
--------------------------------------------------------
--  DDL for Table IPR_OWNER_CONTACT
--------------------------------------------------------

  CREATE TABLE "MEDSADMIN"."IPR_OWNER_CONTACT" 
   (	"OWNER" VARCHAR2(50), 
	"OWNER_CODE" NUMBER(10,0)
   ) ;
--------------------------------------------------------
--  DDL for Table IPR_OWNER_TYPE
--------------------------------------------------------

  CREATE TABLE "MEDSADMIN"."IPR_OWNER_TYPE" 
   (	"DESRIPTION" VARCHAR2(50), 
	"TYPE" NUMBER(3,0)
   ) ;
--------------------------------------------------------
--  DDL for Table IW_IMAGE_DATA
--------------------------------------------------------

  CREATE TABLE "MEDSADMIN"."IW_IMAGE_DATA" 
   (	"FEATURE_TYPE" VARCHAR2(100), 
	"FEATURE_NAME" VARCHAR2(100), 
	"OCEAN_AREA" VARCHAR2(100), 
	"LATITUDE_NW" VARCHAR2(50), 
	"LATITUDE_NE" VARCHAR2(50), 
	"LATITUDE_SE" VARCHAR2(50), 
	"LATITUDE_SW" VARCHAR2(50), 
	"ORBIT_NUMBER" VARCHAR2(50), 
	"IMAGE_SOURCE" VARCHAR2(100), 
	"STORAGE_FORMAT" VARCHAR2(50), 
	"ARCHIVE_DATE" DATE, 
	"ADDITIONAL_INFORMATION" VARCHAR2(500), 
	"FILE_REFERENCE" VARCHAR2(100), 
	"LONGITUDE_NE" VARCHAR2(50), 
	"LONGITUDE_SW" VARCHAR2(50), 
	"LONGITUDE_NW" VARCHAR2(50), 
	"LONGITUDE_SE" VARCHAR2(50), 
	"PROJECT_OF_IMAGE" VARCHAR2(50), 
	"IMAGE_FILE_REFERENCE" VARCHAR2(500), 
	"NUMBER_OF_WAVE_PACKETS" VARCHAR2(20), 
	"INSTRUMENT" VARCHAR2(50), 
	"MEASUREMENT" VARCHAR2(50), 
	"SATELLITE" VARCHAR2(50), 
	"TIME_OF_TIDE" VARCHAR2(20), 
	"SOURCE_OF_TIDE_INFO" VARCHAR2(100), 
	"FRAME_NUMBER" VARCHAR2(20), 
	"ORIGINAL_IMAGE_PRODUCT" VARCHAR2(100), 
	"ORIGINAL_X_RESOLUTION" VARCHAR2(20), 
	"ORIGINAL_Y_RESOLUTION" VARCHAR2(20), 
	"STORED_X_RESOLUTION" VARCHAR2(20), 
	"STORED_Y_RESOLUTION" VARCHAR2(20), 
	"X_SIZE" VARCHAR2(20), 
	"Y_SIZE" VARCHAR2(20), 
	"MEDS_JOB_NUMBER" NUMBER(10,0), 
	"MEDS_OBSERVATION_NUMBER" VARCHAR2(10)
   ) ;
--------------------------------------------------------
--  DDL for Table IW_IMAGE_OBSERVATION
--------------------------------------------------------

  CREATE TABLE "MEDSADMIN"."IW_IMAGE_OBSERVATION" 
   (	"LATITUDE" NUMBER(12,6), 
	"LONGITUDE" NUMBER(12,6), 
	"LOCATION" "MDSYS"."SDO_GEOMETRY" , 
	"RECORDED_DATE" DATE, 
	"IMAGE_ID" VARCHAR2(20), 
	"MEDS_JOB_NUMBER" NUMBER(10,0), 
	"MEDS_OBSERVATION_NUMBER" NUMBER(10,0)
   ) ;
--------------------------------------------------------
--  DDL for Table IW_PACKETS
--------------------------------------------------------

  CREATE TABLE "MEDSADMIN"."IW_PACKETS" 
   (	"MEDS_JOB_NUMBER" NUMBER(10,0), 
	"MEDS_OBSERVATION_NUMBER" NUMBER(10,0), 
	"WAVE_PACKET_ID" VARCHAR2(20), 
	"NUMBER_OF_WAVES" VARCHAR2(20), 
	"LATITUDE_FOCAL_PT" VARCHAR2(20), 
	"LONGITUDE_FOCAL_PT" VARCHAR2(20), 
	"FOCAL_RADIUS" VARCHAR2(20), 
	"ESTIMATED_FOCAL_ERROR" VARCHAR2(20), 
	"FOCAL_POINT_FLAG" VARCHAR2(20), 
	"LEAD_WAVE_LENGTH" VARCHAR2(20), 
	"LEAD_WAVE_CURVATURE" VARCHAR2(20), 
	"LEAD_WAVE_PROP_DIR" VARCHAR2(20), 
	"PROP_DIRECTION_COMMENT" VARCHAR2(500), 
	"DISTANCE_BETWEEN_PACKETS" VARCHAR2(20), 
	"DISTANCE_CALC_PACKETS" VARCHAR2(20), 
	"COMMENTS" VARCHAR2(500), 
	"MIN_LAT_BREAKPOINTS" VARCHAR2(20), 
	"MIN_LON_BREAKPOINTS" VARCHAR2(20), 
	"MAX_LAT_BREAKPOINTS" VARCHAR2(20), 
	"MAX_LON_BREAKPOINTS" VARCHAR2(20)
   ) ;
--------------------------------------------------------
--  DDL for Table IW_POINTS
--------------------------------------------------------

  CREATE TABLE "MEDSADMIN"."IW_POINTS" 
   (	"MEDS_JOB_NUMBER" NUMBER(10,0), 
	"MEDS_OBSERVATION_NUMBER" NUMBER(10,0), 
	"BREAK_POINT_ID" VARCHAR2(20), 
	"POINT_LATITUDE" VARCHAR2(20), 
	"POINT_LONGITUDE" VARCHAR2(20), 
	"PROPOGATION_DIRECTION" VARCHAR2(20)
   ) ;
--------------------------------------------------------
--  DDL for Table IW_WAVES
--------------------------------------------------------

  CREATE TABLE "MEDSADMIN"."IW_WAVES" 
   (	"MEDS_JOB_NUMBER" NUMBER(10,0), 
	"MEDS_OBSERVATION_NUMBER" NUMBER(10,0), 
	"INDIVIDUAL_WAVE_ID" VARCHAR2(20), 
	"CREST_LENGTH" VARCHAR2(20), 
	"MEAN_WAVE_SEPERATION" VARCHAR2(20), 
	"MIN_WAVE_SEPERATION" VARCHAR2(20), 
	"MAX_WAVE_SEPERATION" VARCHAR2(20), 
	"NUMBER_OF_BREAKPOINTS" VARCHAR2(20), 
	"BRIGHTNESS" VARCHAR2(50)
   ) ;
--------------------------------------------------------
--  DDL for Table JOB
--------------------------------------------------------

  CREATE TABLE "MEDSADMIN"."JOB" 
   (	"JOB_NUMBER" NUMBER(6,0), 
	"DATE_OF_RECEIPT" DATE, 
	"ORIG_INITIALS" VARCHAR2(6), 
	"PERIOD_START" DATE, 
	"PERIOD_END" DATE, 
	"PRIORITY_AREA" NUMBER(2,0), 
	"SOURCE" VARCHAR2(20), 
	"VESSEL_NAME" VARCHAR2(20), 
	"MEDIA_TYPE" VARCHAR2(20), 
	"RAW_PREPROCESS" VARCHAR2(10), 
	"CLASSIFICATION" VARCHAR2(20), 
	"REMARKS" VARCHAR2(70), 
	"NUM_OBS" NUMBER(6,0), 
	"TARGET_DATE" DATE
   ) ;
--------------------------------------------------------
--  DDL for Table JOB_ERROR
--------------------------------------------------------

  CREATE TABLE "MEDSADMIN"."JOB_ERROR" 
   (	"JOB_NUMBER" NUMBER, 
	"JOB_TYPE" NUMBER, 
	"ERROR_NUMBER" NUMBER, 
	"NUM_ERROR" NUMBER
   ) ;
--------------------------------------------------------
--  DDL for Table JOB_LOOKUPS
--------------------------------------------------------

  CREATE TABLE "MEDSADMIN"."JOB_LOOKUPS" 
   (	"TYPE" VARCHAR2(20), 
	"DESCRIPTION" VARCHAR2(50), 
	"USAGE" VARCHAR2(50), 
	"INDEX_FIELD" NUMBER(8,0), 
	"GENERIC" NUMBER(1,0), 
	"TEXT_FILE_TYPE" NUMBER(1,0), 
	"FILE_EXTENSION" VARCHAR2(50), 
	"DUC_POSITION" NUMBER(4,0)
   ) ;
--------------------------------------------------------
--  DDL for Table JOB_STATS
--------------------------------------------------------

  CREATE TABLE "MEDSADMIN"."JOB_STATS" 
   (	"DESCRIPTION" VARCHAR2(30), 
	"PERIOD_START" DATE, 
	"PERIOD_END" DATE, 
	"RECEIVED" NUMBER, 
	"ACCEPTED" NUMBER, 
	"REJECTED" NUMBER, 
	"ON_TIME" NUMBER, 
	"LATE" NUMBER, 
	"MONTH_LATE_1" NUMBER, 
	"MONTH_LATE_1_3" NUMBER, 
	"MONTH_LATE_3_6" NUMBER, 
	"LATE_GREATER_6" NUMBER, 
	"WAITING" NUMBER, 
	"WAITING_ON_TIME" NUMBER, 
	"WAITING_LATE" NUMBER, 
	"WAITING_LATE_1" NUMBER, 
	"WAITING_LATE_1_3" NUMBER, 
	"WAITING_LATE_3_6" NUMBER, 
	"WAITING_GREATER_6" NUMBER
   ) ;
--------------------------------------------------------
--  DDL for Table JOB_STATS_SETTINGS
--------------------------------------------------------

  CREATE TABLE "MEDSADMIN"."JOB_STATS_SETTINGS" 
   (	"VALUE" NUMBER, 
	"DAY" NUMBER, 
	"MONTH" VARCHAR2(16), 
	"YEAR" NUMBER, 
	"INTERVAL" VARCHAR2(16), 
	"NUM_INTERVALS" NUMBER, 
	"TOTAL" NUMBER
   ) ;
--------------------------------------------------------
--  DDL for Table JOB_TRACKING
--------------------------------------------------------

  CREATE TABLE "MEDSADMIN"."JOB_TRACKING" 
   (	"MEIC_NUMBER" NUMBER, 
	"PO_NUMBER" NUMBER, 
	"PO_TASK" CHAR(1), 
	"STATUS" CHAR(16), 
	"JOB_TYPE_NUMBER" NUMBER, 
	"INGRES_NO" NUMBER, 
	"DATE_RECEIVED" DATE, 
	"SUPPLIER" VARCHAR2(64), 
	"SOURCE" VARCHAR2(30), 
	"CLASSIFICATION" VARCHAR2(50), 
	"ASSIGNED" VARCHAR2(16), 
	"ACKNOWLEDGE" CHAR(1), 
	"PRIORITY_REGION1" CHAR(1), 
	"PRIORITY_REGION2" CHAR(1), 
	"PRIORITY_REGION3" CHAR(1), 
	"PRIORITY_REGION4" CHAR(1), 
	"PRIORITY_REGION5" CHAR(1), 
	"PRIORITY_REGION6" CHAR(1), 
	"NO_OBS" NUMBER, 
	"OBS_ACCEPTED" NUMBER, 
	"OBS_REJECTED" NUMBER, 
	"COLLECTION_START" DATE, 
	"COLLECTION_END" DATE, 
	"QA_PROCESSING_TARGET" DATE, 
	"QA_PROCESSING_COMPLETED" DATE, 
	"QA_PROCESSING_NAME" VARCHAR2(16), 
	"DATABASED" DATE, 
	"DATABASED_NAME" VARCHAR2(16), 
	"VALIDATED" DATE, 
	"VALIDATED_NAME" VARCHAR2(16), 
	"COMMENTS" VARCHAR2(512)
   ) ;
--------------------------------------------------------
--  DDL for Table JOB_TYPE
--------------------------------------------------------

  CREATE TABLE "MEDSADMIN"."JOB_TYPE" 
   (	"JOB_NUMBER" NUMBER(6,0), 
	"JOB_TYPE" NUMBER(4,0), 
	"PROCESSING_COMPLETE" DATE, 
	"PROCESSOR_INITIALS" VARCHAR2(8), 
	"VALIDATION_COMPLETE" DATE, 
	"VALIDATOR_INITIALS" VARCHAR2(8), 
	"DATABASE_UPDATED" DATE, 
	"UPDATOR_INITIALS" VARCHAR2(8), 
	"NUM_JOBS_ACCEPTED" NUMBER(6,0), 
	"NUM_JOBS_REJECTED" NUMBER(6,0), 
	"NUM_DUPLICATES" NUMBER(10,0)
   ) ;
--------------------------------------------------------
--  DDL for Table LAYER_CRITERIA_LOOKUP
--------------------------------------------------------

  CREATE TABLE "MEDSADMIN"."LAYER_CRITERIA_LOOKUP" 
   (	"LAYER_INDEX" NUMBER(10,0), 
	"CRITERIA_STRING" VARCHAR2(4000), 
	"CAPTION" VARCHAR2(50)
   ) ;
--------------------------------------------------------
--  DDL for Table LAYER_CRITERIA_LOOKUP_COPY
--------------------------------------------------------

  CREATE TABLE "MEDSADMIN"."LAYER_CRITERIA_LOOKUP_COPY" 
   (	"LAYER_INDEX" NUMBER(10,0), 
	"CRITERIA_STRING" VARCHAR2(100), 
	"CAPTION" VARCHAR2(50)
   ) ;
--------------------------------------------------------
--  DDL for Table LAYER_LOOKUP
--------------------------------------------------------

  CREATE TABLE "MEDSADMIN"."LAYER_LOOKUP" 
   (	"LAYER_SQL" VARCHAR2(4000), 
	"USER_NAME" VARCHAR2(75), 
	"LAYER_NAME" VARCHAR2(150), 
	"DATA_TYPE_INDEX" NUMBER(10,0), 
	"LAYER_INDEX" NUMBER(10,0), 
	"DATA_TABLE" VARCHAR2(128)
   ) ;
--------------------------------------------------------
--  DDL for Table LEVEL_TABLES_LOOKUP
--------------------------------------------------------

  CREATE TABLE "MEDSADMIN"."LEVEL_TABLES_LOOKUP" 
   (	"DATA_TYPE_INDEX" NUMBER(8,0), 
	"TABLE_NAME" VARCHAR2(50), 
	"INSERT_UPDATE" NUMBER(1,0), 
	"FILE_EXTENSION" VARCHAR2(20), 
	"LEVEL_TABLE_INDEX" NUMBER(8,0)
   ) ;
--------------------------------------------------------
--  DDL for Table MAPINFO_MAPCATALOG
--------------------------------------------------------

  CREATE TABLE "MEDSADMIN"."MAPINFO_MAPCATALOG" 
   (	"SPATIALTYPE" FLOAT(126), 
	"TABLENAME" VARCHAR2(32), 
	"OWNERNAME" VARCHAR2(32), 
	"SPATIALCOLUMN" VARCHAR2(32), 
	"DB_X_LL" FLOAT(126), 
	"DB_Y_LL" FLOAT(126), 
	"DB_X_UR" FLOAT(126), 
	"DB_Y_UR" FLOAT(126), 
	"COORDINATESYSTEM" VARCHAR2(254), 
	"SYMBOL" VARCHAR2(254), 
	"XCOLUMNNAME" VARCHAR2(32), 
	"YCOLUMNNAME" VARCHAR2(32)
   ) ;
--------------------------------------------------------
--  DDL for Table MASTER_GRID_D1
--------------------------------------------------------

  CREATE TABLE "MEDSADMIN"."MASTER_GRID_D1" 
   (	"LOCATION" "MDSYS"."SDO_GEOMETRY" , 
	"GRID_INDEX" NUMBER(8,0), 
	"JOB_PROFILE_JAN" NUMBER(8,0), 
	"JOB_PROFILE_FEB" NUMBER(8,0), 
	"JOB_PROFILE_MAR" NUMBER(8,0), 
	"JOB_PROFILE_APR" NUMBER(8,0), 
	"JOB_PROFILE_MAY" NUMBER(8,0), 
	"JOB_PROFILE_JUN" NUMBER(8,0), 
	"JOB_PROFILE_JUL" NUMBER(8,0), 
	"JOB_PROFILE_AUG" NUMBER(8,0), 
	"JOB_PROFILE_SEP" NUMBER(8,0), 
	"JOB_PROFILE_OCT" NUMBER(8,0), 
	"JOB_PROFILE_NOV" NUMBER(8,0), 
	"JOB_PROFILE_DEC" NUMBER(8,0), 
	"JOB_SEA_SURFACE" NUMBER(8,0), 
	"JOB_MIXED_LAYER" NUMBER(8,0), 
	"JOB_SPECIES_BALEEN" NUMBER(8,0), 
	"JOB_SPECIES_DOLPHINS" NUMBER(8,0), 
	"JOB_SPECIES_PORPOISES" NUMBER(8,0), 
	"JOB_SPECIES_HAZARDS" NUMBER(8,0), 
	"JOB_SPECIES_STINGING" NUMBER(8,0), 
	"JOB_SPECIES_TOOTHED" NUMBER(8,0), 
	"JOB_SPECIES_REVERBERATORS" NUMBER(8,0), 
	"JOB_PADS" NUMBER(8,0), 
	"JOB_HITS" NUMBER(8,0), 
	"JOB_AMBIENT_OMNI" NUMBER(8,0), 
	"LATITUDE" NUMBER(12,3), 
	"LONGITUDE" NUMBER(12,3), 
	"DATATYPE" VARCHAR2(20) DEFAULT 'GENERIC'
   ) ;
--------------------------------------------------------
--  DDL for Table MASTER_GRID_L1
--------------------------------------------------------

  CREATE TABLE "MEDSADMIN"."MASTER_GRID_L1" 
   (	"DBDBV_DEPTH" NUMBER(8,2), 
	"LOCATION" "MDSYS"."SDO_GEOMETRY" , 
	"GRID_INDEX" NUMBER(8,0), 
	"JOB_PROFILE_JAN" NUMBER(8,0), 
	"JOB_PROFILE_FEB" NUMBER(8,0), 
	"JOB_PROFILE_MAR" NUMBER(8,0), 
	"JOB_PROFILE_APR" NUMBER(8,0), 
	"JOB_PROFILE_MAY" NUMBER(8,0), 
	"JOB_PROFILE_JUN" NUMBER(8,0), 
	"JOB_PROFILE_JUL" NUMBER(8,0), 
	"JOB_PROFILE_AUG" NUMBER(8,0), 
	"JOB_PROFILE_SEP" NUMBER(8,0), 
	"JOB_PROFILE_OCT" NUMBER(8,0), 
	"JOB_PROFILE_NOV" NUMBER(8,0), 
	"JOB_PROFILE_DEC" NUMBER(8,0), 
	"JOB_SEA_SURFACE" NUMBER(8,0), 
	"JOB_MIXED_LAYER" NUMBER(8,0), 
	"JOB_SPECIES_BALEEN" NUMBER(8,0), 
	"JOB_SPECIES_DOLPHINS" NUMBER(8,0), 
	"JOB_SPECIES_HAZARDS" NUMBER(8,0), 
	"JOB_SPECIES_STINGING" NUMBER(8,0), 
	"JOB_SPECIES_TOOTHED" NUMBER(8,0), 
	"JOB_SPECIES_REVERBERATORS" NUMBER(8,0), 
	"JOB_PADS" NUMBER(8,0), 
	"JOB_HITS" NUMBER(8,0), 
	"JOB_AMBIENT_OMNI" NUMBER(8,0), 
	"LATITUDE" NUMBER(12,3), 
	"LONGITUDE" NUMBER(12,3), 
	"JOB_SPECIES_PORPOISE" NUMBER(8,0), 
	"JOB_SPECIES_BEAKED" NUMBER(8,0), 
	"DATATYPE" VARCHAR2(20) DEFAULT 'GENERIC'
   ) ;
--------------------------------------------------------
--  DDL for Table MASTER_GRID_L2
--------------------------------------------------------

  CREATE TABLE "MEDSADMIN"."MASTER_GRID_L2" 
   (	"DBDBV_DEPTH" NUMBER(8,2), 
	"LOCATION" "MDSYS"."SDO_GEOMETRY" , 
	"GRID_INDEX" NUMBER(8,0), 
	"JOB_PROFILE_JAN" NUMBER(8,0), 
	"JOB_PROFILE_FEB" NUMBER(8,0), 
	"JOB_PROFILE_MAR" NUMBER(8,0), 
	"JOB_PROFILE_APR" NUMBER(8,0), 
	"JOB_PROFILE_MAY" NUMBER(8,0), 
	"JOB_PROFILE_JUN" NUMBER(8,0), 
	"JOB_PROFILE_JUL" NUMBER(8,0), 
	"JOB_PROFILE_AUG" NUMBER(8,0), 
	"JOB_PROFILE_SEP" NUMBER(8,0), 
	"JOB_PROFILE_OCT" NUMBER(8,0), 
	"JOB_PROFILE_NOV" NUMBER(8,0), 
	"JOB_PROFILE_DEC" NUMBER(8,0), 
	"JOB_SEA_SURFACE" NUMBER(8,0), 
	"JOB_MIXED_LAYER" NUMBER(8,0), 
	"JOB_SPECIES_BALEEN" NUMBER(8,0), 
	"JOB_SPECIES_DOLPHINS" NUMBER(8,0), 
	"JOB_SPECIES_PORPOISES" NUMBER(8,0), 
	"JOB_SPECIES_HAZARDS" NUMBER(8,0), 
	"JOB_SPECIES_STINGING" NUMBER(8,0), 
	"JOB_SPECIES_TOOTHED" NUMBER(8,0), 
	"JOB_SPECIES_REVERBERATORS" NUMBER(8,0), 
	"JOB_PADS" NUMBER(8,0), 
	"JOB_HITS" NUMBER(8,0), 
	"JOB_AMBIENT_OMNI" NUMBER(8,0), 
	"LATITUDE" NUMBER(12,3), 
	"LONGITUDE" NUMBER(12,3), 
	"DATATYPE" VARCHAR2(20) DEFAULT 'GENERIC'
   ) ;
--------------------------------------------------------
--  DDL for Table MASTER_GRID_L4
--------------------------------------------------------

  CREATE TABLE "MEDSADMIN"."MASTER_GRID_L4" 
   (	"DBDBV_DEPTH" NUMBER(8,2), 
	"LOCATION" "MDSYS"."SDO_GEOMETRY" , 
	"GRID_INDEX" NUMBER(8,0), 
	"JOB_PROFILE_JAN" NUMBER(8,0), 
	"JOB_PROFILE_FEB" NUMBER(8,0), 
	"JOB_PROFILE_MAR" NUMBER(8,0), 
	"JOB_PROFILE_APR" NUMBER(8,0), 
	"JOB_PROFILE_MAY" NUMBER(8,0), 
	"JOB_PROFILE_JUN" NUMBER(8,0), 
	"JOB_PROFILE_JUL" NUMBER(8,0), 
	"JOB_PROFILE_AUG" NUMBER(8,0), 
	"JOB_PROFILE_SEP" NUMBER(8,0), 
	"JOB_PROFILE_OCT" NUMBER(8,0), 
	"JOB_PROFILE_NOV" NUMBER(8,0), 
	"JOB_PROFILE_DEC" NUMBER(8,0), 
	"JOB_SEA_SURFACE" NUMBER(8,0), 
	"JOB_MIXED_LAYER" NUMBER(8,0), 
	"JOB_SPECIES_BALEEN" NUMBER(8,0), 
	"JOB_SPECIES_DOLPHINS" NUMBER(8,0), 
	"JOB_SPECIES_PORPOISES" NUMBER(8,0), 
	"JOB_SPECIES_HAZARDS" NUMBER(8,0), 
	"JOB_SPECIES_STINGING" NUMBER(8,0), 
	"JOB_SPECIES_TOOTHED" NUMBER(8,0), 
	"JOB_SPECIES_REVERBERATORS" NUMBER(8,0), 
	"JOB_PADS" NUMBER(8,0), 
	"JOB_HITS" NUMBER(8,0), 
	"JOB_AMBIENT_OMNI" NUMBER(8,0), 
	"LATITUDE" NUMBER(12,3), 
	"LONGITUDE" NUMBER(12,3), 
	"DATATYPE" VARCHAR2(20) DEFAULT 'GENERIC'
   ) ;
--------------------------------------------------------
--  DDL for Table MEDS_ERRORS
--------------------------------------------------------

  CREATE TABLE "MEDSADMIN"."MEDS_ERRORS" 
   (	"MEDS_ERROR_TYPE" NUMBER, 
	"ERROR_DESCRIPTION" VARCHAR2(64)
   ) ;
--------------------------------------------------------
--  DDL for Table MEDS_FLAGS
--------------------------------------------------------

  CREATE TABLE "MEDSADMIN"."MEDS_FLAGS" 
   (	"FLAG_STATUS" NUMBER(1,0), 
	"AUTHOR" VARCHAR2(100), 
	"DATE_ENTERED" DATE, 
	"COMMENTS" VARCHAR2(500), 
	"CATEGORY" VARCHAR2(20), 
	"DATA_TYPE" VARCHAR2(50), 
	"MEDS_JOB_NUMBER" NUMBER(10,0), 
	"MEDS_OBSERVATION_NUMBER" NUMBER(10,0), 
	"GRID_INDEX" NUMBER(10,0), 
	"GRID_RESOLUTION" NUMBER(2,0), 
	"GRID_TYPE" VARCHAR2(1), 
	"GRID_TEMPORAL" VARCHAR2(10), 
	"FLAGS_INDEX" NUMBER(10,0), 
	"LATITUDE" NUMBER(12,6), 
	"LONGITUDE" NUMBER(12,6), 
	"FLAG_VALUE" VARCHAR2(100)
   ) ;
--------------------------------------------------------
--  DDL for Table MEDS_FLAGS_ORIG
--------------------------------------------------------

  CREATE TABLE "MEDSADMIN"."MEDS_FLAGS_ORIG" 
   (	"FLAG_STATUS" NUMBER(1,0), 
	"AUTHOR" VARCHAR2(100), 
	"DATE_ENTERED" DATE, 
	"COMMENTS" VARCHAR2(500), 
	"CATEGORY" VARCHAR2(20), 
	"DATA_TYPE" VARCHAR2(50), 
	"MEDS_JOB_NUMBER" NUMBER(10,0), 
	"MEDS_OBSERVATION_NUMBER" NUMBER(10,0), 
	"GRID_INDEX" NUMBER(10,0), 
	"GRID_RESOLUTION" NUMBER(2,0), 
	"GRID_TYPE" VARCHAR2(1), 
	"GRID_TEMPORAL" VARCHAR2(10), 
	"FLAGS_INDEX" NUMBER(10,0), 
	"LATITUDE" NUMBER(10,0), 
	"LONGITUDE" NUMBER(10,0), 
	"FLAG_VALUE" VARCHAR2(100)
   ) ;
--------------------------------------------------------
--  DDL for Table MEDS_IPR
--------------------------------------------------------

  CREATE TABLE "MEDSADMIN"."MEDS_IPR" 
   (	"MEDS_JOB_NUMBER" NUMBER(10,0), 
	"IPR_OWNER_TYPE" VARCHAR2(100), 
	"IPR_PERIOD" DATE, 
	"IPR_OWNER" VARCHAR2(50), 
	"RIGHTS" NUMBER(1,0), 
	"FOI_EXEMPT" NUMBER(1,0), 
	"FOI_DECLARE" NUMBER(1,0), 
	"CONFIDENTIALITY" NUMBER(1,0)
   ) ;
--------------------------------------------------------
--  DDL for Table MEDS_JOB_ERROR
--------------------------------------------------------

  CREATE TABLE "MEDSADMIN"."MEDS_JOB_ERROR" 
   (	"MEIC_NUMBER" NUMBER, 
	"MEDS_ERROR_TYPE" NUMBER, 
	"NUM_ERRORS" NUMBER
   ) ;
--------------------------------------------------------
--  DDL for Table MEDS_JOB_TYPE
--------------------------------------------------------

  CREATE TABLE "MEDSADMIN"."MEDS_JOB_TYPE" 
   (	"JOB_TYPE_NUMBER" NUMBER, 
	"DESCRIPTION" VARCHAR2(32)
   ) ;
--------------------------------------------------------
--  DDL for Table MEDS_PROCESSING_JOB
--------------------------------------------------------

  CREATE TABLE "MEDSADMIN"."MEDS_PROCESSING_JOB" 
   (	"DATUM_TRANSFORMATION" VARCHAR2(50), 
	"LOCAL_DATUM" VARCHAR2(50), 
	"ARCHIVE_DATE" DATE, 
	"COMMENTS" VARCHAR2(500), 
	"JOB_NUMBER" NUMBER(10,0), 
	"BOOKING_JOB" NUMBER(10,0), 
	"PROCESSOR" VARCHAR2(50), 
	"DATE_OF_RECEIPT" DATE, 
	"DATA_TYPE" VARCHAR2(50), 
	"PROTECTION" VARCHAR2(50), 
	"CAVEAT" VARCHAR2(50), 
	"DESCRIPTOR" VARCHAR2(50), 
	"IDO" VARCHAR2(50), 
	"DATA_QUALITY" VARCHAR2(50), 
	"RELEASE_CONDITIONS" VARCHAR2(50), 
	"OWNER_AUTHORITY_PREFIX" VARCHAR2(50), 
	"ORIGINATOR" VARCHAR2(50), 
	"INSTRUMENT_CODE" NUMBER(2,0), 
	"MEDS_SHIP_NUMBER" NUMBER(10,0), 
	"MEDS_CRUISE_NUMBER" NUMBER(10,0), 
	"DATA_USE_CODE" VARCHAR2(20), 
	"MEIC_NUMBER" NUMBER(10,0)
   ) ;
--------------------------------------------------------
--  DDL for Table MEDS_ROWS_PER_JOB
--------------------------------------------------------

  CREATE TABLE "MEDSADMIN"."MEDS_ROWS_PER_JOB" 
   (	"BOOKING_JOB" NUMBER(*,0), 
	"N_ROWS" NUMBER(*,0)
   ) ;
--------------------------------------------------------
--  DDL for Table MLO_BIRDS_DATA
--------------------------------------------------------

  CREATE TABLE "MEDSADMIN"."MLO_BIRDS_DATA" 
   (	"RECORD_NUMBER" NUMBER(6,0), 
	"COUNT_QUALITY" NUMBER(10,0), 
	"SPECIES" VARCHAR2(100), 
	"ID_QUALITY" NUMBER(8,0), 
	"SHIP" VARCHAR2(50), 
	"MEDS_OBSERVATION_NUMBER" NUMBER(10,0), 
	"MEDS_JOB_NUMBER" NUMBER(10,0), 
	"ARCHIVED" VARCHAR2(25), 
	"FILE_REFERENCE" VARCHAR2(100), 
	"COMMENTS" VARCHAR2(500), 
	"COUNT" VARCHAR2(20), 
	"WD_DIR" VARCHAR2(50), 
	"WD_SPD" VARCHAR2(50)
   ) ;
--------------------------------------------------------
--  DDL for Table MLO_BIRDS_OBSERVATION
--------------------------------------------------------

  CREATE TABLE "MEDSADMIN"."MLO_BIRDS_OBSERVATION" 
   (	"MEDS_OBSERVATION_NUMBER" NUMBER(10,0), 
	"MEDS_JOB_NUMBER" NUMBER(10,0), 
	"LATITUDE" NUMBER(12,6), 
	"LONGITUDE" NUMBER(12,6), 
	"LOCATION" "MDSYS"."SDO_GEOMETRY" , 
	"DATE_RECORDED" DATE
   ) ;
--------------------------------------------------------
--  DDL for Table MLO_CETACEANS_DATA
--------------------------------------------------------

  CREATE TABLE "MEDSADMIN"."MLO_CETACEANS_DATA" 
   (	"RECORD_NUMBER" NUMBER(6,0), 
	"COUNT" VARCHAR2(10), 
	"COUNT_QUALITY" NUMBER(10,0), 
	"SPECIES" VARCHAR2(200), 
	"ID_QUALITY" NUMBER(8,0), 
	"HEADING" VARCHAR2(50), 
	"COMMENTS" VARCHAR2(600), 
	"SST" NUMBER(8,3), 
	"PISCES_GENUS" VARCHAR2(30), 
	"SPEED" NUMBER(8,0), 
	"REFERENCE" VARCHAR2(100), 
	"SHIP" VARCHAR2(50), 
	"MEDS_OBSERVATION_NUMBER" NUMBER(10,0), 
	"MEDS_JOB_NUMBER" NUMBER(10,0), 
	"ARCHIVED" VARCHAR2(25), 
	"SIZE_CLASS" VARCHAR2(20), 
	"MONTH" NUMBER(4,0), 
	"PISCES_SPECIES" VARCHAR2(20), 
	"SIZE_" VARCHAR2(20), 
	"MEIC" VARCHAR2(20), 
	"RANGE" VARCHAR2(20), 
	"BEARING" VARCHAR2(20), 
	"PERSON_NAME" VARCHAR2(100)
   ) ;
--------------------------------------------------------
--  DDL for Table MLO_CETACEANS_ENVELOPE
--------------------------------------------------------

  CREATE TABLE "MEDSADMIN"."MLO_CETACEANS_ENVELOPE" 
   (	"ENVELOPE" "MDSYS"."SDO_GEOMETRY" , 
	"MEDS_JOB_NUMBER" NUMBER(10,0), 
	"MEDS_TRACK_NUMBER" NUMBER(10,0), 
	"NUM_OBS" NUMBER(10,0) DEFAULT 0
   ) ;
--------------------------------------------------------
--  DDL for Table MLO_CETACEANS_OBSERVATION
--------------------------------------------------------

  CREATE TABLE "MEDSADMIN"."MLO_CETACEANS_OBSERVATION" 
   (	"MEDS_OBSERVATION_NUMBER" NUMBER(10,0), 
	"MEDS_JOB_NUMBER" NUMBER(10,0), 
	"LATITUDE" NUMBER(12,6), 
	"LONGITUDE" NUMBER(12,6), 
	"LOCATION" "MDSYS"."SDO_GEOMETRY" , 
	"DATE_RECORDED" DATE, 
	"MEDS_TRACK_NUMBER" NUMBER(10,0) DEFAULT NULL
   ) ;
--------------------------------------------------------
--  DDL for Table MLO_CETACEANS_TRACK
--------------------------------------------------------

  CREATE TABLE "MEDSADMIN"."MLO_CETACEANS_TRACK" 
   (	"LOCATION" "MDSYS"."SDO_GEOMETRY" , 
	"MEDS_JOB_NUMBER" NUMBER(10,0), 
	"MEDS_TRACK_NUMBER" NUMBER(10,0), 
	"LONGITUDE_START" NUMBER(20,10), 
	"LATITUDE_START" NUMBER(20,10), 
	"DATE_RECORDED_START" DATE, 
	"OFFSET_START" NUMBER(20,10), 
	"LONGITUDE_END" NUMBER(20,10), 
	"LATITUDE_END" NUMBER(20,10), 
	"DATE_RECORDED_END" DATE, 
	"OFFSET_END" NUMBER(20,10), 
	"SHIP" VARCHAR2(50), 
	"NUM_OBS" NUMBER(10,0) DEFAULT 0, 
	"MONITORING_TYPE" VARCHAR2(1) DEFAULT 'V', 
	"PERIOD_OF_SONAR" NUMBER(10,0) DEFAULT NULL, 
	"SEA_STATE" NUMBER(10,0) DEFAULT NULL, 
	"WIND_SPEED" NUMBER(10,0) DEFAULT NULL, 
	"SOUND_SOURCE" VARCHAR2(50) DEFAULT NULL
   ) ;
--------------------------------------------------------
--  DDL for Table MLO_COLUMNS
--------------------------------------------------------

  CREATE TABLE "MEDSADMIN"."MLO_COLUMNS" 
   (	"TABLE_NAME" VARCHAR2(30), 
	"COLUMN_NAME" VARCHAR2(30), 
	"EDIT_ORDER" NUMBER, 
	"MANDATORY" NUMBER
   ) ;
--------------------------------------------------------
--  DDL for Table MLO_DATA_TYPE
--------------------------------------------------------

  CREATE TABLE "MEDSADMIN"."MLO_DATA_TYPE" 
   (	"DATA_TYPE" VARCHAR2(32), 
	"PROCESSING_JOB_TYPE" VARCHAR2(50), 
	"VARIABLE_COLUMN_NAME" VARCHAR2(32)
   ) ;
--------------------------------------------------------
--  DDL for Table MLO_FISH_DATA
--------------------------------------------------------

  CREATE TABLE "MEDSADMIN"."MLO_FISH_DATA" 
   (	"RECORD_NUMBER" NUMBER(6,0), 
	"COUNT" VARCHAR2(10), 
	"SPECIES" VARCHAR2(50), 
	"ID_QUALITY" NUMBER(8,0), 
	"HEADING" VARCHAR2(50), 
	"MEDS_OBSERVATION_NUMBER" NUMBER(10,0), 
	"MEDS_JOB_NUMBER" NUMBER(10,0), 
	"REFERENCE" VARCHAR2(100), 
	"SHIP" VARCHAR2(50), 
	"ARCHIVED" VARCHAR2(25), 
	"COMMENTS" VARCHAR2(500), 
	"SIZE_" VARCHAR2(50), 
	"SPEED" VARCHAR2(20), 
	"MONTH" VARCHAR2(20), 
	"COUNT_QUALITY" VARCHAR2(20), 
	"RANGE" VARCHAR2(20), 
	"BEARING" VARCHAR2(20), 
	"PERSON_NAME" VARCHAR2(100)
   ) ;
--------------------------------------------------------
--  DDL for Table MLO_FISH_OBSERVATION
--------------------------------------------------------

  CREATE TABLE "MEDSADMIN"."MLO_FISH_OBSERVATION" 
   (	"MEDS_OBSERVATION_NUMBER" NUMBER(10,0), 
	"MEDS_JOB_NUMBER" NUMBER(10,0), 
	"LATITUDE" NUMBER(12,6), 
	"LONGITUDE" NUMBER(12,6), 
	"LOCATION" "MDSYS"."SDO_GEOMETRY" , 
	"DATE_RECORDED" DATE
   ) ;
--------------------------------------------------------
--  DDL for Table MLO_JELLYFISH_DATA
--------------------------------------------------------

  CREATE TABLE "MEDSADMIN"."MLO_JELLYFISH_DATA" 
   (	"RECORD_NUMBER" NUMBER(6,0), 
	"COUNT" VARCHAR2(20), 
	"SPECIES" VARCHAR2(50), 
	"HEADING" VARCHAR2(50), 
	"COMMENTS" VARCHAR2(200), 
	"MEDS_OBSERVATION_NUMBER" NUMBER(10,0), 
	"MEDS_JOB_NUMBER" NUMBER(10,0), 
	"REFERENCE" VARCHAR2(100), 
	"SHIP" VARCHAR2(50), 
	"ARCHIVED" VARCHAR2(25), 
	"COUNT_QUALITY" VARCHAR2(20), 
	"SPEED" VARCHAR2(20), 
	"ID_QUALITY" VARCHAR2(20), 
	"SST" VARCHAR2(20), 
	"MONTH" VARCHAR2(10), 
	"SIZE_" VARCHAR2(20), 
	"BEARING" VARCHAR2(20), 
	"RANGE" VARCHAR2(20), 
	"PERSON_NAME" VARCHAR2(100)
   ) ;
--------------------------------------------------------
--  DDL for Table MLO_JELLYFISH_OBSERVATION
--------------------------------------------------------

  CREATE TABLE "MEDSADMIN"."MLO_JELLYFISH_OBSERVATION" 
   (	"MEDS_OBSERVATION_NUMBER" NUMBER(10,0), 
	"MEDS_JOB_NUMBER" NUMBER(10,0), 
	"LATITUDE" NUMBER(12,6), 
	"LONGITUDE" NUMBER(12,6), 
	"LOCATION" "MDSYS"."SDO_GEOMETRY" , 
	"DATE_RECORDED" DATE
   ) ;
--------------------------------------------------------
--  DDL for Table MLO_OTHERS_DATA
--------------------------------------------------------

  CREATE TABLE "MEDSADMIN"."MLO_OTHERS_DATA" 
   (	"RECORD_NUMBER" NUMBER(6,0), 
	"COUNT" VARCHAR2(10), 
	"SPECIES" VARCHAR2(500), 
	"HEADING" VARCHAR2(50), 
	"COMMENTS" VARCHAR2(200), 
	"MEDS_JOB_NUMBER" NUMBER(10,0), 
	"MEDS_OBSERVATION_NUMBER" NUMBER(10,0), 
	"REFERENCE" VARCHAR2(100), 
	"SHIP" VARCHAR2(50), 
	"ARCHIVED" VARCHAR2(25), 
	"MONTH" VARCHAR2(20), 
	"SIZE_" VARCHAR2(20), 
	"COUNT_QUALITY" VARCHAR2(20), 
	"ID_QUALITY" VARCHAR2(20), 
	"SPEED" VARCHAR2(20), 
	"SST" VARCHAR2(20), 
	"WD_DIR" VARCHAR2(50), 
	"WD_SPD" VARCHAR2(50), 
	"RANGE" VARCHAR2(20), 
	"BEARING" VARCHAR2(20)
   ) ;
--------------------------------------------------------
--  DDL for Table MLO_OTHERS_OBSERVATION
--------------------------------------------------------

  CREATE TABLE "MEDSADMIN"."MLO_OTHERS_OBSERVATION" 
   (	"MEDS_OBSERVATION_NUMBER" NUMBER(10,0), 
	"MEDS_JOB_NUMBER" NUMBER(10,0), 
	"LATITUDE" NUMBER(12,6), 
	"LONGITUDE" NUMBER(12,6), 
	"LOCATION" "MDSYS"."SDO_GEOMETRY" , 
	"DATE_RECORDED" DATE
   ) ;
--------------------------------------------------------
--  DDL for Table MLO_PINNIPEDS_DATA
--------------------------------------------------------

  CREATE TABLE "MEDSADMIN"."MLO_PINNIPEDS_DATA" 
   (	"RECORD_NUMBER" NUMBER(6,0), 
	"COUNT" VARCHAR2(10), 
	"HEADING" VARCHAR2(50), 
	"COMMENTS" VARCHAR2(200), 
	"SPECIES" VARCHAR2(50), 
	"REFERENCE" VARCHAR2(100), 
	"SHIP" VARCHAR2(50), 
	"MEDS_JOB_NUMBER" NUMBER(10,0), 
	"MEDS_OBSERVATION_NUMBER" NUMBER(10,0), 
	"ARCHIVED" VARCHAR2(25), 
	"COUNT_QUALITY" VARCHAR2(20), 
	"ID_QUALITY" VARCHAR2(20), 
	"CODE" VARCHAR2(10), 
	"SPEED" VARCHAR2(20), 
	"SIZE_" VARCHAR2(20), 
	"MONTH" VARCHAR2(10), 
	"RANGE" VARCHAR2(20), 
	"BEARING" VARCHAR2(20), 
	"PERSON_NAME" VARCHAR2(100)
   ) ;
--------------------------------------------------------
--  DDL for Table MLO_PINNIPEDS_OBSERVATION
--------------------------------------------------------

  CREATE TABLE "MEDSADMIN"."MLO_PINNIPEDS_OBSERVATION" 
   (	"MEDS_OBSERVATION_NUMBER" NUMBER(10,0), 
	"MEDS_JOB_NUMBER" NUMBER(10,0), 
	"LATITUDE" NUMBER(12,6), 
	"LONGITUDE" NUMBER(12,6), 
	"LOCATION" "MDSYS"."SDO_GEOMETRY" , 
	"DATE_RECORDED" DATE
   ) ;
--------------------------------------------------------
--  DDL for Table MLO_REPTILES_DATA
--------------------------------------------------------

  CREATE TABLE "MEDSADMIN"."MLO_REPTILES_DATA" 
   (	"RECORD_NUMBER" NUMBER(6,0), 
	"COUNT" VARCHAR2(10), 
	"SPECIES" VARCHAR2(50), 
	"HEADING" VARCHAR2(50), 
	"COMMENTS" VARCHAR2(200), 
	"MEDS_OBSERVATION_NUMBER" NUMBER(10,0), 
	"MEDS_JOB_NUMBER" NUMBER(10,0), 
	"REFERENCE" VARCHAR2(100), 
	"SHIP" VARCHAR2(50), 
	"ARCHIVED" VARCHAR2(25), 
	"COUNT_QUALITY" VARCHAR2(20), 
	"SIZE_" VARCHAR2(20), 
	"MONTH" VARCHAR2(10), 
	"ID_QUALITY" VARCHAR2(20), 
	"SPEED" VARCHAR2(20), 
	"RANGE" VARCHAR2(20), 
	"BEARING" VARCHAR2(20), 
	"PERSON_NAME" VARCHAR2(100)
   ) ;
--------------------------------------------------------
--  DDL for Table MLO_REPTILES_OBSERVATION
--------------------------------------------------------

  CREATE TABLE "MEDSADMIN"."MLO_REPTILES_OBSERVATION" 
   (	"MEDS_OBSERVATION_NUMBER" NUMBER(10,0), 
	"MEDS_JOB_NUMBER" NUMBER(10,0), 
	"LATITUDE" NUMBER(12,6), 
	"LONGITUDE" NUMBER(12,6), 
	"LOCATION" "MDSYS"."SDO_GEOMETRY" , 
	"DATE_RECORDED" DATE
   ) ;
--------------------------------------------------------
--  DDL for Table MLO_SEABED_CONTACT_DATA
--------------------------------------------------------

  CREATE TABLE "MEDSADMIN"."MLO_SEABED_CONTACT_DATA" 
   (	"MEDS_JOB_NUMBER" NUMBER(10,0), 
	"MEDS_OBSERVATION_NUMBER" NUMBER(10,0), 
	"DEPTH" NUMBER(12,4), 
	"SHIP" VARCHAR2(50), 
	"REPORT_NO" VARCHAR2(20), 
	"PREVAILING_WEATHER" VARCHAR2(50), 
	"BOTTOM_TYPE" VARCHAR2(10), 
	"BOTTOM_MATERIAL" VARCHAR2(50), 
	"SOFTNESS_HS_P1" VARCHAR2(50), 
	"SOFTNESS_S_P2" NUMBER(8,0), 
	"SLOPE" VARCHAR2(50), 
	"WRECKAGE_AND_DEBRIS" VARCHAR2(50), 
	"MARINE_LIFE" VARCHAR2(50), 
	"MARINE_GROWTH" VARCHAR2(50), 
	"VISIBILITY_AT_SEABED" VARCHAR2(50), 
	"CONTACT_REFERENCE" VARCHAR2(20), 
	"DIMENSIONS_LENGTH" NUMBER(10,4), 
	"DIMENSIONS_WIDTH" NUMBER(10,4), 
	"DIMENSIONS_HEIGHT" NUMBER(10,4), 
	"DESCRIPTION" VARCHAR2(500), 
	"GENERAL_LOCATION" VARCHAR2(50), 
	"BOTTOM_MATERIAL_NO" NUMBER(10,0), 
	"VISIBILITY_AT_SEABED_DEPTH" NUMBER(12,4), 
	"DIMENSIONS_MEASURED" VARCHAR2(50)
   ) ;

   COMMENT ON COLUMN "MEDSADMIN"."MLO_SEABED_CONTACT_DATA"."MARINE_LIFE" IS 'LOW=1-5 FISH; MEDIUM=6-20 FISH; HIGH=>20 FISH';
   COMMENT ON COLUMN "MEDSADMIN"."MLO_SEABED_CONTACT_DATA"."MARINE_GROWTH" IS 'LOW=SLIGHT COVERING; MEDIUM=WELL COVERED BUT VISUAL ID POSSIBLE; HIGH=VERY DENSE VISUAL ID DIFFICULT';
--------------------------------------------------------
--  DDL for Table MLO_SEABED_CONTACT_OBSERVATION
--------------------------------------------------------

  CREATE TABLE "MEDSADMIN"."MLO_SEABED_CONTACT_OBSERVATION" 
   (	"LOCATION" "MDSYS"."SDO_GEOMETRY" , 
	"MEDS_JOB_NUMBER" NUMBER(10,0), 
	"MEDS_OBSERVATION_NUMBER" NUMBER(10,0), 
	"LATITUDE" NUMBER(12,6), 
	"LONGITUDE" NUMBER(12,6), 
	"DATE_RECORDED" DATE
   ) ;
--------------------------------------------------------
--  DDL for Table MLO_SHIP_DATA
--------------------------------------------------------

  CREATE TABLE "MEDSADMIN"."MLO_SHIP_DATA" 
   (	"RECORD_NUMBER" NUMBER(6,0), 
	"COUNT" VARCHAR2(10), 
	"SHIP_TABLE" VARCHAR2(200), 
	"HEADING" VARCHAR2(50), 
	"COMMENTS" VARCHAR2(200), 
	"CODE" NUMBER(4,0), 
	"ACTIVITY" VARCHAR2(200), 
	"REFERENCE" VARCHAR2(100), 
	"SHIP" VARCHAR2(50), 
	"MEDS_OBSERVATION_NUMBER" NUMBER(10,0), 
	"MEDS_JOB_NUMBER" NUMBER(10,0), 
	"ARCHIVED" VARCHAR2(25), 
	"SIZE_" VARCHAR2(20), 
	"MONTH" VARCHAR2(20), 
	"COUNT_QUALITY" VARCHAR2(20), 
	"SPEED" VARCHAR2(20), 
	"SPARE_1" VARCHAR2(20), 
	"SPARE_2" VARCHAR2(20), 
	"SPARE_3" VARCHAR2(20), 
	"RANGE" VARCHAR2(20), 
	"BEARING" VARCHAR2(20)
   ) ;
--------------------------------------------------------
--  DDL for Table MLO_SHIP_OBSERVATION
--------------------------------------------------------

  CREATE TABLE "MEDSADMIN"."MLO_SHIP_OBSERVATION" 
   (	"MEDS_OBSERVATION_NUMBER" NUMBER(10,0), 
	"MEDS_JOB_NUMBER" NUMBER(10,0), 
	"LATITUDE" NUMBER(12,6), 
	"LONGITUDE" NUMBER(12,6), 
	"LOCATION" "MDSYS"."SDO_GEOMETRY" , 
	"DATE_RECORDED" DATE
   ) ;
--------------------------------------------------------
--  DDL for Table MLO_SOSUS_DATA
--------------------------------------------------------

  CREATE TABLE "MEDSADMIN"."MLO_SOSUS_DATA" 
   (	"RECORD_NUMBER" NUMBER(6,0), 
	"FILE_IDENTIFIER" NUMBER(8,0), 
	"TRACK_NAME" VARCHAR2(20), 
	"TRACK_NUMBER" NUMBER(4,0), 
	"SPECIES" VARCHAR2(20), 
	"MONTH" NUMBER(2,0), 
	"INCLINATION" NUMBER(3,0), 
	"MAJOR_AXIS" NUMBER(8,3), 
	"MINOR_AXIS" NUMBER(8,3), 
	"MEDS_JOB_NUMBER" NUMBER(10,0), 
	"MEDS_OBSERVATION_NUMBER" NUMBER(10,0)
   ) ;
--------------------------------------------------------
--  DDL for Table MLO_SOSUS_OBSERVATION
--------------------------------------------------------

  CREATE TABLE "MEDSADMIN"."MLO_SOSUS_OBSERVATION" 
   (	"MEDS_OBSERVATION_NUMBER" NUMBER(10,0), 
	"MEDS_JOB_NUMBER" NUMBER(10,0), 
	"LOCATION" "MDSYS"."SDO_GEOMETRY" , 
	"DATE_RECORDED" DATE, 
	"LATITUDE" NUMBER(12,8), 
	"LONGITUDE" NUMBER(12,8)
   ) ;
--------------------------------------------------------
--  DDL for Table MONTH_LOOKUPS
--------------------------------------------------------

  CREATE TABLE "MEDSADMIN"."MONTH_LOOKUPS" 
   (	"MONTH_LONG" VARCHAR2(30), 
	"MONTH_SHORT" VARCHAR2(3), 
	"INT_MONTH" NUMBER(2,0), 
	"MAX_DAYS" NUMBER(2,0)
   ) ;
--------------------------------------------------------
--  DDL for Table OBSERVATION
--------------------------------------------------------

  CREATE TABLE "MEDSADMIN"."OBSERVATION" 
   (	"USER_NAME" CHAR(64), 
	"JOB_NUMBER" NUMBER(*,0), 
	"OBSERVATION_NUMBER" NUMBER(*,0)
   ) ;
--------------------------------------------------------
--  DDL for Table OBSERVATIONS
--------------------------------------------------------

  CREATE TABLE "MEDSADMIN"."OBSERVATIONS" 
   (	"MEDS_JOB_NUMBER" NUMBER(*,0), 
	"MEDS_OBSERVATION_NUMBER" NUMBER(*,0)
   ) ;
--------------------------------------------------------
--  DDL for Table OCEAN_IMAGE_DATA
--------------------------------------------------------

  CREATE TABLE "MEDSADMIN"."OCEAN_IMAGE_DATA" 
   (	"IMAGE_ID" VARCHAR2(20), 
	"FEATURE_TYPE" VARCHAR2(100), 
	"FEATURE_NAME" VARCHAR2(50), 
	"OCEAN_AREA" VARCHAR2(50), 
	"PARAMETER_OBSERVED" VARCHAR2(50), 
	"SATELLITE" VARCHAR2(50), 
	"IMAGE_SIZE" VARCHAR2(100), 
	"IMAGE_CENTRE" VARCHAR2(100), 
	"ORBIT_NUMBER" VARCHAR2(20), 
	"SOURCE_OF_IMAGE" VARCHAR2(100), 
	"STORGE_FORMAT" VARCHAR2(30), 
	"ARCHIVE_DATE" DATE, 
	"ADDITION_INFORMATION" VARCHAR2(100), 
	"FILE_REF" VARCHAR2(500), 
	"INSTRUMENT" VARCHAR2(50), 
	"IMAGE_RESOLUTION" VARCHAR2(100), 
	"MEDS_JOB_NUMBER" NUMBER(10,0), 
	"MEDS_OBSERVATION_NUMBER" NUMBER(10,0)
   ) ;
--------------------------------------------------------
--  DDL for Table OCEAN_IMAGE_OBSERVATION
--------------------------------------------------------

  CREATE TABLE "MEDSADMIN"."OCEAN_IMAGE_OBSERVATION" 
   (	"LOCATION" "MDSYS"."SDO_GEOMETRY" , 
	"OBSERVATION_DATE" DATE, 
	"MEDS_JOB_NUMBER" NUMBER(10,0), 
	"MEDS_OBSERVATION_NUMBER" NUMBER(10,0)
   ) ;
--------------------------------------------------------
--  DDL for Table OMNI_AMBIENT_DATA
--------------------------------------------------------

  CREATE TABLE "MEDSADMIN"."OMNI_AMBIENT_DATA" 
   (	"DEPTH" VARCHAR2(20), 
	"WAVE_HEIGHT" VARCHAR2(20), 
	"WIND_SPEED" VARCHAR2(20), 
	"OFP_NUMBER" VARCHAR2(20), 
	"WIND_DIRECTION" VARCHAR2(20), 
	"WAVE_DIRECTION" VARCHAR2(20), 
	"SEA_STATE" VARCHAR2(20), 
	"CONTACT_DENSITY" VARCHAR2(20), 
	"S05_10" VARCHAR2(100), 
	"S20_50" VARCHAR2(100), 
	"CONTACT_DETAILS" VARCHAR2(100), 
	"QC" VARCHAR2(20), 
	"RECORD_NUMBER" VARCHAR2(20), 
	"COMMENTS" VARCHAR2(200), 
	"MEDS_OBSERVATION_NUMBER" NUMBER(10,0), 
	"MEDS_JOB_NUMBER" NUMBER(10,0), 
	"S00_05" VARCHAR2(20), 
	"S10_20" VARCHAR2(20), 
	"SHIP" VARCHAR2(20), 
	"COUNTRY" VARCHAR2(20), 
	"PLATFORM_TYPE" VARCHAR2(50), 
	"MONTH" VARCHAR2(20), 
	"PRECIPITATION" VARCHAR2(50), 
	"TIME" VARCHAR2(25), 
	"HZ_3_15" NUMBER(15,6), 
	"HZ_4" NUMBER(15,6), 
	"HZ_5" NUMBER(15,6), 
	"HZ_6_3" NUMBER(15,6), 
	"HZ_8" NUMBER(15,6), 
	"HZ_10" NUMBER(15,6), 
	"HZ_12_5" NUMBER(15,6), 
	"HZ_16" NUMBER(15,6), 
	"HZ_20" NUMBER(15,6), 
	"HZ_25" NUMBER(15,6), 
	"HZ_31_5" NUMBER(15,6), 
	"HZ_40" NUMBER(15,6), 
	"HZ_50" NUMBER(15,6), 
	"HZ_55" NUMBER(15,6), 
	"HZ_60" NUMBER(15,6), 
	"HZ_63" NUMBER(15,6), 
	"HZ_80" NUMBER(15,6), 
	"HZ_100" NUMBER(15,6), 
	"HZ_115" NUMBER(15,6), 
	"HZ_120" NUMBER(15,6), 
	"HZ_125" NUMBER(15,6), 
	"HZ_135" NUMBER(15,6), 
	"HZ_150" NUMBER(15,6), 
	"HZ_155" NUMBER(15,6), 
	"HZ_160" NUMBER(15,6), 
	"HZ_200" NUMBER(15,6), 
	"HZ_240" NUMBER(15,6), 
	"HZ_248" NUMBER(15,6), 
	"HZ_250" NUMBER(15,6), 
	"HZ_300" NUMBER(15,6), 
	"HZ_305" NUMBER(15,6), 
	"HZ_315" NUMBER(15,6), 
	"HZ_400" NUMBER(15,6), 
	"HZ_440" NUMBER(15,6), 
	"HZ_450" NUMBER(15,6), 
	"HZ_500" NUMBER(15,6), 
	"HZ_600" NUMBER(15,6), 
	"HZ_605" NUMBER(15,6), 
	"HZ_630" NUMBER(15,6), 
	"HZ_660" NUMBER(15,6), 
	"HZ_800" NUMBER(15,6), 
	"HZ_850" NUMBER(15,6), 
	"HZ_1000" NUMBER(15,6), 
	"HZ_1150" NUMBER(15,6), 
	"HZ_1205" NUMBER(15,6), 
	"HZ_1250" NUMBER(15,6), 
	"HZ_1500" NUMBER(15,6), 
	"HZ_1600" NUMBER(15,6), 
	"HZ_1700" NUMBER(15,6), 
	"HZ_2000" NUMBER(15,6), 
	"HZ_2400" NUMBER(15,6), 
	"HZ_2500" NUMBER(15,6), 
	"HZ_3150" NUMBER(15,6), 
	"HZ_4000" NUMBER(15,6), 
	"HZ_5000" NUMBER(15,6), 
	"HZ_6300" NUMBER(15,6), 
	"HZ_8000" NUMBER(15,6), 
	"HZ_75" NUMBER(15,6), 
	"BUOY_TYPE" VARCHAR2(20), 
	"CHANNEL" NUMBER(15,6), 
	"HZ_950" NUMBER(15,6)
   ) ;
--------------------------------------------------------
--  DDL for Table OMNI_AMBIENT_OBSERVATION
--------------------------------------------------------

  CREATE TABLE "MEDSADMIN"."OMNI_AMBIENT_OBSERVATION" 
   (	"LATITUDE" NUMBER(12,6), 
	"LONGITUDE" NUMBER(12,6), 
	"LOCATION" "MDSYS"."SDO_GEOMETRY" , 
	"DATE_RECORDED" DATE, 
	"MEDS_OBSERVATION_NUMBER" NUMBER(10,0), 
	"MEDS_JOB_NUMBER" NUMBER(10,0)
   ) ;
--------------------------------------------------------
--  DDL for Table ORIGINATOR_CODE
--------------------------------------------------------

  CREATE TABLE "MEDSADMIN"."ORIGINATOR_CODE" 
   (	"CODE" VARCHAR2(100), 
	"DESCRIPTION" VARCHAR2(100)
   ) ;
--------------------------------------------------------
--  DDL for Table PRODUCTS_REGION_DATA
--------------------------------------------------------

  CREATE TABLE "MEDSADMIN"."PRODUCTS_REGION_DATA" 
   (	"LONG1" NUMBER(12,3), 
	"LONG2" NUMBER(12,3), 
	"LONG3" NUMBER(12,3), 
	"LONG4" NUMBER(12,3), 
	"LAT1" NUMBER(12,3), 
	"LAT2" NUMBER(12,3), 
	"LAT3" NUMBER(12,3), 
	"LAT4" NUMBER(12,3), 
	"WORKING_TITLE" VARCHAR2(100), 
	"FINAL_TITLE" VARCHAR2(100), 
	"TIME_PERIOD" VARCHAR2(50), 
	"BRANCH" VARCHAR2(20), 
	"VARIANT" VARCHAR2(20), 
	"EDITION" VARCHAR2(20), 
	"CLASSIFICATION" VARCHAR2(20), 
	"RELEASE_CONDITIONS" VARCHAR2(100), 
	"SOFTCOPY" VARCHAR2(3), 
	"FORMAT" VARCHAR2(20), 
	"ENCRYPTION" VARCHAR2(3), 
	"HARDCOPY" VARCHAR2(3), 
	"MEDS_JOB_NUMBER" NUMBER(10,0), 
	"MEDS_OBSERVATION_NUMBER" NUMBER(10,0), 
	"AGENCY" VARCHAR2(20), 
	"COMMENTS" VARCHAR2(250)
   ) ;
--------------------------------------------------------
--  DDL for Table PRODUCTS_REGION_OBSERVATION
--------------------------------------------------------

  CREATE TABLE "MEDSADMIN"."PRODUCTS_REGION_OBSERVATION" 
   (	"LOCATION" "MDSYS"."SDO_GEOMETRY" , 
	"TYPE" VARCHAR2(20), 
	"STATUS_DATE" DATE, 
	"UKHO_ID" VARCHAR2(20), 
	"STATUS" VARCHAR2(20), 
	"ORIGIN" VARCHAR2(20), 
	"MEDS_JOB_NUMBER" NUMBER(10,0), 
	"MEDS_OBSERVATION_NUMBER" NUMBER(10,0)
   ) ;
--------------------------------------------------------
--  DDL for Table PROFILE_DATA_SV
--------------------------------------------------------

  CREATE TABLE "MEDSADMIN"."PROFILE_DATA_SV" 
   (	"DEPTH" NUMBER(10,3), 
	"TEMPERATURE" NUMBER(10,3), 
	"SALINITY" NUMBER(10,3), 
	"SV_RECORDED" NUMBER(10,3), 
	"MEDS_JOB_NUMBER" NUMBER(10,0), 
	"MEDS_OBSERVATION_NUMBER" NUMBER(8,0), 
	"SV_QUALITY" VARCHAR2(1), 
	"D_QUALITY" VARCHAR2(1), 
	"T_QUALITY" VARCHAR2(1), 
	"S_QUALITY" VARCHAR2(1), 
	"SV_CODE" VARCHAR2(1)
   ) ;
--------------------------------------------------------
--  DDL for Table PROFILE_DATA_TONLY
--------------------------------------------------------

  CREATE TABLE "MEDSADMIN"."PROFILE_DATA_TONLY" 
   (	"DEPTH" NUMBER(10,3), 
	"TEMPERATURE" NUMBER(10,3), 
	"MEDS_JOB_NUMBER" NUMBER(10,0), 
	"MEDS_OBSERVATION_NUMBER" NUMBER(10,0), 
	"T_QUALITY" VARCHAR2(1), 
	"D_QUALITY" VARCHAR2(1)
   ) ;
--------------------------------------------------------
--  DDL for Table PROFILE_DATA_TS
--------------------------------------------------------

  CREATE TABLE "MEDSADMIN"."PROFILE_DATA_TS" 
   (	"DEPTH" NUMBER(10,3), 
	"TEMPERATURE" NUMBER(10,3), 
	"SALINITY" NUMBER(10,3), 
	"SV_CALCULATED" NUMBER(10,3), 
	"MEDS_OBSERVATION_NUMBER" NUMBER(10,0), 
	"MEDS_JOB_NUMBER" NUMBER(10,0), 
	"DEPTH_INDICATOR" VARCHAR2(1), 
	"T_QUALITY" VARCHAR2(1), 
	"SV_QUALITY" VARCHAR2(1), 
	"S_QUALITY" VARCHAR2(1), 
	"D_QUALITY" VARCHAR2(1), 
	"SV_CODE" VARCHAR2(1)
   ) ;
--------------------------------------------------------
--  DDL for Table PROFILE_HEADER_SV
--------------------------------------------------------

  CREATE TABLE "MEDSADMIN"."PROFILE_HEADER_SV" 
   (	"ADDITIONAL_POSN_REF" VARCHAR2(12), 
	"LAND_CHECK" VARCHAR2(1), 
	"HOOD_CRUISE_ID" VARCHAR2(8), 
	"MEDS_OBSERVATION_NUMBER" NUMBER(8,0), 
	"MEDS_JOB_NUMBER" NUMBER(10,0), 
	"SV_CORR" VARCHAR2(3), 
	"T_CORR" VARCHAR2(3), 
	"S_CORR" VARCHAR2(4), 
	"D_CORR" VARCHAR2(3), 
	"HOOD_STATION_NUMBER" VARCHAR2(6), 
	"WATER_COLOUR" VARCHAR2(2), 
	"WATER_TRANS" VARCHAR2(2), 
	"ATMOSPHERIC_PRESSURE" VARCHAR2(5), 
	"CLOUD" VARCHAR2(1), 
	"DRY_AIR_TEMP" VARCHAR2(4), 
	"SEA_STATE" VARCHAR2(1), 
	"WAVE_HEIGHT" VARCHAR2(2), 
	"WIND_SPEED" VARCHAR2(2), 
	"WAVE_PERIOD" VARCHAR2(2), 
	"WEATHER" VARCHAR2(1), 
	"WET_AIR_TEMP" VARCHAR2(4), 
	"WIND_DIR" VARCHAR2(2), 
	"S_SCALE_CODE" VARCHAR2(1), 
	"POSN_DETERMINATION" VARCHAR2(1), 
	"POSN_ACCURACY_CODE" VARCHAR2(1), 
	"DATA_METHOD" VARCHAR2(1), 
	"DATA_MODE" VARCHAR2(1), 
	"UNITS" VARCHAR2(1), 
	"DATA_TYPE" VARCHAR2(1), 
	"DATA_IDENTIFIER" VARCHAR2(1), 
	"FILE_FILLER" VARCHAR2(4)
   ) ;
--------------------------------------------------------
--  DDL for Table PROFILE_HEADER_TONLY
--------------------------------------------------------

  CREATE TABLE "MEDSADMIN"."PROFILE_HEADER_TONLY" 
   (	"ADDITIONAL_POSN_REF" VARCHAR2(12), 
	"LAND_CHECK" VARCHAR2(1), 
	"HOOD_CRUISE_ID" VARCHAR2(8), 
	"MEDS_OBSERVATION_NUMBER" NUMBER(8,0), 
	"MEDS_JOB_NUMBER" NUMBER(10,0), 
	"WIND_SPEED" VARCHAR2(2), 
	"WIND_DIR" VARCHAR2(2), 
	"WET_AIR_TEMP" VARCHAR2(4), 
	"DRY_AIR_TEMP" VARCHAR2(4), 
	"WAVE_HEIGHT" VARCHAR2(2), 
	"WAVE_PERIOD" VARCHAR2(2), 
	"BT_SST_REF" VARCHAR2(4), 
	"D_CORR" VARCHAR2(3), 
	"T_CORR" VARCHAR2(3), 
	"ATMOSPHERIC_PRESSURE" VARCHAR2(5), 
	"BT_SST_INSTRUMENT" VARCHAR2(1), 
	"HOOD_STATION_NUMBER" VARCHAR2(6), 
	"MBT_SURFACE_T_CORR" VARCHAR2(3), 
	"WATER_TRANS" VARCHAR2(2), 
	"WATER_COLOUR" VARCHAR2(2), 
	"MBT_TYPE_QUALITY" VARCHAR2(1), 
	"MBT_GRADE_QUALITY" VARCHAR2(1), 
	"CLOUD" VARCHAR2(1), 
	"SEA_STATE" VARCHAR2(1), 
	"WEATHER" VARCHAR2(1), 
	"S_SCALE_CODE" VARCHAR2(1), 
	"POSN_DETERMINATION" VARCHAR2(1), 
	"POSN_ACCURACY_CODE" VARCHAR2(1), 
	"DATA_METHOD" VARCHAR2(1), 
	"DATA_MODE" VARCHAR2(1), 
	"UNITS" VARCHAR2(1), 
	"DATA_TYPE" VARCHAR2(1), 
	"DATA_IDENTIFIER" VARCHAR2(1), 
	"FILE_FILLER" VARCHAR2(4)
   ) ;
--------------------------------------------------------
--  DDL for Table PROFILE_HEADER_TS
--------------------------------------------------------

  CREATE TABLE "MEDSADMIN"."PROFILE_HEADER_TS" 
   (	"ADDITIONAL_POSN_REF" VARCHAR2(12), 
	"LAND_CHECK" VARCHAR2(1), 
	"HOOD_CRUISE_ID" VARCHAR2(8), 
	"MEDS_OBSERVATION_NUMBER" NUMBER(8,0), 
	"MEDS_JOB_NUMBER" NUMBER(10,0), 
	"WEATHER" VARCHAR2(1), 
	"WIND_DIR" VARCHAR2(2), 
	"WIND_SPEED" VARCHAR2(2), 
	"T_CORR" VARCHAR2(3), 
	"S_CORR" VARCHAR2(4), 
	"DRY_AIR_TEMP" VARCHAR2(4), 
	"D_CORR" VARCHAR2(3), 
	"HOOD_STATION_NUMBER" VARCHAR2(6), 
	"WET_AIR_TEMP" VARCHAR2(4), 
	"SEA_STATE" VARCHAR2(1), 
	"CLOUD" VARCHAR2(1), 
	"WATER_COLOUR" VARCHAR2(2), 
	"WATER_TRANS" VARCHAR2(2), 
	"WAVE_PERIOD" VARCHAR2(2), 
	"SV_CORR" VARCHAR2(3), 
	"ATMOSPHERIC_PRESSURE" VARCHAR2(5), 
	"WAVE_HEIGHT" VARCHAR2(2), 
	"S_SCALE_CODE" VARCHAR2(1), 
	"POSN_DETERMINATION" VARCHAR2(1), 
	"POSN_ACCURACY_CODE" VARCHAR2(1), 
	"DATA_METHOD" VARCHAR2(1), 
	"DATA_MODE" VARCHAR2(1), 
	"UNITS" VARCHAR2(1), 
	"DATA_TYPE" VARCHAR2(1), 
	"DATA_IDENTIFIER" VARCHAR2(1), 
	"FILE_FILLER" VARCHAR2(4)
   ) ;
--------------------------------------------------------
--  DDL for Table PROFILE_INDEX_SV
--------------------------------------------------------

  CREATE TABLE "MEDSADMIN"."PROFILE_INDEX_SV" 
   (	"COMMENTS" VARCHAR2(640), 
	"DATE_TIME" DATE, 
	"LOCATION" "MDSYS"."SDO_GEOMETRY" , 
	"MEDS_JOB_NUMBER" NUMBER(10,0), 
	"MEDS_CRUISE_NUMBER" NUMBER(8,0), 
	"INSTRUMENT_CODE" NUMBER(3,0), 
	"MEDS_OBSERVATION_NUMBER" NUMBER(8,0), 
	"OBSERVED_DEPTH" NUMBER(12,4), 
	"MINIMUM_DEPTH_LEVEL" NUMBER(12,4), 
	"MAXIMUM_DEPTH_LEVEL" NUMBER(12,4), 
	"NUMBER_OF_DEPTH_LEVELS" NUMBER(5,0), 
	"MARSDEN_SQUARE" NUMBER(8,0), 
	"QUADRANT" NUMBER(2,0), 
	"DEGREE_SQURE" NUMBER(5,0), 
	"DUPLICATE_FLAG" NUMBER(1,0), 
	"YEAR" NUMBER(4,0), 
	"NO_OF_COMMENTS" NUMBER(2,0), 
	"HOOD_ARCHIVE_YEAR" NUMBER(2,0), 
	"STRING_LOCATION" VARCHAR2(15), 
	"LATITUDE" NUMBER(12,4), 
	"LONGITUDE" NUMBER(12,4), 
	"MEDS_SHIP_NUMBER" NUMBER(10,0), 
	"MONTH" NUMBER(2,0)
   ) ;
--------------------------------------------------------
--  DDL for Table PROFILE_INDEX_TONLY
--------------------------------------------------------

  CREATE TABLE "MEDSADMIN"."PROFILE_INDEX_TONLY" 
   (	"COMMENTS" VARCHAR2(640), 
	"DATE_TIME" DATE, 
	"LOCATION" "MDSYS"."SDO_GEOMETRY" , 
	"MEDS_JOB_NUMBER" NUMBER(10,0), 
	"MEDS_CRUISE_NUMBER" NUMBER(8,0), 
	"INSTRUMENT_CODE" NUMBER(3,0), 
	"MEDS_OBSERVATION_NUMBER" NUMBER(8,0), 
	"OBSERVED_DEPTH" NUMBER(12,4), 
	"MINIMUM_DEPTH_LEVEL" NUMBER(12,4), 
	"MAXIMUM_DEPTH_LEVEL" NUMBER(12,4), 
	"NUMBER_OF_DEPTH_LEVELS" NUMBER(5,0), 
	"MARSDEN_SQUARE" NUMBER(8,0), 
	"QUADRANT" NUMBER(2,0), 
	"DEGREE_SQURE" NUMBER(5,0), 
	"DUPLICATE_FLAG" NUMBER(1,0), 
	"YEAR" NUMBER(4,0), 
	"NO_OF_COMMENTS" NUMBER(2,0), 
	"HOOD_ARCHIVE_YEAR" NUMBER(2,0), 
	"STRING_LOCATION" VARCHAR2(15), 
	"LATITUDE" NUMBER(12,4), 
	"LONGITUDE" NUMBER(12,4), 
	"MEDS_SHIP_NUMBER" NUMBER(10,0), 
	"MONTH" NUMBER(2,0)
   ) ;
--------------------------------------------------------
--  DDL for Table PROFILE_INDEX_TS
--------------------------------------------------------

  CREATE TABLE "MEDSADMIN"."PROFILE_INDEX_TS" 
   (	"COMMENTS" VARCHAR2(640), 
	"DATE_TIME" DATE, 
	"LOCATION" "MDSYS"."SDO_GEOMETRY" , 
	"MEDS_JOB_NUMBER" NUMBER(10,0), 
	"MEDS_CRUISE_NUMBER" NUMBER(8,0), 
	"INSTRUMENT_CODE" NUMBER(3,0), 
	"MEDS_OBSERVATION_NUMBER" NUMBER(8,0), 
	"OBSERVED_DEPTH" NUMBER(12,4), 
	"MINIMUM_DEPTH_LEVEL" NUMBER(12,4), 
	"MAXIMUM_DEPTH_LEVEL" NUMBER(12,4), 
	"NUMBER_OF_DEPTH_LEVELS" NUMBER(5,0), 
	"MARSDEN_SQUARE" NUMBER(8,0), 
	"QUADRANT" NUMBER(2,0), 
	"DEGREE_SQURE" NUMBER(5,0), 
	"DUPLICATE_FLAG" NUMBER(1,0), 
	"YEAR" NUMBER(4,0), 
	"NO_OF_COMMENTS" NUMBER(2,0), 
	"HOOD_ARCHIVE_YEAR" NUMBER(2,0), 
	"STRING_LOCATION" VARCHAR2(15), 
	"LATITUDE" NUMBER(12,4), 
	"LONGITUDE" NUMBER(12,4), 
	"MEDS_SHIP_NUMBER" NUMBER(10,0), 
	"MONTH" NUMBER(2,0)
   ) ;
--------------------------------------------------------
--  DDL for Table PROFILE_WATER_COLOUR_LOOKUP
--------------------------------------------------------

  CREATE TABLE "MEDSADMIN"."PROFILE_WATER_COLOUR_LOOKUP" 
   (	"PROFILE_ORIGINATOR" VARCHAR2(50) DEFAULT NULL, 
	"PROFILE_FORE_ULE_SCALE" NUMBER(5,0) DEFAULT NULL, 
	"PROFILE_COLOUR_H631A_SCALE" NUMBER(5,0) DEFAULT NULL, 
	"PROFILE_COLOUR_H631A_NAME" VARCHAR2(50) DEFAULT NULL
   ) ;
--------------------------------------------------------
--  DDL for Table REPEAT_TABLES_LOOKUP
--------------------------------------------------------

  CREATE TABLE "MEDSADMIN"."REPEAT_TABLES_LOOKUP" 
   (	"DATA_TYPE_INDEX" NUMBER(8,0), 
	"REPEAT_TABLE_INDEX" NUMBER(8,0), 
	"INSERT_UPDATE" NUMBER(1,0), 
	"TABLE_NAME" VARCHAR2(50), 
	"FILE_EXTENSION" VARCHAR2(20), 
	"OFFSET" NUMBER(8,0), 
	"COUNT" NUMBER(8,0), 
	"REPEAT_INLINE" NUMBER(1,0), 
	"UPDATE_METHOD" NUMBER(1,0)
   ) ;
--------------------------------------------------------
--  DDL for Table REPRES_AMBIENTNOISE_D1
--------------------------------------------------------

  CREATE TABLE "MEDSADMIN"."REPRES_AMBIENTNOISE_D1" 
   (	"GRID_INDEX" NUMBER(8,0), 
	"TIME_OF_YEAR" VARCHAR2(50), 
	"FREQ_HZ" NUMBER(8,0), 
	"MEAN" NUMBER(10,3), 
	"NUMBER_OF_OBSERVATIONS" NUMBER(5,0), 
	"STANDARD_DEVIATION" NUMBER(9,3), 
	"MAXIMUM" NUMBER(9,3), 
	"MINIMUM" NUMBER(9,3), 
	"LATITUDE" NUMBER(12,3), 
	"LONGITUDE" NUMBER(12,3)
   ) ;
--------------------------------------------------------
--  DDL for Table REPRES_IGPADS_L1
--------------------------------------------------------

  CREATE TABLE "MEDSADMIN"."REPRES_IGPADS_L1" 
   (	"GRID_INDEX" NUMBER(8,0), 
	"HZ_SHIPPING_NOISE" NUMBER(9,3), 
	"LOW_FREQUENCY_BOTTOM_LOSS" NUMBER(1,0), 
	"HIGH_FREQUENCY_BOTTOM_LOSS" NUMBER(1,0), 
	"LATITUDE" NUMBER(12,3), 
	"LONGITUDE" NUMBER(12,3)
   ) ;
--------------------------------------------------------
--  DDL for Table REPRES_IGPADS_L2
--------------------------------------------------------

  CREATE TABLE "MEDSADMIN"."REPRES_IGPADS_L2" 
   (	"GRID_INDEX" NUMBER(8,0), 
	"HZ_SHIPPING_NOISE" NUMBER(9,3), 
	"LOW_FREQUENCY_BOTTOM_LOSS" NUMBER(1,0), 
	"HIGH_FREQUENCY_BOTTOM_LOSS" NUMBER(1,0), 
	"LATITUDE" NUMBER(12,3), 
	"LONGITUDE" NUMBER(12,3)
   ) ;
--------------------------------------------------------
--  DDL for Table REPRES_MIXEDLAYER_D1
--------------------------------------------------------

  CREATE TABLE "MEDSADMIN"."REPRES_MIXEDLAYER_D1" 
   (	"GRID_INDEX" NUMBER(8,0), 
	"MONTH_ID" NUMBER(8,0), 
	"MEAN_MLD" NUMBER(10,3), 
	"STANDARD_DEVIATION" NUMBER(10,3), 
	"MAXIMUM_MLD" NUMBER(10,3), 
	"MINIMUM_MLD" NUMBER(10,3), 
	"COEFFICIENT_VARIABILITY" NUMBER(10,3), 
	"NO_MLD" NUMBER(10,3), 
	"NUMBER_OF_OBSERVATIONS" NUMBER(6,0), 
	"MODE_MLD" NUMBER(10,3), 
	"LATITUDE" NUMBER(12,3), 
	"LONGITUDE" NUMBER(12,3)
   ) ;
--------------------------------------------------------
--  DDL for Table REPRES_MIXEDLAYER_L1
--------------------------------------------------------

  CREATE TABLE "MEDSADMIN"."REPRES_MIXEDLAYER_L1" 
   (	"GRID_INDEX" NUMBER(8,0), 
	"MONTH_ID" NUMBER(8,0), 
	"MEAN_MLD" NUMBER(10,3), 
	"STANDARD_DEVIATION" NUMBER(10,3), 
	"MAXIMUM_MLD" NUMBER(10,3), 
	"MINIMUM_MLD" NUMBER(10,3), 
	"COEFFICIENT_VARIABILITY" NUMBER(10,3), 
	"NO_MLD" NUMBER(10,3), 
	"NUMBER_OF_OBSERVATIONS" NUMBER(6,0), 
	"MODE_MLD" NUMBER(10,3), 
	"LATITUDE" NUMBER(12,3), 
	"LONGITUDE" NUMBER(12,3)
   ) ;
--------------------------------------------------------
--  DDL for Table REPRES_MIXEDLAYER_L2
--------------------------------------------------------

  CREATE TABLE "MEDSADMIN"."REPRES_MIXEDLAYER_L2" 
   (	"GRID_INDEX" NUMBER(8,0), 
	"MONTH_ID" NUMBER(8,0), 
	"MEAN_MLD" NUMBER(10,3), 
	"STANDARD_DEVIATION" NUMBER(10,3), 
	"MAXIMUM_MLD" NUMBER(10,3), 
	"MINIMUM_MLD" NUMBER(10,3), 
	"COEFFICIENT_VARIABILITY" NUMBER(10,3), 
	"NO_MLD" NUMBER(10,3), 
	"NUMBER_OF_OBSERVATIONS" NUMBER(6,0), 
	"MODE_MLD" NUMBER(10,3), 
	"LATITUDE" NUMBER(12,3), 
	"LONGITUDE" NUMBER(12,3)
   ) ;
--------------------------------------------------------
--  DDL for Table REPRES_PROFILE_L1_APR
--------------------------------------------------------

  CREATE TABLE "MEDSADMIN"."REPRES_PROFILE_L1_APR" 
   (	"GRID_INDEX" NUMBER(8,0), 
	"DEPTH" NUMBER(10,3), 
	"TEMPERATURE" NUMBER(10,3), 
	"SALINITY" NUMBER(10,3), 
	"SOUND_SPEED" NUMBER(10,3), 
	"SIGMA_T" NUMBER(10,3), 
	"LH_SALINITY" NUMBER(10,3), 
	"RH_SALINITY" NUMBER(10,3), 
	"LH_TEMPERATURE" NUMBER(10,3), 
	"RH_TEMPERATURE" NUMBER(10,3), 
	"RH_SIGMA_T" NUMBER(10,3), 
	"LH_SIGMA_T" NUMBER(10,3), 
	"RH_SOUND_SPEED" NUMBER(10,3), 
	"LH_SOUND_SPEED" NUMBER(10,3), 
	"LATITUDE" NUMBER(12,3), 
	"LONGITUDE" NUMBER(12,3), 
	"DATATYPE" VARCHAR2(20) DEFAULT 'GENERIC', 
	"TEMP_STDEV" NUMBER(10,3), 
	"SALINITY_STDEV" NUMBER(10,3)
   ) ;
--------------------------------------------------------
--  DDL for Table REPRES_PROFILE_L1_AUG
--------------------------------------------------------

  CREATE TABLE "MEDSADMIN"."REPRES_PROFILE_L1_AUG" 
   (	"GRID_INDEX" NUMBER(8,0), 
	"DEPTH" NUMBER(10,3), 
	"TEMPERATURE" NUMBER(10,3), 
	"SALINITY" NUMBER(10,3), 
	"SOUND_SPEED" NUMBER(10,3), 
	"SIGMA_T" NUMBER(10,3), 
	"LH_SALINITY" NUMBER(10,3), 
	"RH_SALINITY" NUMBER(10,3), 
	"LH_TEMPERATURE" NUMBER(10,3), 
	"RH_TEMPERATURE" NUMBER(10,3), 
	"RH_SIGMA_T" NUMBER(10,3), 
	"LH_SIGMA_T" NUMBER(10,3), 
	"RH_SOUND_SPEED" NUMBER(10,3), 
	"LH_SOUND_SPEED" NUMBER(10,3), 
	"LATITUDE" NUMBER(12,3), 
	"LONGITUDE" NUMBER(12,3), 
	"DATATYPE" VARCHAR2(20) DEFAULT 'GENERIC', 
	"TEMP_STDEV" NUMBER(10,3), 
	"SALINITY_STDEV" NUMBER(10,3)
   ) ;
--------------------------------------------------------
--  DDL for Table REPRES_PROFILE_L1_DEC
--------------------------------------------------------

  CREATE TABLE "MEDSADMIN"."REPRES_PROFILE_L1_DEC" 
   (	"GRID_INDEX" NUMBER(8,0), 
	"DEPTH" NUMBER(10,3), 
	"TEMPERATURE" NUMBER(10,3), 
	"SALINITY" NUMBER(10,3), 
	"SOUND_SPEED" NUMBER(10,3), 
	"SIGMA_T" NUMBER(10,3), 
	"LH_SALINITY" NUMBER(10,3), 
	"RH_SALINITY" NUMBER(10,3), 
	"LH_TEMPERATURE" NUMBER(10,3), 
	"RH_TEMPERATURE" NUMBER(10,3), 
	"RH_SIGMA_T" NUMBER(10,3), 
	"LH_SIGMA_T" NUMBER(10,3), 
	"RH_SOUND_SPEED" NUMBER(10,3), 
	"LH_SOUND_SPEED" NUMBER(10,3), 
	"LATITUDE" NUMBER(12,3), 
	"LONGITUDE" NUMBER(12,3), 
	"DATATYPE" VARCHAR2(20) DEFAULT 'GENERIC', 
	"TEMP_STDEV" NUMBER(10,3), 
	"SALINITY_STDEV" NUMBER(10,3)
   ) ;
--------------------------------------------------------
--  DDL for Table REPRES_PROFILE_L1_FEB
--------------------------------------------------------

  CREATE TABLE "MEDSADMIN"."REPRES_PROFILE_L1_FEB" 
   (	"GRID_INDEX" NUMBER(8,0), 
	"DEPTH" NUMBER(10,3), 
	"TEMPERATURE" NUMBER(10,3), 
	"SALINITY" NUMBER(10,3), 
	"SOUND_SPEED" NUMBER(10,3), 
	"SIGMA_T" NUMBER(10,3), 
	"LH_SALINITY" NUMBER(10,3), 
	"RH_SALINITY" NUMBER(10,3), 
	"LH_TEMPERATURE" NUMBER(10,3), 
	"RH_TEMPERATURE" NUMBER(10,3), 
	"RH_SIGMA_T" NUMBER(10,3), 
	"LH_SIGMA_T" NUMBER(10,3), 
	"RH_SOUND_SPEED" NUMBER(10,3), 
	"LH_SOUND_SPEED" NUMBER(10,3), 
	"LATITUDE" NUMBER(12,3), 
	"LONGITUDE" NUMBER(12,3), 
	"DATATYPE" VARCHAR2(20) DEFAULT 'GENERIC', 
	"TEMP_STDEV" NUMBER(10,3), 
	"SALINITY_STDEV" NUMBER(10,3)
   ) ;
--------------------------------------------------------
--  DDL for Table REPRES_PROFILE_L1_JAN
--------------------------------------------------------

  CREATE TABLE "MEDSADMIN"."REPRES_PROFILE_L1_JAN" 
   (	"GRID_INDEX" NUMBER(8,0), 
	"DEPTH" NUMBER(10,3), 
	"TEMPERATURE" NUMBER(10,3), 
	"SALINITY" NUMBER(10,3), 
	"SOUND_SPEED" NUMBER(10,3), 
	"SIGMA_T" NUMBER(10,3), 
	"LH_SALINITY" NUMBER(10,3), 
	"RH_SALINITY" NUMBER(10,3), 
	"LH_TEMPERATURE" NUMBER(10,3), 
	"RH_TEMPERATURE" NUMBER(10,3), 
	"RH_SIGMA_T" NUMBER(10,3), 
	"LH_SIGMA_T" NUMBER(10,3), 
	"RH_SOUND_SPEED" NUMBER(10,3), 
	"LH_SOUND_SPEED" NUMBER(10,3), 
	"LATITUDE" NUMBER(12,3), 
	"LONGITUDE" NUMBER(12,3), 
	"DATATYPE" VARCHAR2(20) DEFAULT 'GENERIC', 
	"TEMP_STDEV" NUMBER(10,3), 
	"SALINITY_STDEV" NUMBER(10,3)
   ) ;
--------------------------------------------------------
--  DDL for Table REPRES_PROFILE_L1_JUL
--------------------------------------------------------

  CREATE TABLE "MEDSADMIN"."REPRES_PROFILE_L1_JUL" 
   (	"GRID_INDEX" NUMBER(8,0), 
	"DEPTH" NUMBER(10,3), 
	"TEMPERATURE" NUMBER(10,3), 
	"SALINITY" NUMBER(10,3), 
	"SOUND_SPEED" NUMBER(10,3), 
	"SIGMA_T" NUMBER(10,3), 
	"LH_SALINITY" NUMBER(10,3), 
	"RH_SALINITY" NUMBER(10,3), 
	"LH_TEMPERATURE" NUMBER(10,3), 
	"RH_TEMPERATURE" NUMBER(10,3), 
	"RH_SIGMA_T" NUMBER(10,3), 
	"LH_SIGMA_T" NUMBER(10,3), 
	"RH_SOUND_SPEED" NUMBER(10,3), 
	"LH_SOUND_SPEED" NUMBER(10,3), 
	"LATITUDE" NUMBER(12,3), 
	"LONGITUDE" NUMBER(12,3), 
	"DATATYPE" VARCHAR2(20) DEFAULT 'GENERIC', 
	"TEMP_STDEV" NUMBER(10,3), 
	"SALINITY_STDEV" NUMBER(10,3)
   ) ;
--------------------------------------------------------
--  DDL for Table REPRES_PROFILE_L1_JUN
--------------------------------------------------------

  CREATE TABLE "MEDSADMIN"."REPRES_PROFILE_L1_JUN" 
   (	"GRID_INDEX" NUMBER(8,0), 
	"DEPTH" NUMBER(10,3), 
	"TEMPERATURE" NUMBER(10,3), 
	"SALINITY" NUMBER(10,3), 
	"SOUND_SPEED" NUMBER(10,3), 
	"SIGMA_T" NUMBER(10,3), 
	"LH_SALINITY" NUMBER(10,3), 
	"RH_SALINITY" NUMBER(10,3), 
	"LH_TEMPERATURE" NUMBER(10,3), 
	"RH_TEMPERATURE" NUMBER(10,3), 
	"RH_SIGMA_T" NUMBER(10,3), 
	"LH_SIGMA_T" NUMBER(10,3), 
	"RH_SOUND_SPEED" NUMBER(10,3), 
	"LH_SOUND_SPEED" NUMBER(10,3), 
	"LATITUDE" NUMBER(12,3), 
	"LONGITUDE" NUMBER(12,3), 
	"DATATYPE" VARCHAR2(20) DEFAULT 'GENERIC', 
	"TEMP_STDEV" NUMBER(10,3), 
	"SALINITY_STDEV" NUMBER(10,3)
   ) ;
--------------------------------------------------------
--  DDL for Table REPRES_PROFILE_L1_MAR
--------------------------------------------------------

  CREATE TABLE "MEDSADMIN"."REPRES_PROFILE_L1_MAR" 
   (	"GRID_INDEX" NUMBER(8,0), 
	"DEPTH" NUMBER(10,3), 
	"TEMPERATURE" NUMBER(10,3), 
	"SALINITY" NUMBER(10,3), 
	"SOUND_SPEED" NUMBER(10,3), 
	"SIGMA_T" NUMBER(10,3), 
	"LH_SALINITY" NUMBER(10,3), 
	"RH_SALINITY" NUMBER(10,3), 
	"LH_TEMPERATURE" NUMBER(10,3), 
	"RH_TEMPERATURE" NUMBER(10,3), 
	"RH_SIGMA_T" NUMBER(10,3), 
	"LH_SIGMA_T" NUMBER(10,3), 
	"RH_SOUND_SPEED" NUMBER(10,3), 
	"LH_SOUND_SPEED" NUMBER(10,3), 
	"LATITUDE" NUMBER(12,3), 
	"LONGITUDE" NUMBER(12,3), 
	"DATATYPE" VARCHAR2(20) DEFAULT 'GENERIC', 
	"TEMP_STDEV" NUMBER(10,3), 
	"SALINITY_STDEV" NUMBER(10,3)
   ) ;
--------------------------------------------------------
--  DDL for Table REPRES_PROFILE_L1_MAY
--------------------------------------------------------

  CREATE TABLE "MEDSADMIN"."REPRES_PROFILE_L1_MAY" 
   (	"GRID_INDEX" NUMBER(8,0), 
	"DEPTH" NUMBER(10,3), 
	"TEMPERATURE" NUMBER(10,3), 
	"SALINITY" NUMBER(10,3), 
	"SOUND_SPEED" NUMBER(10,3), 
	"SIGMA_T" NUMBER(10,3), 
	"LH_SALINITY" NUMBER(10,3), 
	"RH_SALINITY" NUMBER(10,3), 
	"LH_TEMPERATURE" NUMBER(10,3), 
	"RH_TEMPERATURE" NUMBER(10,3), 
	"RH_SIGMA_T" NUMBER(10,3), 
	"LH_SIGMA_T" NUMBER(10,3), 
	"RH_SOUND_SPEED" NUMBER(10,3), 
	"LH_SOUND_SPEED" NUMBER(10,3), 
	"LATITUDE" NUMBER(12,3), 
	"LONGITUDE" NUMBER(12,3), 
	"DATATYPE" VARCHAR2(20) DEFAULT 'GENERIC', 
	"TEMP_STDEV" NUMBER(10,3), 
	"SALINITY_STDEV" NUMBER(10,3)
   ) ;
--------------------------------------------------------
--  DDL for Table REPRES_PROFILE_L1_NOV
--------------------------------------------------------

  CREATE TABLE "MEDSADMIN"."REPRES_PROFILE_L1_NOV" 
   (	"GRID_INDEX" NUMBER(8,0), 
	"DEPTH" NUMBER(10,3), 
	"TEMPERATURE" NUMBER(10,3), 
	"SALINITY" NUMBER(10,3), 
	"SOUND_SPEED" NUMBER(10,3), 
	"SIGMA_T" NUMBER(10,3), 
	"LH_SALINITY" NUMBER(10,3), 
	"RH_SALINITY" NUMBER(10,3), 
	"LH_TEMPERATURE" NUMBER(10,3), 
	"RH_TEMPERATURE" NUMBER(10,3), 
	"RH_SIGMA_T" NUMBER(10,3), 
	"LH_SIGMA_T" NUMBER(10,3), 
	"RH_SOUND_SPEED" NUMBER(10,3), 
	"LH_SOUND_SPEED" NUMBER(10,3), 
	"LATITUDE" NUMBER(12,3), 
	"LONGITUDE" NUMBER(12,3), 
	"DATATYPE" VARCHAR2(20) DEFAULT 'GENERIC', 
	"TEMP_STDEV" NUMBER(10,3), 
	"SALINITY_STDEV" NUMBER(10,3)
   ) ;
--------------------------------------------------------
--  DDL for Table REPRES_PROFILE_L1_OCT
--------------------------------------------------------

  CREATE TABLE "MEDSADMIN"."REPRES_PROFILE_L1_OCT" 
   (	"GRID_INDEX" NUMBER(8,0), 
	"DEPTH" NUMBER(10,3), 
	"TEMPERATURE" NUMBER(10,3), 
	"SALINITY" NUMBER(10,3), 
	"SOUND_SPEED" NUMBER(10,3), 
	"SIGMA_T" NUMBER(10,3), 
	"LH_SALINITY" NUMBER(10,3), 
	"RH_SALINITY" NUMBER(10,3), 
	"LH_TEMPERATURE" NUMBER(10,3), 
	"RH_TEMPERATURE" NUMBER(10,3), 
	"RH_SIGMA_T" NUMBER(10,3), 
	"LH_SIGMA_T" NUMBER(10,3), 
	"RH_SOUND_SPEED" NUMBER(10,3), 
	"LH_SOUND_SPEED" NUMBER(10,3), 
	"LATITUDE" NUMBER(12,3), 
	"LONGITUDE" NUMBER(12,3), 
	"DATATYPE" VARCHAR2(20) DEFAULT 'GENERIC', 
	"TEMP_STDEV" NUMBER(10,3), 
	"SALINITY_STDEV" NUMBER(10,3)
   ) ;
--------------------------------------------------------
--  DDL for Table REPRES_PROFILE_L1_SEP
--------------------------------------------------------

  CREATE TABLE "MEDSADMIN"."REPRES_PROFILE_L1_SEP" 
   (	"GRID_INDEX" NUMBER(8,0), 
	"DEPTH" NUMBER(10,3), 
	"TEMPERATURE" NUMBER(10,3), 
	"SALINITY" NUMBER(10,3), 
	"SOUND_SPEED" NUMBER(10,3), 
	"SIGMA_T" NUMBER(10,3), 
	"LH_SALINITY" NUMBER(10,3), 
	"RH_SALINITY" NUMBER(10,3), 
	"LH_TEMPERATURE" NUMBER(10,3), 
	"RH_TEMPERATURE" NUMBER(10,3), 
	"RH_SIGMA_T" NUMBER(10,3), 
	"LH_SIGMA_T" NUMBER(10,3), 
	"RH_SOUND_SPEED" NUMBER(10,3), 
	"LH_SOUND_SPEED" NUMBER(10,3), 
	"LATITUDE" NUMBER(12,3), 
	"LONGITUDE" NUMBER(12,3), 
	"DATATYPE" VARCHAR2(20) DEFAULT 'GENERIC', 
	"TEMP_STDEV" NUMBER(10,3), 
	"SALINITY_STDEV" NUMBER(10,3)
   ) ;
--------------------------------------------------------
--  DDL for Table REPRES_PROFILE_L2_APR
--------------------------------------------------------

  CREATE TABLE "MEDSADMIN"."REPRES_PROFILE_L2_APR" 
   (	"GRID_INDEX" NUMBER(8,0), 
	"DEPTH" NUMBER(10,3), 
	"TEMPERATURE" NUMBER(10,3), 
	"SALINITY" NUMBER(10,3), 
	"SOUND_SPEED" NUMBER(10,3), 
	"SIGMA_T" NUMBER(10,3), 
	"LH_SALINITY" NUMBER(10,3), 
	"RH_SALINITY" NUMBER(10,3), 
	"LH_TEMPERATURE" NUMBER(10,3), 
	"RH_TEMPERATURE" NUMBER(10,3), 
	"RH_SIGMA_T" NUMBER(10,3), 
	"LH_SIGMA_T" NUMBER(10,3), 
	"RH_SOUND_SPEED" NUMBER(10,3), 
	"LH_SOUND_SPEED" NUMBER(10,3), 
	"LATITUDE" NUMBER(12,3), 
	"LONGITUDE" NUMBER(12,3), 
	"DATATYPE" VARCHAR2(20) DEFAULT 'GENERIC', 
	"TEMP_STDEV" NUMBER(10,3), 
	"SALINITY_STDEV" NUMBER(10,3)
   ) ;
--------------------------------------------------------
--  DDL for Table REPRES_PROFILE_L2_AUG
--------------------------------------------------------

  CREATE TABLE "MEDSADMIN"."REPRES_PROFILE_L2_AUG" 
   (	"GRID_INDEX" NUMBER(8,0), 
	"DEPTH" NUMBER(10,3), 
	"TEMPERATURE" NUMBER(10,3), 
	"SALINITY" NUMBER(10,3), 
	"SOUND_SPEED" NUMBER(10,3), 
	"SIGMA_T" NUMBER(10,3), 
	"LH_SALINITY" NUMBER(10,3), 
	"RH_SALINITY" NUMBER(10,3), 
	"LH_TEMPERATURE" NUMBER(10,3), 
	"RH_TEMPERATURE" NUMBER(10,3), 
	"RH_SIGMA_T" NUMBER(10,3), 
	"LH_SIGMA_T" NUMBER(10,3), 
	"RH_SOUND_SPEED" NUMBER(10,3), 
	"LH_SOUND_SPEED" NUMBER(10,3), 
	"LATITUDE" NUMBER(12,3), 
	"LONGITUDE" NUMBER(12,3), 
	"DATATYPE" VARCHAR2(20) DEFAULT 'GENERIC', 
	"TEMP_STDEV" NUMBER(10,3), 
	"SALINITY_STDEV" NUMBER(10,3)
   ) ;
--------------------------------------------------------
--  DDL for Table REPRES_PROFILE_L2_DEC
--------------------------------------------------------

  CREATE TABLE "MEDSADMIN"."REPRES_PROFILE_L2_DEC" 
   (	"GRID_INDEX" NUMBER(8,0), 
	"DEPTH" NUMBER(10,3), 
	"TEMPERATURE" NUMBER(10,3), 
	"SALINITY" NUMBER(10,3), 
	"SOUND_SPEED" NUMBER(10,3), 
	"SIGMA_T" NUMBER(10,3), 
	"LH_SALINITY" NUMBER(10,3), 
	"RH_SALINITY" NUMBER(10,3), 
	"LH_TEMPERATURE" NUMBER(10,3), 
	"RH_TEMPERATURE" NUMBER(10,3), 
	"RH_SIGMA_T" NUMBER(10,3), 
	"LH_SIGMA_T" NUMBER(10,3), 
	"RH_SOUND_SPEED" NUMBER(10,3), 
	"LH_SOUND_SPEED" NUMBER(10,3), 
	"LATITUDE" NUMBER(12,3), 
	"LONGITUDE" NUMBER(12,3), 
	"DATATYPE" VARCHAR2(20) DEFAULT 'GENERIC', 
	"TEMP_STDEV" NUMBER(10,3), 
	"SALINITY_STDEV" NUMBER(10,3)
   ) ;
--------------------------------------------------------
--  DDL for Table REPRES_PROFILE_L2_FEB
--------------------------------------------------------

  CREATE TABLE "MEDSADMIN"."REPRES_PROFILE_L2_FEB" 
   (	"GRID_INDEX" NUMBER(8,0), 
	"DEPTH" NUMBER(10,3), 
	"TEMPERATURE" NUMBER(10,3), 
	"SALINITY" NUMBER(10,3), 
	"SOUND_SPEED" NUMBER(10,3), 
	"SIGMA_T" NUMBER(10,3), 
	"LH_SALINITY" NUMBER(10,3), 
	"RH_SALINITY" NUMBER(10,3), 
	"LH_TEMPERATURE" NUMBER(10,3), 
	"RH_TEMPERATURE" NUMBER(10,3), 
	"RH_SIGMA_T" NUMBER(10,3), 
	"LH_SIGMA_T" NUMBER(10,3), 
	"RH_SOUND_SPEED" NUMBER(10,3), 
	"LH_SOUND_SPEED" NUMBER(10,3), 
	"LATITUDE" NUMBER(12,3), 
	"LONGITUDE" NUMBER(12,3), 
	"DATATYPE" VARCHAR2(20) DEFAULT 'GENERIC', 
	"TEMP_STDEV" NUMBER(10,3), 
	"SALINITY_STDEV" NUMBER(10,3)
   ) ;
--------------------------------------------------------
--  DDL for Table REPRES_PROFILE_L2_JAN
--------------------------------------------------------

  CREATE TABLE "MEDSADMIN"."REPRES_PROFILE_L2_JAN" 
   (	"GRID_INDEX" NUMBER(8,0), 
	"DEPTH" NUMBER(10,3), 
	"TEMPERATURE" NUMBER(10,3), 
	"SALINITY" NUMBER(10,3), 
	"SOUND_SPEED" NUMBER(10,3), 
	"SIGMA_T" NUMBER(10,3), 
	"LH_SALINITY" NUMBER(10,3), 
	"RH_SALINITY" NUMBER(10,3), 
	"LH_TEMPERATURE" NUMBER(10,3), 
	"RH_TEMPERATURE" NUMBER(10,3), 
	"RH_SIGMA_T" NUMBER(10,3), 
	"LH_SIGMA_T" NUMBER(10,3), 
	"RH_SOUND_SPEED" NUMBER(10,3), 
	"LH_SOUND_SPEED" NUMBER(10,3), 
	"LATITUDE" NUMBER(12,3), 
	"LONGITUDE" NUMBER(12,3), 
	"DATATYPE" VARCHAR2(20) DEFAULT 'GENERIC', 
	"TEMP_STDEV" NUMBER(10,3), 
	"SALINITY_STDEV" NUMBER(10,3)
   ) ;
--------------------------------------------------------
--  DDL for Table REPRES_PROFILE_L2_JUL
--------------------------------------------------------

  CREATE TABLE "MEDSADMIN"."REPRES_PROFILE_L2_JUL" 
   (	"GRID_INDEX" NUMBER(8,0), 
	"DEPTH" NUMBER(10,3), 
	"TEMPERATURE" NUMBER(10,3), 
	"SALINITY" NUMBER(10,3), 
	"SOUND_SPEED" NUMBER(10,3), 
	"SIGMA_T" NUMBER(10,3), 
	"LH_SALINITY" NUMBER(10,3), 
	"RH_SALINITY" NUMBER(10,3), 
	"LH_TEMPERATURE" NUMBER(10,3), 
	"RH_TEMPERATURE" NUMBER(10,3), 
	"RH_SIGMA_T" NUMBER(10,3), 
	"LH_SIGMA_T" NUMBER(10,3), 
	"RH_SOUND_SPEED" NUMBER(10,3), 
	"LH_SOUND_SPEED" NUMBER(10,3), 
	"LATITUDE" NUMBER(12,3), 
	"LONGITUDE" NUMBER(12,3), 
	"DATATYPE" VARCHAR2(20) DEFAULT 'GENERIC', 
	"TEMP_STDEV" NUMBER(10,3), 
	"SALINITY_STDEV" NUMBER(10,3)
   ) ;
--------------------------------------------------------
--  DDL for Table REPRES_PROFILE_L2_JUN
--------------------------------------------------------

  CREATE TABLE "MEDSADMIN"."REPRES_PROFILE_L2_JUN" 
   (	"GRID_INDEX" NUMBER(8,0), 
	"DEPTH" NUMBER(10,3), 
	"TEMPERATURE" NUMBER(10,3), 
	"SALINITY" NUMBER(10,3), 
	"SOUND_SPEED" NUMBER(10,3), 
	"SIGMA_T" NUMBER(10,3), 
	"LH_SALINITY" NUMBER(10,3), 
	"RH_SALINITY" NUMBER(10,3), 
	"LH_TEMPERATURE" NUMBER(10,3), 
	"RH_TEMPERATURE" NUMBER(10,3), 
	"RH_SIGMA_T" NUMBER(10,3), 
	"LH_SIGMA_T" NUMBER(10,3), 
	"RH_SOUND_SPEED" NUMBER(10,3), 
	"LH_SOUND_SPEED" NUMBER(10,3), 
	"LATITUDE" NUMBER(12,3), 
	"LONGITUDE" NUMBER(12,3), 
	"DATATYPE" VARCHAR2(20) DEFAULT 'GENERIC', 
	"TEMP_STDEV" NUMBER(10,3), 
	"SALINITY_STDEV" NUMBER(10,3)
   ) ;
--------------------------------------------------------
--  DDL for Table REPRES_PROFILE_L2_MAR
--------------------------------------------------------

  CREATE TABLE "MEDSADMIN"."REPRES_PROFILE_L2_MAR" 
   (	"GRID_INDEX" NUMBER(8,0), 
	"DEPTH" NUMBER(10,3), 
	"TEMPERATURE" NUMBER(10,3), 
	"SALINITY" NUMBER(10,3), 
	"SOUND_SPEED" NUMBER(10,3), 
	"SIGMA_T" NUMBER(10,3), 
	"LH_SALINITY" NUMBER(10,3), 
	"RH_SALINITY" NUMBER(10,3), 
	"LH_TEMPERATURE" NUMBER(10,3), 
	"RH_TEMPERATURE" NUMBER(10,3), 
	"RH_SIGMA_T" NUMBER(10,3), 
	"LH_SIGMA_T" NUMBER(10,3), 
	"RH_SOUND_SPEED" NUMBER(10,3), 
	"LH_SOUND_SPEED" NUMBER(10,3), 
	"LATITUDE" NUMBER(12,3), 
	"LONGITUDE" NUMBER(12,3), 
	"DATATYPE" VARCHAR2(20) DEFAULT 'GENERIC', 
	"TEMP_STDEV" NUMBER(10,3), 
	"SALINITY_STDEV" NUMBER(10,3)
   ) ;
--------------------------------------------------------
--  DDL for Table REPRES_PROFILE_L2_MAY
--------------------------------------------------------

  CREATE TABLE "MEDSADMIN"."REPRES_PROFILE_L2_MAY" 
   (	"GRID_INDEX" NUMBER(8,0), 
	"DEPTH" NUMBER(10,3), 
	"TEMPERATURE" NUMBER(10,3), 
	"SALINITY" NUMBER(10,3), 
	"SOUND_SPEED" NUMBER(10,3), 
	"SIGMA_T" NUMBER(10,3), 
	"LH_SALINITY" NUMBER(10,3), 
	"RH_SALINITY" NUMBER(10,3), 
	"LH_TEMPERATURE" NUMBER(10,3), 
	"RH_TEMPERATURE" NUMBER(10,3), 
	"RH_SIGMA_T" NUMBER(10,3), 
	"LH_SIGMA_T" NUMBER(10,3), 
	"RH_SOUND_SPEED" NUMBER(10,3), 
	"LH_SOUND_SPEED" NUMBER(10,3), 
	"LATITUDE" NUMBER(12,3), 
	"LONGITUDE" NUMBER(12,3), 
	"DATATYPE" VARCHAR2(20) DEFAULT 'GENERIC', 
	"TEMP_STDEV" NUMBER(10,3), 
	"SALINITY_STDEV" NUMBER(10,3)
   ) ;
--------------------------------------------------------
--  DDL for Table REPRES_PROFILE_L2_NOV
--------------------------------------------------------

  CREATE TABLE "MEDSADMIN"."REPRES_PROFILE_L2_NOV" 
   (	"GRID_INDEX" NUMBER(8,0), 
	"DEPTH" NUMBER(10,3), 
	"TEMPERATURE" NUMBER(10,3), 
	"SALINITY" NUMBER(10,3), 
	"SOUND_SPEED" NUMBER(10,3), 
	"SIGMA_T" NUMBER(10,3), 
	"LH_SALINITY" NUMBER(10,3), 
	"RH_SALINITY" NUMBER(10,3), 
	"LH_TEMPERATURE" NUMBER(10,3), 
	"RH_TEMPERATURE" NUMBER(10,3), 
	"RH_SIGMA_T" NUMBER(10,3), 
	"LH_SIGMA_T" NUMBER(10,3), 
	"RH_SOUND_SPEED" NUMBER(10,3), 
	"LH_SOUND_SPEED" NUMBER(10,3), 
	"LATITUDE" NUMBER(12,3), 
	"LONGITUDE" NUMBER(12,3), 
	"DATATYPE" VARCHAR2(20) DEFAULT 'GENERIC', 
	"TEMP_STDEV" NUMBER(10,3), 
	"SALINITY_STDEV" NUMBER(10,3)
   ) ;
--------------------------------------------------------
--  DDL for Table REPRES_PROFILE_L2_OCT
--------------------------------------------------------

  CREATE TABLE "MEDSADMIN"."REPRES_PROFILE_L2_OCT" 
   (	"GRID_INDEX" NUMBER(8,0), 
	"DEPTH" NUMBER(10,3), 
	"TEMPERATURE" NUMBER(10,3), 
	"SALINITY" NUMBER(10,3), 
	"SOUND_SPEED" NUMBER(10,3), 
	"SIGMA_T" NUMBER(10,3), 
	"LH_SALINITY" NUMBER(10,3), 
	"RH_SALINITY" NUMBER(10,3), 
	"LH_TEMPERATURE" NUMBER(10,3), 
	"RH_TEMPERATURE" NUMBER(10,3), 
	"RH_SIGMA_T" NUMBER(10,3), 
	"LH_SIGMA_T" NUMBER(10,3), 
	"RH_SOUND_SPEED" NUMBER(10,3), 
	"LH_SOUND_SPEED" NUMBER(10,3), 
	"LATITUDE" NUMBER(12,3), 
	"LONGITUDE" NUMBER(12,3), 
	"DATATYPE" VARCHAR2(20) DEFAULT 'GENERIC', 
	"TEMP_STDEV" NUMBER(10,3), 
	"SALINITY_STDEV" NUMBER(10,3)
   ) ;
--------------------------------------------------------
--  DDL for Table REPRES_PROFILE_L2_SEP
--------------------------------------------------------

  CREATE TABLE "MEDSADMIN"."REPRES_PROFILE_L2_SEP" 
   (	"GRID_INDEX" NUMBER(8,0), 
	"DEPTH" NUMBER(10,3), 
	"TEMPERATURE" NUMBER(10,3), 
	"SALINITY" NUMBER(10,3), 
	"SOUND_SPEED" NUMBER(10,3), 
	"SIGMA_T" NUMBER(10,3), 
	"LH_SALINITY" NUMBER(10,3), 
	"RH_SALINITY" NUMBER(10,3), 
	"LH_TEMPERATURE" NUMBER(10,3), 
	"RH_TEMPERATURE" NUMBER(10,3), 
	"RH_SIGMA_T" NUMBER(10,3), 
	"LH_SIGMA_T" NUMBER(10,3), 
	"RH_SOUND_SPEED" NUMBER(10,3), 
	"LH_SOUND_SPEED" NUMBER(10,3), 
	"LATITUDE" NUMBER(12,3), 
	"LONGITUDE" NUMBER(12,3), 
	"DATATYPE" VARCHAR2(20) DEFAULT 'GENERIC', 
	"TEMP_STDEV" NUMBER(10,3), 
	"SALINITY_STDEV" NUMBER(10,3)
   ) ;
--------------------------------------------------------
--  DDL for Table REPRES_PROFILE_L4_APR
--------------------------------------------------------

  CREATE TABLE "MEDSADMIN"."REPRES_PROFILE_L4_APR" 
   (	"GRID_INDEX" NUMBER(8,0), 
	"DEPTH" NUMBER(10,3), 
	"TEMPERATURE" NUMBER(10,3), 
	"SALINITY" NUMBER(10,3), 
	"SOUND_SPEED" NUMBER(10,3), 
	"SIGMA_T" NUMBER(10,3), 
	"LH_SALINITY" NUMBER(10,3), 
	"RH_SALINITY" NUMBER(10,3), 
	"LH_TEMPERATURE" NUMBER(10,3), 
	"RH_TEMPERATURE" NUMBER(10,3), 
	"RH_SIGMA_T" NUMBER(10,3), 
	"LH_SIGMA_T" NUMBER(10,3), 
	"RH_SOUND_SPEED" NUMBER(10,3), 
	"LH_SOUND_SPEED" NUMBER(10,3), 
	"LATITUDE" NUMBER(12,3), 
	"LONGITUDE" NUMBER(12,3), 
	"DATATYPE" VARCHAR2(20) DEFAULT 'GENERIC', 
	"TEMP_STDEV" NUMBER(10,3), 
	"SALINITY_STDEV" NUMBER(10,3)
   ) ;
--------------------------------------------------------
--  DDL for Table REPRES_PROFILE_L4_AUG
--------------------------------------------------------

  CREATE TABLE "MEDSADMIN"."REPRES_PROFILE_L4_AUG" 
   (	"GRID_INDEX" NUMBER(8,0), 
	"DEPTH" NUMBER(10,3), 
	"TEMPERATURE" NUMBER(10,3), 
	"SALINITY" NUMBER(10,3), 
	"SOUND_SPEED" NUMBER(10,3), 
	"SIGMA_T" NUMBER(10,3), 
	"LH_SALINITY" NUMBER(10,3), 
	"RH_SALINITY" NUMBER(10,3), 
	"LH_TEMPERATURE" NUMBER(10,3), 
	"RH_TEMPERATURE" NUMBER(10,3), 
	"RH_SIGMA_T" NUMBER(10,3), 
	"LH_SIGMA_T" NUMBER(10,3), 
	"RH_SOUND_SPEED" NUMBER(10,3), 
	"LH_SOUND_SPEED" NUMBER(10,3), 
	"LATITUDE" NUMBER(12,3), 
	"LONGITUDE" NUMBER(12,3), 
	"DATATYPE" VARCHAR2(20) DEFAULT 'GENERIC', 
	"TEMP_STDEV" NUMBER(10,3), 
	"SALINITY_STDEV" NUMBER(10,3)
   ) ;
--------------------------------------------------------
--  DDL for Table REPRES_PROFILE_L4_DEC
--------------------------------------------------------

  CREATE TABLE "MEDSADMIN"."REPRES_PROFILE_L4_DEC" 
   (	"GRID_INDEX" NUMBER(8,0), 
	"DEPTH" NUMBER(10,3), 
	"TEMPERATURE" NUMBER(10,3), 
	"SALINITY" NUMBER(10,3), 
	"SOUND_SPEED" NUMBER(10,3), 
	"SIGMA_T" NUMBER(10,3), 
	"LH_SALINITY" NUMBER(10,3), 
	"RH_SALINITY" NUMBER(10,3), 
	"LH_TEMPERATURE" NUMBER(10,3), 
	"RH_TEMPERATURE" NUMBER(10,3), 
	"RH_SIGMA_T" NUMBER(10,3), 
	"LH_SIGMA_T" NUMBER(10,3), 
	"RH_SOUND_SPEED" NUMBER(10,3), 
	"LH_SOUND_SPEED" NUMBER(10,3), 
	"LATITUDE" NUMBER(12,3), 
	"LONGITUDE" NUMBER(12,3), 
	"DATATYPE" VARCHAR2(20) DEFAULT 'GENERIC', 
	"TEMP_STDEV" NUMBER(10,3), 
	"SALINITY_STDEV" NUMBER(10,3)
   ) ;
--------------------------------------------------------
--  DDL for Table REPRES_PROFILE_L4_FEB
--------------------------------------------------------

  CREATE TABLE "MEDSADMIN"."REPRES_PROFILE_L4_FEB" 
   (	"GRID_INDEX" NUMBER(8,0), 
	"DEPTH" NUMBER(10,3), 
	"TEMPERATURE" NUMBER(10,3), 
	"SALINITY" NUMBER(10,3), 
	"SOUND_SPEED" NUMBER(10,3), 
	"SIGMA_T" NUMBER(10,3), 
	"LH_SALINITY" NUMBER(10,3), 
	"RH_SALINITY" NUMBER(10,3), 
	"LH_TEMPERATURE" NUMBER(10,3), 
	"RH_TEMPERATURE" NUMBER(10,3), 
	"RH_SIGMA_T" NUMBER(10,3), 
	"LH_SIGMA_T" NUMBER(10,3), 
	"RH_SOUND_SPEED" NUMBER(10,3), 
	"LH_SOUND_SPEED" NUMBER(10,3), 
	"LATITUDE" NUMBER(12,3), 
	"LONGITUDE" NUMBER(12,3), 
	"DATATYPE" VARCHAR2(20) DEFAULT 'GENERIC', 
	"TEMP_STDEV" NUMBER(10,3), 
	"SALINITY_STDEV" NUMBER(10,3)
   ) ;
--------------------------------------------------------
--  DDL for Table REPRES_PROFILE_L4_JAN
--------------------------------------------------------

  CREATE TABLE "MEDSADMIN"."REPRES_PROFILE_L4_JAN" 
   (	"GRID_INDEX" NUMBER(8,0), 
	"DEPTH" NUMBER(10,3), 
	"TEMPERATURE" NUMBER(10,3), 
	"SALINITY" NUMBER(10,3), 
	"SOUND_SPEED" NUMBER(10,3), 
	"SIGMA_T" NUMBER(10,3), 
	"LH_SALINITY" NUMBER(10,3), 
	"RH_SALINITY" NUMBER(10,3), 
	"LH_TEMPERATURE" NUMBER(10,3), 
	"RH_TEMPERATURE" NUMBER(10,3), 
	"RH_SIGMA_T" NUMBER(10,3), 
	"LH_SIGMA_T" NUMBER(10,3), 
	"RH_SOUND_SPEED" NUMBER(10,3), 
	"LH_SOUND_SPEED" NUMBER(10,3), 
	"LATITUDE" NUMBER(12,3), 
	"LONGITUDE" NUMBER(12,3), 
	"DATATYPE" VARCHAR2(20) DEFAULT 'GENERIC', 
	"TEMP_STDEV" NUMBER(10,3), 
	"SALINITY_STDEV" NUMBER(10,3)
   ) ;
--------------------------------------------------------
--  DDL for Table REPRES_PROFILE_L4_JUL
--------------------------------------------------------

  CREATE TABLE "MEDSADMIN"."REPRES_PROFILE_L4_JUL" 
   (	"GRID_INDEX" NUMBER(8,0), 
	"DEPTH" NUMBER(10,3), 
	"TEMPERATURE" NUMBER(10,3), 
	"SALINITY" NUMBER(10,3), 
	"SOUND_SPEED" NUMBER(10,3), 
	"SIGMA_T" NUMBER(10,3), 
	"LH_SALINITY" NUMBER(10,3), 
	"RH_SALINITY" NUMBER(10,3), 
	"LH_TEMPERATURE" NUMBER(10,3), 
	"RH_TEMPERATURE" NUMBER(10,3), 
	"RH_SIGMA_T" NUMBER(10,3), 
	"LH_SIGMA_T" NUMBER(10,3), 
	"RH_SOUND_SPEED" NUMBER(10,3), 
	"LH_SOUND_SPEED" NUMBER(10,3), 
	"LATITUDE" NUMBER(12,3), 
	"LONGITUDE" NUMBER(12,3), 
	"DATATYPE" VARCHAR2(20) DEFAULT 'GENERIC', 
	"TEMP_STDEV" NUMBER(10,3), 
	"SALINITY_STDEV" NUMBER(10,3)
   ) ;
--------------------------------------------------------
--  DDL for Table REPRES_PROFILE_L4_JUN
--------------------------------------------------------

  CREATE TABLE "MEDSADMIN"."REPRES_PROFILE_L4_JUN" 
   (	"GRID_INDEX" NUMBER(8,0), 
	"DEPTH" NUMBER(10,3), 
	"TEMPERATURE" NUMBER(10,3), 
	"SALINITY" NUMBER(10,3), 
	"SOUND_SPEED" NUMBER(10,3), 
	"SIGMA_T" NUMBER(10,3), 
	"LH_SALINITY" NUMBER(10,3), 
	"RH_SALINITY" NUMBER(10,3), 
	"LH_TEMPERATURE" NUMBER(10,3), 
	"RH_TEMPERATURE" NUMBER(10,3), 
	"RH_SIGMA_T" NUMBER(10,3), 
	"LH_SIGMA_T" NUMBER(10,3), 
	"RH_SOUND_SPEED" NUMBER(10,3), 
	"LH_SOUND_SPEED" NUMBER(10,3), 
	"LATITUDE" NUMBER(12,3), 
	"LONGITUDE" NUMBER(12,3), 
	"DATATYPE" VARCHAR2(20) DEFAULT 'GENERIC', 
	"TEMP_STDEV" NUMBER(10,3), 
	"SALINITY_STDEV" NUMBER(10,3)
   ) ;
--------------------------------------------------------
--  DDL for Table REPRES_PROFILE_L4_MAR
--------------------------------------------------------

  CREATE TABLE "MEDSADMIN"."REPRES_PROFILE_L4_MAR" 
   (	"GRID_INDEX" NUMBER(8,0), 
	"DEPTH" NUMBER(10,3), 
	"TEMPERATURE" NUMBER(10,3), 
	"SALINITY" NUMBER(10,3), 
	"SOUND_SPEED" NUMBER(10,3), 
	"SIGMA_T" NUMBER(10,3), 
	"LH_SALINITY" NUMBER(10,3), 
	"RH_SALINITY" NUMBER(10,3), 
	"LH_TEMPERATURE" NUMBER(10,3), 
	"RH_TEMPERATURE" NUMBER(10,3), 
	"RH_SIGMA_T" NUMBER(10,3), 
	"LH_SIGMA_T" NUMBER(10,3), 
	"RH_SOUND_SPEED" NUMBER(10,3), 
	"LH_SOUND_SPEED" NUMBER(10,3), 
	"LATITUDE" NUMBER(12,3), 
	"LONGITUDE" NUMBER(12,3), 
	"DATATYPE" VARCHAR2(20) DEFAULT 'GENERIC', 
	"TEMP_STDEV" NUMBER(10,3), 
	"SALINITY_STDEV" NUMBER(10,3)
   ) ;
--------------------------------------------------------
--  DDL for Table REPRES_PROFILE_L4_MAY
--------------------------------------------------------

  CREATE TABLE "MEDSADMIN"."REPRES_PROFILE_L4_MAY" 
   (	"GRID_INDEX" NUMBER(8,0), 
	"DEPTH" NUMBER(10,3), 
	"TEMPERATURE" NUMBER(10,3), 
	"SALINITY" NUMBER(10,3), 
	"SOUND_SPEED" NUMBER(10,3), 
	"SIGMA_T" NUMBER(10,3), 
	"LH_SALINITY" NUMBER(10,3), 
	"RH_SALINITY" NUMBER(10,3), 
	"LH_TEMPERATURE" NUMBER(10,3), 
	"RH_TEMPERATURE" NUMBER(10,3), 
	"RH_SIGMA_T" NUMBER(10,3), 
	"LH_SIGMA_T" NUMBER(10,3), 
	"RH_SOUND_SPEED" NUMBER(10,3), 
	"LH_SOUND_SPEED" NUMBER(10,3), 
	"LATITUDE" NUMBER(12,3), 
	"LONGITUDE" NUMBER(12,3), 
	"DATATYPE" VARCHAR2(20) DEFAULT 'GENERIC', 
	"TEMP_STDEV" NUMBER(10,3), 
	"SALINITY_STDEV" NUMBER(10,3)
   ) ;
--------------------------------------------------------
--  DDL for Table REPRES_PROFILE_L4_NOV
--------------------------------------------------------

  CREATE TABLE "MEDSADMIN"."REPRES_PROFILE_L4_NOV" 
   (	"GRID_INDEX" NUMBER(8,0), 
	"DEPTH" NUMBER(10,3), 
	"TEMPERATURE" NUMBER(10,3), 
	"SALINITY" NUMBER(10,3), 
	"SOUND_SPEED" NUMBER(10,3), 
	"SIGMA_T" NUMBER(10,3), 
	"LH_SALINITY" NUMBER(10,3), 
	"RH_SALINITY" NUMBER(10,3), 
	"LH_TEMPERATURE" NUMBER(10,3), 
	"RH_TEMPERATURE" NUMBER(10,3), 
	"RH_SIGMA_T" NUMBER(10,3), 
	"LH_SIGMA_T" NUMBER(10,3), 
	"RH_SOUND_SPEED" NUMBER(10,3), 
	"LH_SOUND_SPEED" NUMBER(10,3), 
	"LATITUDE" NUMBER(12,3), 
	"LONGITUDE" NUMBER(12,3), 
	"DATATYPE" VARCHAR2(20) DEFAULT 'GENERIC', 
	"TEMP_STDEV" NUMBER(10,3), 
	"SALINITY_STDEV" NUMBER(10,3)
   ) ;
--------------------------------------------------------
--  DDL for Table REPRES_PROFILE_L4_OCT
--------------------------------------------------------

  CREATE TABLE "MEDSADMIN"."REPRES_PROFILE_L4_OCT" 
   (	"GRID_INDEX" NUMBER(8,0), 
	"DEPTH" NUMBER(10,3), 
	"TEMPERATURE" NUMBER(10,3), 
	"SALINITY" NUMBER(10,3), 
	"SOUND_SPEED" NUMBER(10,3), 
	"SIGMA_T" NUMBER(10,3), 
	"LH_SALINITY" NUMBER(10,3), 
	"RH_SALINITY" NUMBER(10,3), 
	"LH_TEMPERATURE" NUMBER(10,3), 
	"RH_TEMPERATURE" NUMBER(10,3), 
	"RH_SIGMA_T" NUMBER(10,3), 
	"LH_SIGMA_T" NUMBER(10,3), 
	"RH_SOUND_SPEED" NUMBER(10,3), 
	"LH_SOUND_SPEED" NUMBER(10,3), 
	"LATITUDE" NUMBER(12,3), 
	"LONGITUDE" NUMBER(12,3), 
	"DATATYPE" VARCHAR2(20) DEFAULT 'GENERIC', 
	"TEMP_STDEV" NUMBER(10,3), 
	"SALINITY_STDEV" NUMBER(10,3)
   ) ;
--------------------------------------------------------
--  DDL for Table REPRES_PROFILE_L4_SEP
--------------------------------------------------------

  CREATE TABLE "MEDSADMIN"."REPRES_PROFILE_L4_SEP" 
   (	"GRID_INDEX" NUMBER(8,0), 
	"DEPTH" NUMBER(10,3), 
	"TEMPERATURE" NUMBER(10,3), 
	"SALINITY" NUMBER(10,3), 
	"SOUND_SPEED" NUMBER(10,3), 
	"SIGMA_T" NUMBER(10,3), 
	"LH_SALINITY" NUMBER(10,3), 
	"RH_SALINITY" NUMBER(10,3), 
	"LH_TEMPERATURE" NUMBER(10,3), 
	"RH_TEMPERATURE" NUMBER(10,3), 
	"RH_SIGMA_T" NUMBER(10,3), 
	"LH_SIGMA_T" NUMBER(10,3), 
	"RH_SOUND_SPEED" NUMBER(10,3), 
	"LH_SOUND_SPEED" NUMBER(10,3), 
	"LATITUDE" NUMBER(12,3), 
	"LONGITUDE" NUMBER(12,3), 
	"DATATYPE" VARCHAR2(20) DEFAULT 'GENERIC', 
	"TEMP_STDEV" NUMBER(10,3), 
	"SALINITY_STDEV" NUMBER(10,3)
   ) ;
--------------------------------------------------------
--  DDL for Table REPRES_SEASURFACE_L1
--------------------------------------------------------

  CREATE TABLE "MEDSADMIN"."REPRES_SEASURFACE_L1" 
   (	"GRID_INDEX" NUMBER(8,0), 
	"MONTH_ID" NUMBER(8,0), 
	"WAVE_HEIGHT" NUMBER(10,3), 
	"WAVE_STANDARD_DEVIATION" NUMBER(10,3), 
	"LATITUDE" NUMBER(12,3), 
	"LONGITUDE" NUMBER(12,3)
   ) ;
--------------------------------------------------------
--  DDL for Table REPRES_SHIPHITS_D1
--------------------------------------------------------

  CREATE TABLE "MEDSADMIN"."REPRES_SHIPHITS_D1" 
   (	"GRID_INDEX" NUMBER(8,0), 
	"MONTH_ID" NUMBER(8,0), 
	"SHIP_DENSITY" NUMBER(10,3), 
	"LATITUDE" NUMBER(12,3), 
	"LONGITUDE" NUMBER(12,3)
   ) ;
--------------------------------------------------------
--  DDL for Table REPRES_SPECIES_L1
--------------------------------------------------------

  CREATE TABLE "MEDSADMIN"."REPRES_SPECIES_L1" 
   (	"GRID_INDEX" NUMBER(8,0), 
	"TIME_OF_YR" VARCHAR2(50), 
	"SPECIES_TYPE" VARCHAR2(20), 
	"SPECIES_NAME" VARCHAR2(30), 
	"DEPTH_RANGE" VARCHAR2(30), 
	"ABUNDANCE" NUMBER(10,3), 
	"LATITUDE" NUMBER(12,3), 
	"LONGITUDE" NUMBER(12,3)
   ) ;
--------------------------------------------------------
--  DDL for Table ROWS_PER_JOB
--------------------------------------------------------

  CREATE TABLE "MEDSADMIN"."ROWS_PER_JOB" 
   (	"BOOKING_JOB" NUMBER(*,0), 
	"TABLE_TYPE" CHAR(64), 
	"N_ROWS" NUMBER(*,0)
   ) ;
--------------------------------------------------------
--  DDL for Table SEASOAR_PROFILE_DATA
--------------------------------------------------------

  CREATE TABLE "MEDSADMIN"."SEASOAR_PROFILE_DATA" 
   (	"MEDS_JOB_NUMBER" NUMBER(10,0), 
	"MEDS_OBSERVATION_NUMBER" NUMBER(10,0), 
	"DEPTH" NUMBER(12,4), 
	"TEMPERATURE" NUMBER(12,4), 
	"SALINITY" NUMBER(12,4), 
	"SOUND_SPEED" NUMBER(12,4), 
	"DENSITY" NUMBER(12,4), 
	"TURBIDITY" NUMBER(20,10), 
	"CHLOROPHYLL" NUMBER(20,10), 
	"HYDROCARBONS" NUMBER(20,10), 
	"GELBSTOFFE" NUMBER(20,10), 
	"ATTENUATION" NUMBER(20,10), 
	"BIOLUMINESCENCE" NUMBER(20,4), 
	"PROFILE_ID" NUMBER(10,0)
   ) ;
--------------------------------------------------------
--  DDL for Table SEASOAR_PROFILE_OBSERVATION
--------------------------------------------------------

  CREATE TABLE "MEDSADMIN"."SEASOAR_PROFILE_OBSERVATION" 
   (	"LOCATION" "MDSYS"."SDO_GEOMETRY" , 
	"MEDS_JOB_NUMBER" NUMBER(10,0), 
	"MEDS_OBSERVATION_NUMBER" NUMBER(10,0), 
	"LONGITUDE" NUMBER(12,4), 
	"LATITUDE" NUMBER(12,4), 
	"DATE_RECORDED" DATE, 
	"PROFILE_ID" NUMBER(10,0)
   ) ;
--------------------------------------------------------
--  DDL for Table SEASOAR_TOW_DATA
--------------------------------------------------------

  CREATE TABLE "MEDSADMIN"."SEASOAR_TOW_DATA" 
   (	"MEDS_JOB_NUMBER" NUMBER(10,0), 
	"MEDS_OBSERVATION_NUMBER" NUMBER(10,0), 
	"DEPTH" NUMBER(12,4), 
	"PRESSURE" NUMBER(12,4), 
	"TEMPERATURE" NUMBER(12,4), 
	"SALINITY" NUMBER(12,4), 
	"SOUND_SPEED" NUMBER(12,4), 
	"DENSITY" NUMBER(12,4), 
	"TURBIDITY" NUMBER(20,10), 
	"CHLOROPHYLL" NUMBER(20,10), 
	"HYDROCARBONS" NUMBER(20,10), 
	"GELBSTOFFE" NUMBER(20,10), 
	"ATTENUATION" NUMBER(20,10), 
	"BIOLUMINESCENCE" NUMBER(20,4), 
	"LATITUDE" NUMBER(12,4), 
	"LONGITUDE" NUMBER(12,4), 
	"DATE_RECORDED" DATE
   ) ;
--------------------------------------------------------
--  DDL for Table SEASOAR_TOW_OBSERVATION
--------------------------------------------------------

  CREATE TABLE "MEDSADMIN"."SEASOAR_TOW_OBSERVATION" 
   (	"LOCATION" "MDSYS"."SDO_GEOMETRY" , 
	"MEDS_JOB_NUMBER" NUMBER(10,0), 
	"MEDS_OBSERVATION_NUMBER" NUMBER(10,0), 
	"LONGITUDE" NUMBER(12,4), 
	"LATITUDE" NUMBER(12,4), 
	"DATE_RECORDED" DATE
   ) ;
--------------------------------------------------------
--  DDL for Table SECCHI_DISK_DATA
--------------------------------------------------------

  CREATE TABLE "MEDSADMIN"."SECCHI_DISK_DATA" 
   (	"RECORD_NUMBER" NUMBER(6,0), 
	"WATER_COLOUR" VARCHAR2(20), 
	"SUN_ANGLE" VARCHAR2(10), 
	"REFERENCE" VARCHAR2(100), 
	"GENERAL_AREA" VARCHAR2(100), 
	"STN_IDENTIFIER" VARCHAR2(50), 
	"PHOTIC_DEPTH" NUMBER(12,4), 
	"HORIZONTAL_VISABILITY" NUMBER(12,4), 
	"WEATHER" NUMBER(5,0), 
	"CLOUD" NUMBER(5,0), 
	"SHIP" VARCHAR2(100), 
	"CRUISE_IDENTIFIER" VARCHAR2(100), 
	"MEDS_OBSERVATION_NUMBER" NUMBER(10,0), 
	"MEDS_JOB_NUMBER" NUMBER(10,0), 
	"ARCHIVED" VARCHAR2(20), 
	"DIFFUSE_ATTN_COE" VARCHAR2(20), 
	"MONTH" VARCHAR2(10), 
	"GMT" VARCHAR2(20), 
	"SEA_STATE" VARCHAR2(10), 
	"SECCHI_DEPTH" NUMBER(12,4), 
	"WATER_COLOUR_H631A" NUMBER(5,0) DEFAULT NULL, 
	"WIND_DIRECTION" NUMBER(5,0), 
	"WIND_SPEED" NUMBER(5,0), 
	"SWELL_DIRECTION" NUMBER(5,0), 
	"SWELL_PERIOD" NUMBER(5,0), 
	"ATMOS_PRESSURE" NUMBER(5,1), 
	"DRYAIR_TEMPERATURE" NUMBER(5,1), 
	"WETAIR_TEMPERATURE" NUMBER(5,1)
   ) ;

   COMMENT ON TABLE "MEDSADMIN"."SECCHI_DISK_DATA"  IS 'DIFFUSE_ATTN_COE = 1.7/SECCHI_DEPTH 
PHOTIC_DEPTH = 3*SECCHI_DEPTH 
HORIZONTAL_VISABILITY = 0.7*SECCHI_DEPTH';
--------------------------------------------------------
--  DDL for Table SECCHI_DISK_DATA_PRE_CR71887
--------------------------------------------------------

  CREATE TABLE "MEDSADMIN"."SECCHI_DISK_DATA_PRE_CR71887" 
   (	"RECORD_NUMBER" NUMBER(6,0), 
	"WATER_COLOUR" VARCHAR2(20), 
	"SUN_ANGLE" VARCHAR2(10), 
	"REFERENCE" VARCHAR2(100), 
	"GENERAL_AREA" VARCHAR2(100), 
	"STN_IDENTIFIER" VARCHAR2(50), 
	"PHOTIC_DEPTH" NUMBER(12,4), 
	"HORIZONTAL_VISABILITY" NUMBER(12,4), 
	"WEATHER" NUMBER(5,0), 
	"CLOUD" NUMBER(5,0), 
	"SHIP" VARCHAR2(100), 
	"CRUISE_IDENTIFIER" VARCHAR2(100), 
	"MEDS_OBSERVATION_NUMBER" NUMBER(10,0), 
	"MEDS_JOB_NUMBER" NUMBER(10,0), 
	"ARCHIVED" VARCHAR2(20), 
	"DIFFUSE_ATTN_COE" VARCHAR2(20), 
	"MONTH" VARCHAR2(10), 
	"GMT" VARCHAR2(20), 
	"SEA_STATE" VARCHAR2(10), 
	"SECCHI_DEPTH" NUMBER(12,4)
   ) ;
--------------------------------------------------------
--  DDL for Table SECCHI_DISK_OBSERVATION
--------------------------------------------------------

  CREATE TABLE "MEDSADMIN"."SECCHI_DISK_OBSERVATION" 
   (	"MEDS_OBSERVATION_NUMBER" NUMBER(10,0), 
	"MEDS_JOB_NUMBER" NUMBER(10,0), 
	"LOCATION" "MDSYS"."SDO_GEOMETRY" , 
	"DATE_RECORDED" DATE, 
	"LATITUDE" NUMBER(12,8), 
	"LONGITUDE" NUMBER(12,8)
   ) ;
--------------------------------------------------------
--  DDL for Table SECCHI_WATER_COLOUR_LOOKUP
--------------------------------------------------------

  CREATE TABLE "MEDSADMIN"."SECCHI_WATER_COLOUR_LOOKUP" 
   (	"SECCHI_COLOUR_NAME" VARCHAR2(20) DEFAULT NULL, 
	"SECCHI_COLOUR_H631A_SCALE" NUMBER(5,0) DEFAULT NULL
   ) ;
--------------------------------------------------------
--  DDL for Table SHIPPING_DATA
--------------------------------------------------------

  CREATE TABLE "MEDSADMIN"."SHIPPING_DATA" 
   (	"FLAG" VARCHAR2(6), 
	"SHIP_NAME" VARCHAR2(100), 
	"IRCS" VARCHAR2(10), 
	"IMO" VARCHAR2(20), 
	"MMSI" VARCHAR2(20), 
	"COURSE" NUMBER(8,3), 
	"SPEED" NUMBER(8,3), 
	"DESTINATION" VARCHAR2(50), 
	"ETA" DATE, 
	"COI" VARCHAR2(20), 
	"LENGTH" NUMBER(6,0), 
	"REMARKS" VARCHAR2(100), 
	"MEDS_JOB_NUMBER" NUMBER(10,0), 
	"MEDS_OBSERVATION_NUMBER" NUMBER(10,0)
   ) ;
--------------------------------------------------------
--  DDL for Table SHIPPING_OBSERVATION
--------------------------------------------------------

  CREATE TABLE "MEDSADMIN"."SHIPPING_OBSERVATION" 
   (	"MEDS_OBSERVATION_NUMBER" NUMBER(10,0), 
	"MEDS_JOB_NUMBER" NUMBER(10,0), 
	"LOCATION" "MDSYS"."SDO_GEOMETRY" , 
	"DATE_RECORDED" DATE, 
	"LATITUDE" NUMBER(12,8), 
	"LONGITUDE" NUMBER(12,8)
   ) ;
--------------------------------------------------------
--  DDL for Table SHIP_CODES_LOOKUP
--------------------------------------------------------

  CREATE TABLE "MEDSADMIN"."SHIP_CODES_LOOKUP" 
   (	"SHIP_NAME" VARCHAR2(32), 
	"YEAR" NUMBER, 
	"SHIP_CODE" VARCHAR2(4), 
	"COUNTRY_CODE" VARCHAR2(4)
   ) ;
--------------------------------------------------------
--  DDL for Table SHIP_DETAILS
--------------------------------------------------------

  CREATE TABLE "MEDSADMIN"."SHIP_DETAILS" 
   (	"COUNTRY_CODE" VARCHAR2(2), 
	"ICES_SHIP_CODE" VARCHAR2(2), 
	"MIAS_INSTITUTE_CODE" VARCHAR2(3), 
	"MIAS_INSTITUTE_FLAG" NUMBER(2,0), 
	"ICES_SHIP_FLAG" NUMBER(2,0), 
	"VESSEL_NAME" VARCHAR2(50), 
	"MEDS_SHIP_NUMBER" NUMBER(10,0), 
	"MEDS_CRUISE_NUMBER" NUMBER(10,0)
   ) ;
--------------------------------------------------------
--  DDL for Table SPATIAL_DUFF
--------------------------------------------------------

  CREATE TABLE "MEDSADMIN"."SPATIAL_DUFF" 
   (	"LOCATION" "MDSYS"."SDO_GEOMETRY" , 
	"MEDS_JOB_NUMBER" NUMBER(10,0), 
	"MEDS_OBSERVATION_NUMBER" NUMBER(10,0), 
	"MID_LATITUDE" NUMBER(12,6), 
	"MID_LONGITUDE" NUMBER(12,6), 
	"PROFILE_ID" NUMBER(10,0), 
	"PROFILE_DIR" NUMBER(2,0), 
	"DISTANCE_TRAVELED" NUMBER(12,4)
   ) ;
--------------------------------------------------------
--  DDL for Table STANDARD_LOOKUP
--------------------------------------------------------

  CREATE TABLE "MEDSADMIN"."STANDARD_LOOKUP" 
   (	"DESCRIPTION" VARCHAR2(50), 
	"TABLE_NAME" VARCHAR2(50), 
	"COLUMN_NAME" VARCHAR2(50)
   ) ;
--------------------------------------------------------
--  DDL for Table STATS_PERIOD
--------------------------------------------------------

  CREATE TABLE "MEDSADMIN"."STATS_PERIOD" 
   (	"PERIOD_START" DATE, 
	"PERIOD_END" DATE
   ) ;
--------------------------------------------------------
--  DDL for Table SUMMARY_2019_US
--------------------------------------------------------

  CREATE TABLE "MEDSADMIN"."SUMMARY_2019_US" 
   (	"SUPPLIER" VARCHAR2(64), 
	"DESCRIPTION" VARCHAR2(200), 
	"CRUISE_NAME" VARCHAR2(50), 
	"OBS_COUNT" NUMBER, 
	"DATA_USE_CODE" VARCHAR2(20), 
	"DATATYPE" CHAR(5), 
	"MEIC_NUMBER" NUMBER
   ) ;
--------------------------------------------------------
--  DDL for Table SUMMARY_2021_US
--------------------------------------------------------

  CREATE TABLE "MEDSADMIN"."SUMMARY_2021_US" 
   (	"SUPPLIER" VARCHAR2(64), 
	"DESCRIPTION" VARCHAR2(200), 
	"CRUISE_NAME" VARCHAR2(50), 
	"OBS_COUNT" NUMBER, 
	"DATA_USE_CODE" VARCHAR2(20), 
	"DATATYPE" CHAR(5), 
	"MEIC_NUMBER" NUMBER
   ) ;
--------------------------------------------------------
--  DDL for Table SUPPORT_TABLES
--------------------------------------------------------

  CREATE TABLE "MEDSADMIN"."SUPPORT_TABLES" 
   (	"NAME" VARCHAR2(50)
   ) ;
--------------------------------------------------------
--  DDL for Table TABLE_INDEX_LOOKUP
--------------------------------------------------------

  CREATE TABLE "MEDSADMIN"."TABLE_INDEX_LOOKUP" 
   (	"TABLE_NAME" VARCHAR2(50), 
	"FIELD_NAME" VARCHAR2(50), 
	"INDEX_TABLE" VARCHAR2(50), 
	"INDEX_FIELD" VARCHAR2(50), 
	"SINGLE_REPEAT" NUMBER(1,0)
   ) ;
--------------------------------------------------------
--  DDL for Table TAB_COLUMNS
--------------------------------------------------------

  CREATE TABLE "MEDSADMIN"."TAB_COLUMNS" 
   (	"TABLE_NAME" VARCHAR2(30), 
	"COLUMN_NAME" VARCHAR2(30), 
	"TAB_NUMBER" NUMBER, 
	"TAB_NAME" VARCHAR2(20), 
	"EDIT_ORDER" NUMBER, 
	"MANDATORY" NUMBER, 
	"CONSTRAINT" NUMBER
   ) ;
--------------------------------------------------------
--  DDL for Table TAB_COLUMNS_COPY
--------------------------------------------------------

  CREATE TABLE "MEDSADMIN"."TAB_COLUMNS_COPY" 
   (	"TABLE_NAME" VARCHAR2(30), 
	"COLUMN_NAME" VARCHAR2(30), 
	"TAB_NUMBER" NUMBER, 
	"TAB_NAME" VARCHAR2(20), 
	"EDIT_ORDER" NUMBER, 
	"MANDATORY" NUMBER, 
	"CONSTRAINT" NUMBER
   ) ;
--------------------------------------------------------
--  DDL for Table TEMP_JOBS
--------------------------------------------------------

  CREATE TABLE "MEDSADMIN"."TEMP_JOBS" 
   (	"MEDS_JOB_ID" NUMBER, 
	"PO_NO" VARCHAR2(12), 
	"INGRES_NO" NUMBER, 
	"DATE_RECD" DATE, 
	"SUPPLIER" VARCHAR2(128), 
	"ACKNOWLEDGE" CHAR(1), 
	"PRIORITY_REGION1" CHAR(1), 
	"PRIORITY_REGION2" CHAR(1), 
	"PRIORITY_REGION3" CHAR(1), 
	"PRIORITY_REGION4" CHAR(1), 
	"PRIORITY_REGION5" CHAR(1), 
	"PRIORITY_REGION6" CHAR(1), 
	"NO_OF_OBS_APPROX" NUMBER, 
	"COLLECTION_START" DATE, 
	"COLLECTION_END" DATE, 
	"QA_PROCESSING_TARGET_DATE" DATE, 
	"QA_PROCESSING_COMPLETED" DATE, 
	"HOOD_LOAD_DATE_" DATE, 
	"OBS_ACCEPTED" NUMBER, 
	"OBS_REJECTED" NUMBER, 
	"MORS_OR_OTHER_SV_DATA" CHAR(1), 
	"CTD" CHAR(1), 
	"XCTD" CHAR(1), 
	"VOS" CHAR(1), 
	"ARGO" CHAR(1), 
	"SEABED_SAMPLES_H575" CHAR(1), 
	"XSV" CHAR(1), 
	"XBT" CHAR(1), 
	"OMNI_DIRECTIONAL" CHAR(1), 
	"TOWED_ARRAY" CHAR(1), 
	"DIRECTIONAL" CHAR(1), 
	"SECCHI" CHAR(1), 
	"BIOLUMIN_H636" CHAR(1), 
	"MARINE_LIFE_H637" CHAR(1), 
	"NEIL_BROWN_BIOLOGY" CHAR(1), 
	"SONAR_2081" CHAR(1), 
	"AQUASHUTTLE" CHAR(1), 
	"CHLOROPHYLL" CHAR(1), 
	"MISCELLANEOUS" CHAR(1), 
	"EXCHANGE" CHAR(1), 
	"FRONTS" CHAR(1), 
	"SEA_ICE" CHAR(1), 
	"INTERNAL_WAVES" CHAR(1), 
	"COMMENTS" VARCHAR2(512), 
	"PO_NUMBER" NUMBER DEFAULT NULL, 
	"PO_TASK" CHAR(1) DEFAULT NULL, 
	"JOB_TYPE_NUMBER" NUMBER DEFAULT NULL
   ) ;
--------------------------------------------------------
--  DDL for Table TIFF_CHARTS
--------------------------------------------------------

  CREATE TABLE "MEDSADMIN"."TIFF_CHARTS" 
   (	"FILENAME" CHAR(64), 
	"SIZE_DEGREES" NUMBER, 
	"LIMITS" "MDSYS"."SDO_GEOMETRY" 
   ) ;
--------------------------------------------------------
--  DDL for Table TIFF_CHARTS_BACKUP
--------------------------------------------------------

  CREATE TABLE "MEDSADMIN"."TIFF_CHARTS_BACKUP" 
   (	"FILENAME" CHAR(64), 
	"SIZE_DEGREES" NUMBER, 
	"LIMITS" "MDSYS"."SDO_GEOMETRY" 
   ) ;
--------------------------------------------------------
--  DDL for Table TMP_FIELD_CHECK
--------------------------------------------------------

  CREATE TABLE "MEDSADMIN"."TMP_FIELD_CHECK" 
   (	"TABLE_NAME" VARCHAR2(50), 
	"FIELD_NAME" VARCHAR2(50), 
	"FOUND" NUMBER(*,0)
   ) ;
--------------------------------------------------------
--  DDL for Table TSSV_COUNT
--------------------------------------------------------

  CREATE TABLE "MEDSADMIN"."TSSV_COUNT" 
   (	"TYPE" VARCHAR2(50), 
	"TOTAL" NUMBER
   ) ;
--------------------------------------------------------
--  DDL for Table TSSV_INSTRUMENT
--------------------------------------------------------

  CREATE TABLE "MEDSADMIN"."TSSV_INSTRUMENT" 
   (	"INSTRUMENT" VARCHAR2(50), 
	"TOTAL" NUMBER
   ) ;
--------------------------------------------------------
--  DDL for Table UNIFIED_SECCHI_DATA
--------------------------------------------------------

  CREATE TABLE "MEDSADMIN"."UNIFIED_SECCHI_DATA" 
   (	"DATA_TYPE" VARCHAR2(8), 
	"RECORD_NUMBER" NUMBER(6,0), 
	"WATER_COLOUR" NUMBER(5,0), 
	"WATER_COLOUR_NAME" VARCHAR2(20), 
	"SUN_ANGLE" VARCHAR2(10), 
	"REFERENCE" VARCHAR2(100), 
	"GENERAL_AREA" VARCHAR2(100), 
	"STN_IDENTIFIER" VARCHAR2(50), 
	"PHOTIC_DEPTH" NUMBER(12,4), 
	"HORIZONTAL_VISABILITY" NUMBER(12,4), 
	"WEATHER" NUMBER(5,0), 
	"CLOUD" NUMBER(5,0), 
	"SHIP" VARCHAR2(100), 
	"SHIP_CODE" VARCHAR2(2), 
	"COUNTRY_CODE" VARCHAR2(2), 
	"CRUISE_IDENTIFIER" VARCHAR2(100), 
	"MEDS_OBSERVATION_NUMBER" NUMBER(10,0), 
	"MEDS_JOB_NUMBER" NUMBER(10,0), 
	"ARCHIVED" VARCHAR2(20), 
	"DIFFUSE_ATTN_COE" VARCHAR2(20), 
	"MONTH" NUMBER(2,0), 
	"GMT" VARCHAR2(20), 
	"SEA_STATE" VARCHAR2(10), 
	"SECCHI_DEPTH" NUMBER(12,4), 
	"SRC" VARCHAR2(7)
   ) ;
--------------------------------------------------------
--  DDL for Table UNIFIED_SECCHI_OBSERVATION
--------------------------------------------------------

  CREATE TABLE "MEDSADMIN"."UNIFIED_SECCHI_OBSERVATION" 
   (	"MEDS_OBSERVATION_NUMBER" NUMBER(10,0), 
	"MEDS_JOB_NUMBER" NUMBER(10,0), 
	"LOCATION" "MDSYS"."SDO_GEOMETRY" , 
	"DATE_RECORDED" DATE, 
	"LATITUDE" NUMBER(12,8), 
	"LONGITUDE" NUMBER(12,8), 
	"SRC" VARCHAR2(7)
   ) ;
--------------------------------------------------------
--  DDL for Table UNIFIED_WATER_COLOUR_LOOKUP
--------------------------------------------------------

  CREATE TABLE "MEDSADMIN"."UNIFIED_WATER_COLOUR_LOOKUP" 
   (	"UNIFIED_COLOUR_H631A_NAME" VARCHAR2(20) DEFAULT NULL, 
	"UNIFIED_COLOUR_H631A_SCALE" NUMBER(5,0) DEFAULT NULL
   ) ;
--------------------------------------------------------
--  DDL for Table VAL_RESULTS
--------------------------------------------------------

  CREATE TABLE "MEDSADMIN"."VAL_RESULTS" 
   (	"SDO_ROWID" ROWID, 
	"RESULT" VARCHAR2(1000)
   ) ;
--------------------------------------------------------
--  DDL for Table WATER_COLOUR
--------------------------------------------------------

  CREATE TABLE "MEDSADMIN"."WATER_COLOUR" 
   (	"WATER_COLOUR" NUMBER(5,0), 
	"COLOUR_DESC" VARCHAR2(20)
   ) ;
--------------------------------------------------------
--  DDL for Table WVS_3M
--------------------------------------------------------

  CREATE TABLE "MEDSADMIN"."WVS_3M" 
   (	"OBJECTID" NUMBER(38,0), 
	"ID" NUMBER(11,0), 
	"FAC_ID" NUMBER(11,0), 
	"FACC" VARCHAR2(5), 
	"FACC_DESCR" VARCHAR2(254), 
	"CVH" NUMBER(6,0), 
	"CVL" NUMBER(6,0), 
	"TILE_ID" NUMBER(6,0), 
	"SHAPE" "MDSYS"."SDO_GEOMETRY" , 
	"SE_ANNO_CAD_DATA" BLOB
   ) ;
--------------------------------------------------------
--  DDL for Table WVS_3M_BATHY_LINE
--------------------------------------------------------

  CREATE TABLE "MEDSADMIN"."WVS_3M_BATHY_LINE" 
   (	"OBJECTID" NUMBER(*,0), 
	"ID" NUMBER(11,0), 
	"EDG_ID" NUMBER(11,0), 
	"FACC" VARCHAR2(5), 
	"FACC_DESCR" VARCHAR2(254), 
	"CRV" NUMBER(6,0), 
	"TILE_ID" NUMBER(6,0), 
	"SHAPE" "MDSYS"."SDO_GEOMETRY" , 
	"SE_ANNO_CAD_DATA" BLOB
   ) ;
--------------------------------------------------------
--  DDL for Table WVS_D_3M_COUNTPOLY
--------------------------------------------------------

  CREATE TABLE "MEDSADMIN"."WVS_D_3M_COUNTPOLY" 
   (	"OBJECTID" NUMBER(*,0), 
	"NAM_DESCRI" VARCHAR2(254), 
	"COUNT" NUMBER(11,0), 
	"SHAPE" "MDSYS"."SDO_GEOMETRY" , 
	"SE_ANNO_CAD_DATA" BLOB
   ) ;
