...
CREATE TABLE medsadmin.serd_code_loookup (
    serd_code_loookup NUMBER NOT NULL,
...
COMMENT ON COLUMN medsadmin.serd_code_loookup.column_offset IS
    'Unique identifier of the code stored, as the position of the related field in the SERD file. ';

COMMENT ON COLUMN medsadmin.serd_code_loookup.code_value IS
    'Code value.';

COMMENT ON COLUMN medsadmin.serd_code_loookup.label IS
    'Code description';

CREATE UNIQUE INDEX medsadmin.serd_lov_code_lookup_p ON
    medsadmin.serd_code_loookup (
        serd_code_loookup
    ASC );

CREATE UNIQUE INDEX medsadmin.serd_lov_code_lookup_vl_u ON
    medsadmin.serd_code_loookup (
        column_offset
    ASC,
        code_value
...
GRANT SELECT ON medsadmin.serd_code_loookup TO cm;

ALTER TABLE medsadmin.serd_code_loookup ADD CONSTRAINT serd_code_loookup_p PRIMARY KEY ( serd_code_loookup );

ALTER TABLE medsadmin.serd_code_loookup ADD CONSTRAINT serd_code_loookup_value_u UNIQUE ( column_offset,
                                                                                          code_value );
...
CREATE OR REPLACE TRIGGER MEDSADMIN.BI_SERD_CODE_LOOOKUP 
    BEFORE INSERT ON MEDSADMIN.SERD_CODE_LOOOKUP 
    FOR EACH ROW 
begin
   if :new.SERD_LOV_CODE_LOOKUP is null then
      :new.SERD_LOV_CODE_LOOKUP := to_number(sys_guid(),'xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx');
   end if;
end bi_serd_code_lookup; 
/

CREATE OR REPLACE TRIGGER MEDSADMIN.BI_SERD_LOV_SEA_STATE 
    BEFORE INSERT ON MEDSADMIN.SERD_SEA_STATE_LOOKUP 
    FOR EACH ROW 
begin
   if :new.SERD_LOV_SEA_STATE is null then
      :new.SERD_LOV_SEA_STATE := to_number(sys_guid(),'xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx');
   end if;
end bi_serd_lov_sea_state; 
/
